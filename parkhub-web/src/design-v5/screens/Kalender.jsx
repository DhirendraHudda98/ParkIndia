...
export function KalenderV5({ navigate }) {
    ...
    const { data: events = [], isLoading, isError } = useQuery({
        ...
        queryFn: async () => {
            ...
            if (!res.success)
                throw new Error(res.error?.message ?? 'Kalenderdaten konnten nicht geladen werden');
            return res.data ?? [];
        },
        staleTime: 30_000,
    ...
    return (<div style={{ padding: 16, flex: 1, overflow: 'auto', display: 'flex', flexDirection: 'column', gap: 12 }}>
      <div className="v5-ani" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
          <button type="button" aria-label="Vorheriger Monat" onClick={() => setCurrentMonth((d) => new Date(d.getFullYear(), d.getMonth() - 1, 1))} style={{ width: 32, height: 32, borderRadius: 8, background: 'var(--v5-sur2)', border: '1px solid var(--v5-bor)', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', transform: 'rotate(180deg)' }}>
          ...
          <button type="button" aria-label="Nächster Monat" onClick={() => setCurrentMonth((d) => new Date(d.getFullYear(), d.getMonth() + 1, 1))} style={{ width: 32, height: 32, borderRadius: 8, background: 'var(--v5-sur2)', border: '1px solid var(--v5-bor)', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <V5NamedIcon name="chev" size={12} color="var(--v5-mut)"/>
          </button>
          <button type="button" onClick={() => navigate('buchen')} className="v5-btn" style={{ padding: '7px 14px', borderRadius: 9, background: 'var(--v5-acc)', color: 'var(--v5-accent-fg)', border: 'none', fontSize: 11, fontWeight: 600, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 5, marginLeft: 6 }}>
      ...
      <div className="v5-ani" style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1fr) 280px', gap: 12, animationDelay: '0.12s' }}>
        <Card style={{ overflow: 'hidden', padding: 0 }}>
          ...
          <div role="grid" aria-label={`Kalender ${monthLabel}`}>
            <div role="row" style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', borderBottom: '1px solid var(--v5-bor)' }}>
              {WEEKDAYS.map((label) => (<div key={label} role="columnheader" className="v5-mono" style={{ padding: '10px 0', fontSize: 10, letterSpacing: 1.2, textTransform: 'uppercase', textAlign: 'center', color: 'var(--v5-mut)' }}>
                  {label}
                </div>))}
            </div>
            {Array.from({ length: Math.ceil(days.length / 7) }).map((_, weekIdx) => (<div key={`week-${weekIdx}`} role="row" style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)' }}>
                ...
                return (<div role="gridcell" key={idx} style={{ display: 'flex' }}>
                      <button type="button" data-testid="kalender-day" aria-label={day.toLocaleDateString('de-DE', { weekday: 'long', day: 'numeric', month: 'long' })} aria-pressed={selected} onClick={() => setSelectedDate(day)} style={{
                        minHeight: 82,
                        width: '100%',
                        padding: 6,
                        ...
                        textAlign: 'center',
                          ...
                          {dayEvents.length > 0 && (<span className="v5-mono" style={{ fontSize: 9, color: 'var(--v5-mut)' }}>
                              {dayEvents.length}
                            </span>)}
                        </div>
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                          {dayEvents.slice(0, 2).map((e) => (<div key={e.id} title={`${e.title} (${e.status})`} style={{
                            display: 'flex',
                            alignItems: 'center',
                            gap: 4,
                            fontSize: 10,
                          ...
                          {dayEvents.length > 2 && (<div style={{ fontSize: 9, color: 'var(--v5-mut)' }}>+{dayEvents.length - 2}</div>)}
                        </div>
                      </button>
                    </div>);
            })}
        ...
        <Card style={{ padding: 16, display: 'flex', flexDirection: 'column', gap: 10, alignSelf: 'start' }}>
          ...
          {selectedDate ? (selectedEvents.length === 0 ? (<div style={{ padding: 18, textAlign: 'center', color: 'var(--v5-mut)', fontSize: 12 }}>
                Keine Einträge
              </div>) : (<div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {selectedEvents.map((e) => (<div key={e.id} data-testid="kalender-detail" style={{
                    padding: 10,
                    borderRadius: 10,
                    background: 'var(--v5-sur2)',
                    ...
                    <span aria-hidden="true" style={{ width: 3, alignSelf: 'stretch', background: statusColor(e.status), borderRadius: 3 }}/>
              ...
              </div>)) : (<div style={{ padding: 18, textAlign: 'center', color: 'var(--v5-mut)', fontSize: 12 }}>
