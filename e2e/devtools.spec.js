...
test.describe("DevTools — Network", () => {
  ...
  test("no 404 for static assets", async ({ page }) => {
    ...
    page.on("response", (res) => {
      ...
      if (
        res.status() === 404 &&
        /\.(js|css|woff2?|ttf|png|jpg|svg|ico)$/i.test(url)
      ) {
        missing.push(url);
      }
    });
...
test.describe("DevTools — Axe Accessibility Audit", () => {
  test("login page passes axe-core checks", async ({ page }) => {
    ...
    expect(
      serious,
      `Serious a11y violations: ${JSON.stringify(serious.map((v) => v.id))}`,
    ).toEqual([]);
  });
});
...
test.describe("DevTools — Mobile Responsive", () => {
  for (const device of MOBILE_DEVICES) {
    test(`renders on ${device.name} (${device.width}x${device.height})`, async ({
      page,
    }) => {
...
test.describe("DevTools — Interactions", () => {
  ...
  test("navigation works after login", async ({ page }) => {
    ...
    const navLink = page
      .locator(
        'nav a[href*="book"], aside a[href*="book"], a[href="/bookings"]',
      )
      .first();
