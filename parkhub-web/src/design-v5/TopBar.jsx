...
export function V5TopBar({ title, breadcrumb, onOpenCommand, onToggleAssistant, assistantOpen, }) {
    ...
    return (<header style={{
      ...
      <div role="radiogroup" aria-label={t('settings.appearance')} style={{
            display: 'flex',
        ...
        {V5_MODES.map((m) => {
            const active = mode === m;
            return (<button key={m} type="button" role="radio" aria-checked={active} onClick={() => setMode(m)} style={{
                    padding: '5px 10px',
      ...
      <button type="button" onClick={onOpenCommand} aria-label={t('common.search')} style={{
            display: 'flex',
            alignItems: 'center',
            gap: 7,
            background: 'var(--v5-sur2)',
        ...
        <span style={{ fontSize: 11 }}>{t('common.search')}…</span>
      ...
      <button type="button" onClick={onToggleAssistant} aria-pressed={assistantOpen} aria-label="Toggle assistant" style={{
            height: 30,
            padding: '0 10px',
            borderRadius: 20,
