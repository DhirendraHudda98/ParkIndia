...
test.describe("API — Auth Flow", () => {
  test("POST /api/v1/auth/login with valid creds → 200 + token", async ({
    request,
  }) => {
    ...
    const token =
      body.data?.tokens?.access_token ?? body.data?.token ?? body.token;
  ...
  test("POST /api/v1/auth/login returns application/json", async ({
    request,
  }) => {
    const res = await request.post("/api/v1/auth/login", { data: DEMO_ADMIN });
