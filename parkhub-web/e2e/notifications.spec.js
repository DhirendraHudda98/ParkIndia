...
test.describe("Notification Center", () => {
  ...
  test("clicking bell opens notification panel", async ({ page }) => {
    ...
    await expect(
      page
        .getByRole("heading", { name: "Notification Center" })
        .or(page.locator("h3").filter({ hasText: "Notification Center" })),
  ...
  test("notification panel can be dismissed by clicking outside", async ({
    page,
  }) => {
    const bell = page.getByRole("button", { name: "Notification Center" });
    await bell.click();
  ...
  test("notification badge shows numeric count when unread notifications exist", async ({
    page,
  }) => {
  ...
  test("notification panel mark-all-read button appears when unread count > 0", async ({
    page,
  }) => {
    const bell = page.getByRole("button", { name: "Notification Center" });
    await bell.click();
  ...
  test("switching to Unread filter tab stays in notification panel", async ({
    page,
  }) => {
    const bell = page.getByRole("button", { name: "Notification Center" });
    await bell.click();
