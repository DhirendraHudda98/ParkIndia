 ...
 */
export function V5CommandPalette({ open, onClose, onNavigate, }) {
    useEffect(() => {
        if (!open)
            return;
        const h = (e) => {
    ...
    return (<div role="dialog" aria-modal="true" aria-label="Command palette" onClick={onClose} style={{
            position: 'fixed',
            inset: 0,
      ...
      <div onClick={(e) => e.stopPropagation()} style={{
        ...
        <Command label="Screen navigation" loop>
          <div style={{
            ...
            <Command.Input autoFocus placeholder="Search or navigate screens..." style={{ fontSize: 14, padding: '12px 0' }}/>
          ...
          <Command.List>
            ...
            if (!items.length)
                return null;
            return (<Command.Group key={sec} heading={SECTION_HEADINGS[sec]}>
                  {items.map((item) => (<Command.Item key={item.id} value={`${item.label} ${item.n} ${item.id}`} onSelect={() => onNavigate(item.id)}>
                      <div style={{
                        width: 26,
                        height: 26,
                        borderRadius: 8,
