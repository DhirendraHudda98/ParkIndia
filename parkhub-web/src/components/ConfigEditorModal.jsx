...
export function ConfigEditorModal({ moduleName, isOpen, onClose }) {
  ...
  const onSave = useCallback(
    async (next) => {
      ...
      try {
        ...
        toast.error(
          t(
            "admin.modules.config.errorToast",
            "Could not save {{name}} config",
            {
              name: moduleName,
            },
          ),
        ...
        >
          ...
          {loaded === null && loadErr === null && (
            <p
              ...
              data-testid="config-modal-loading"
            >
              {t("loading", "Loading…")}
            </p>
          )}
          ...
          {loadErr && (
            <div
              ...
              data-testid="config-modal-load-error"
            >
              {loadErr}
            </div>
          )}
          ...
          {loaded &&
            Object.keys(loaded.schema.properties ?? {}).length === 0 && (
              <p
                className="text-sm text-surface-500"
                data-testid="config-modal-no-schema"
              >
