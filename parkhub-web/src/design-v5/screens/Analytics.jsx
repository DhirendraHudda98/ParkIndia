...
/* ───────────────────────────────────────────────────────────────────
   ...
   ─────────────────────────────────────────────────────────────────── */
const UPlotChart = lazy(() => import('../primitives/UPlotChart').then((m) => ({ default: m.UPlotChart })));
...
function ChartSkeleton({ ariaLabel }) {
...
function ChartBlock({ label, xs, ys, tickLabels, stroke = 'var(--v5-acc)', fill = 'var(--v5-acc-muted)', }) {
    ...
    const data = useMemo(() => [xs, ys], [xs, ys]);
    ...
    if (xs.length === 0) {
        ...
        <div role="img" aria-label={label} style={{ fontSize: 11, color: 'var(--v5-mut)', padding: '14px 0' }}>
          Keine Daten
        </div>
      </div>);
    }
...
export function AnalyticsV5({ navigate: _navigate }) {
    ...
    if (isLoading) {
        ...
        {[120, 200, 200].map((h, i) => (<div key={i} style={{ height: h, borderRadius: 14, background: 'var(--v5-sur2)', animation: 'ph-v5-pulse 1.6s ease infinite', animationDelay: `${i * 0.1}s` }}/>))}
      </div>);
    }
    if (isError || !stats) {
    ...
    return (<div style={{ padding: 16, flex: 1, overflow: 'auto', display: 'flex', flexDirection: 'column', gap: 12 }}>
      ...
      <Card className="v5-ani" style={{ padding: 16, animationDelay: '0.12s' }}>
        <ChartBlock label="Auslastung nach Stunde (%)" xs={hourData.xs} ys={hourData.ys} tickLabels={hourData.ticks}/>
      </Card>

      <Card className="v5-ani" style={{ padding: 16, animationDelay: '0.18s' }}>
        <ChartBlock label="Auslastung nach Wochentag (%)" xs={dayData.xs} ys={dayData.ys} tickLabels={dayData.ticks} stroke="var(--v5-ok, oklch(0.65 0.17 160))" fill="var(--v5-ok-muted, oklch(0.65 0.17 160 / 0.25))"/>
      </Card>
    </div>);
}
