...
vi.mock("../api/client", () => ({
  ...
  api: {
    updateDesignThemePreference: (id) => updateDesignThemePreferenceSpy(id),
  },
}));
...
function renderWithTheme(children = null) {
  const states = [];
...
describe("Theme Switching — Comprehensive Audit", () => {
  beforeEach(() => {
    ...
    updateDesignThemePreferenceSpy.mockResolvedValue({
      success: true,
      data: { design_theme: "classic" },
    });
  });
  ...
  describe("User vs Admin flow — server sync", () => {
    ...
    it("Admin role: same API method is used (admins use user preferences)", () => {
      const { getState } = renderWithTheme();
      act(() => {
        getState().setDesignTheme("forest");
      });
    ...
    it("API failure on theme sync does not crash UI", () => {
      updateDesignThemePreferenceSpy.mockRejectedValueOnce(
        new Error("network error"),
      );
      const { getState } = renderWithTheme();
      expect(() => {
        act(() => {
          getState().setDesignTheme("neon");
        });
      }).not.toThrow();
  ...
  describe("Rapid theme switching", () => {
    ...
    it("handles rapid light/dark toggles without losing design theme", () => {
      const { getState } = renderWithTheme();
      act(() => {
        getState().setDesignTheme("sakura");
      });
      for (let i = 0; i < 5; i++) {
        act(() => {
          getState().setTheme(i % 2 === 0 ? "dark" : "light");
        });
      }
