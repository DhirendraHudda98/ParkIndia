...
vi.mock('react-i18next', () => ({ useTranslation: () => ({ t: (k, o) => o?.pos ? `Position ${o.pos}` : o?.minutes ? `${o.minutes} min` : o?.total ? `${o.total} slots` : k }) }));
...
describe('WaitlistPage', () => {
    beforeEach(() => {
        ...
        globalThis.fetch = vi.fn((url, opts) => {
            if (opts?.method === 'POST' && url.includes('/subscribe'))
                return Promise.resolve({ json: () => Promise.resolve({ success: true }) });
            if (opts?.method === 'POST' && url.includes('/accept'))
                return Promise.resolve({ json: () => Promise.resolve({ success: true }) });
            if (opts?.method === 'POST' && url.includes('/decline'))
                return Promise.resolve({ json: () => Promise.resolve({ success: true }) });
            if (opts?.method === 'DELETE')
                return Promise.resolve({ json: () => Promise.resolve({ success: true }) });
            if (url.includes('/waitlist'))
                return Promise.resolve({ json: () => Promise.resolve({ success: true, data: { entries: waitlistEntries } }) });
            return Promise.resolve({ json: () => Promise.resolve({ success: true, data: lots }) });
        });
    });
    ...
    it('join waitlist', async () => {
        ...
        globalThis.fetch = vi.fn((url, opts) => {
            if (opts?.method === 'POST' && url.includes('/subscribe'))
                return Promise.resolve({ json: () => Promise.resolve({ success: true }) });
            if (url.includes('/l3/waitlist'))
                return Promise.resolve({ json: () => Promise.resolve({ success: true, data: { entries: [] } }) });
            if (url.includes('/waitlist'))
                return Promise.resolve({ json: () => Promise.resolve({ success: true, data: { entries: waitlistEntries } }) });
            return Promise.resolve({ json: () => Promise.resolve({ success: true, data: lotsWithExtra }) });
        });
        render(<WaitlistPage />);
    ...
    it('accept offered spot', async () => {
        globalThis.fetch = vi.fn((url, opts) => {
            if (opts?.method === 'POST' && url.includes('/accept'))
                return Promise.resolve({ json: () => Promise.resolve({ success: true }) });
            if (url.includes('/waitlist'))
                return Promise.resolve({ json: () => Promise.resolve({ success: true, data: { entries: offeredEntries } }) });
            return Promise.resolve({ json: () => Promise.resolve({ success: true, data: lots }) });
        });
        render(<WaitlistPage />);
    ...
    it('shows no full lots state', async () => {
        globalThis.fetch = vi.fn((url) => {
            if (url.includes('/waitlist'))
                return Promise.resolve({ json: () => Promise.resolve({ success: true, data: { entries: [] } }) });
            return Promise.resolve({ json: () => Promise.resolve({ success: true, data: [{ id: 'l1', name: 'Open', total_slots: 10, available_slots: 5 }] }) });
        });
        render(<WaitlistPage />);
    ...
    it('join error', async () => {
        ...
        globalThis.fetch = vi.fn((url, opts) => {
            if (opts?.method === 'POST' && url.includes('/subscribe'))
                return Promise.resolve({ json: () => Promise.resolve({ success: false, error: { message: 'Already joined' } }) });
            if (url.includes('/waitlist'))
                return Promise.resolve({ json: () => Promise.resolve({ success: true, data: { entries: [] } }) });
            if (url.includes('/lots'))
                return Promise.resolve({ json: () => Promise.resolve({ success: true, data: lotsWithNew }) });
    ...
    it('accept error', async () => {
        globalThis.fetch = vi.fn((url, opts) => {
            if (opts?.method === 'POST' && url.includes('/accept'))
                return Promise.resolve({ json: () => Promise.resolve({ success: false, error: { message: 'Expired' } }) });
            if (url.includes('/waitlist'))
                return Promise.resolve({ json: () => Promise.resolve({ success: true, data: { entries: offeredEntries } }) });
            return Promise.resolve({ json: () => Promise.resolve({ success: true, data: lots }) });
        });
