...
export function V5Icon({ d, size = 15, color = 'currentColor', strokeWidth = 1.6, }) {
...
export function V5NamedIcon({ name, ...rest }) {
...
export function Badge({ variant = 'primary', dot = false, children, }) {
 ...
 */
export function Card({ children, style, className = '', lift = true, as: Tag = 'div', onClick, 'data-testid': dataTestid, }) {
    return (<Tag onClick={onClick} data-testid={dataTestid} className={`${lift ? 'v5-lift' : ''} ${className}`.trim()} style={{
...
export function SectionLabel({ children }) {
    return (<div className="v5-mono" style={{
            fontSize: 9,
            letterSpacing: 1.4,
            color: 'var(--v5-mut)',
...
export function LiveDot({ color = 'var(--v5-ok)' }) {
...
export function Toggle({ checked, onChange, ariaLabel, }) {
    ...
    return (<button type="button" role="switch" aria-checked={checked} aria-label={ariaLabel} disabled={disabled} onClick={() => onChange?.(!checked)} style={{
            width: 38,
            height: 21,
            borderRadius: 11,
 ...
 */
export function StatCard({ label, value, sub, accent = false, icon, delay = 0, }) {
    return (<div className="v5-lift v5-ani" style={{
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
        ...
        {icon && (<div style={{
            ...
            <V5Icon d={icons[icon]} size={12} color={accent ? 'var(--v5-acc)' : 'var(--v5-mut)'}/>
          </div>)}
      </div>
      <div className="v5-mono" style={{
            fontSize: 26,
