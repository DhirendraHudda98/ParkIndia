...
// Lazy helper (JavaScript version)
function lazy(loader, name, path) {
...
const AdminEnhancedDashboardPage = lazy(() => import('./views/AdminEnhancedDashboard'), 'AdminEnhancedDashboardPage', '/admin/enhanced-dashboard');
const AdminUsersManagementPage = lazy(() => import('./views/AdminUsersManagement'), 'AdminUsersManagementPage', '/admin/users-management');
const AdminParkingManagementPage = lazy(() => import('./views/AdminParkingManagement'), 'AdminParkingManagementPage', '/admin/parking-management');
const AdminBookingsManagementPage = lazy(() => import('./views/AdminBookingsManagement'), 'AdminBookingsManagementPage', '/admin/bookings-management');
const AdminRealTimeMonitoringPage = lazy(() => import('./views/AdminRealTimeMonitoring'), 'AdminRealTimeMonitoringPage', '/admin/monitoring');
...
function useThemeLoader(pathname) {
  useEffect(() => {
  ...
  return (
    <Routes location={location}>
      ...
      <Route path="/" element={<ProtectedRoute><CommandPaletteProvider><SuspenseRoute><Layout /></SuspenseRoute></CommandPaletteProvider></ProtectedRoute>}>
        ...
        <Route path="swap-requests" element={<Navigate to="/" replace />} />
        <Route path="guest-pass" element={<Navigate to="/" replace />} />
        ...
        <Route path="admin" element={<AdminRoute><SuspenseRoute><AdminPage /></SuspenseRoute></AdminRoute>}>
          ...
          <Route path="enhanced-dashboard" element={<SuspenseRoute><AdminEnhancedDashboardPage /></SuspenseRoute>} />
          <Route path="users-management" element={<SuspenseRoute><AdminUsersManagementPage /></SuspenseRoute>} />
          <Route path="parking-management" element={<SuspenseRoute><AdminParkingManagementPage /></SuspenseRoute>} />
          <Route path="bookings-management" element={<SuspenseRoute><AdminBookingsManagementPage /></SuspenseRoute>} />
          <Route path="real-time-monitoring" element={<SuspenseRoute><AdminRealTimeMonitoringPage /></SuspenseRoute>} />
        </Route>
      </Route>
...
function DeferredOverlays() {
  ...
  useEffect(() => {
    const schedule = globalThis.requestIdleCallback ?? ((cb) => setTimeout(cb, 500));
