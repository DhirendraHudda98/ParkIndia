    ...
    for (let day = 0; day < 7; day++) {
        for (let hour = 0; hour < 24; hour++) {
            ...
            if (day < 5 && hour >= 8 && hour <= 17)
                pct = 50 + Math.round(Math.random() * 30);
...
describe('OccupancyHeatmapPage', () => {
    ...
    it('fetches data with lot filter when lot selected', async () => {
        ...
        expect(global.fetch).toHaveBeenCalledWith('/api/v1/admin/analytics/occupancy-heatmap', expect.objectContaining({ credentials: 'include' }));
    });
    it('changes lot via selector triggers refetch with lot_id', async () => {
        ...
        const select = screen.getByTestId('lot-selector');
        await user.selectOptions(select, 'lot-1');
        await waitFor(() => {
            expect(global.fetch).toHaveBeenCalledWith('/api/v1/admin/analytics/occupancy-heatmap?lot_id=lot-1', expect.objectContaining({ credentials: 'include' }));
        });
    });
    ...
    it('renders cells with various color buckets (90+, 75-89, 50-74, 20-49, 0-19)', async () => {
        ...
        const cellByLabel = (label) => allCells.find(c => c.getAttribute('aria-label')?.startsWith(label));
