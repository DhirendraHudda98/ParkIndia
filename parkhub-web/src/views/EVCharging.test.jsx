...
const sampleChargers = [
    { id: 'ch1', lot_id: 'lot-1', label: 'Charger A1', connector_type: 'ccs', power_kw: 50, status: 'available', location_hint: 'Near entrance' },
    { id: 'ch2', lot_id: 'lot-1', label: 'Charger B2', connector_type: 'type2', power_kw: 22, status: 'in_use', location_hint: null },
];
    ...
    beforeEach(() => {
        ...
        global.fetch = vi.fn((url) => {
            ...
            if (typeof url === 'string' && url.includes('/chargers')) {
                return Promise.resolve({ json: () => Promise.resolve({ success: true, data: sampleChargers }) });
            }
            if (typeof url === 'string' && url.includes('/lots')) {
                return Promise.resolve({ json: () => Promise.resolve({ success: true, data: sampleLots }) });
            }
            return Promise.resolve({ json: () => Promise.resolve({ success: true, data: [] }) });
        });
    });
...
describe('AdminChargersPage', () => {
    beforeEach(() => {
        mockUseTheme.mockReset();
        mockUseTheme.mockReturnValue({ designTheme: 'marble' });
        global.fetch = vi.fn(() => Promise.resolve({ json: () => Promise.resolve({ success: true, data: { total_chargers: 8, available: 5, in_use: 2, offline: 1, total_sessions: 120, total_kwh: 1500.5 } }) }));
    });
    afterEach(() => vi.restoreAllMocks());
    ...
    it('sends the bearer token for admin charger stats', async () => {
        ...
        expect(fetchMock).toHaveBeenCalledWith('/api/v1/admin/chargers', expect.objectContaining({
            credentials: 'include',
...
describe('EVChargingPage - extended', () => {
    ...
    it('shows empty chargers state', async () => {
        global.fetch = vi.fn((url) => {
            ...
            if (typeof url === 'string' && url.includes('/lots')) {
                return Promise.resolve({ json: () => Promise.resolve({ success: true, data: [{ id: 'lot-1', name: 'Main Lot' }] }) });
            }
            return Promise.resolve({ json: () => Promise.resolve({ success: true, data: [] }) });
        });
        render(<EVChargingPage />);
    ...
    it('start charging failure', async () => {
        global.fetch = vi.fn((url) => {
            if (typeof url === 'string' && url.includes('/start')) {
                return Promise.resolve({ json: () => Promise.resolve({ success: false, error: { message: 'Busy' } }) });
            }
            if (typeof url === 'string' && url.includes('/chargers/sessions')) {
                return Promise.resolve({ json: () => Promise.resolve({ success: true, data: [] }) });
            }
            ...
            if (typeof url === 'string' && url.includes('/lots')) {
                return Promise.resolve({ json: () => Promise.resolve({ success: true, data: [{ id: 'lot-1', name: 'Lot' }] }) });
            }
            return Promise.resolve({ json: () => Promise.resolve({ success: true, data: [] }) });
        });
        render(<EVChargingPage />);
    ...
    it('stops charging with active session', async () => {
        ...
        global.fetch = vi.fn((url) => {
            ...
            if (typeof url === 'string' && url.includes('/chargers/sessions')) {
                return Promise.resolve({ json: () => Promise.resolve({ success: true, data: sessions }) });
            }
            if (typeof url === 'string' && url.includes('/chargers')) {
                return Promise.resolve({ json: () => Promise.resolve({ success: true, data: [
            ...
            if (typeof url === 'string' && url.includes('/lots')) {
                return Promise.resolve({ json: () => Promise.resolve({ success: true, data: [{ id: 'lot-1', name: 'Main' }] }) });
            }
            return Promise.resolve({ json: () => Promise.resolve({ success: true, data: [] }) });
        });
        render(<EVChargingPage />);
    ...
    it('stop charging failure', async () => {
        ...
        global.fetch = vi.fn((url) => {
            if (typeof url === 'string' && url.includes('/stop')) {
                return Promise.resolve({ json: () => Promise.resolve({ success: false, error: { message: 'Cannot stop' } }) });
            }
            if (typeof url === 'string' && url.includes('/chargers/sessions')) {
                return Promise.resolve({ json: () => Promise.resolve({ success: true, data: sessions }) });
            }
    ...
    it('lot selector changes chargers', async () => {
        global.fetch = vi.fn((url) => {
            ...
            if (typeof url === 'string' && url.includes('/lots')) {
                return Promise.resolve({ json: () => Promise.resolve({ success: true, data: [{ id: 'lot-1', name: 'Lot 1' }, { id: 'lot-2', name: 'Lot 2' }] }) });
            }
            return Promise.resolve({ json: () => Promise.resolve({ success: true, data: [] }) });
        });
        render(<EVChargingPage />);
        ...
        await waitFor(() => {
            ...
            expect(fetchCalls.some((url) => url.includes('lot-2'))).toBe(true);
        });
    });
