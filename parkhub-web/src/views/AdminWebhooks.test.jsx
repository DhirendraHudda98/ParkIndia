...
describe('AdminWebhooksPage', () => {
    beforeEach(() => {
        ...
        globalThis.fetch = vi.fn((url, opts) => {
            if (url.includes('/deliveries'))
                return Promise.resolve({ json: () => Promise.resolve({ success: true, data: deliveries }) });
            if (url.includes('/test'))
                return Promise.resolve({ json: () => Promise.resolve({ success: true, data: { success: true } }) });
            if (opts?.method === 'DELETE')
                return Promise.resolve({ json: () => Promise.resolve({ success: true }) });
            if (opts?.method === 'POST' || opts?.method === 'PUT')
                return Promise.resolve({ json: () => Promise.resolve({ success: true }) });
            return Promise.resolve({ json: () => Promise.resolve({ success: true, data: webhooks }) });
        });
    });
    ...
    it('test webhook failure response', async () => {
        globalThis.fetch = vi.fn((url, opts) => {
            if (url.includes('/test'))
                return Promise.resolve({ json: () => Promise.resolve({ success: true, data: { success: false } }) });
            return Promise.resolve({ json: () => Promise.resolve({ success: true, data: webhooks }) });
        });
        render(<AdminWebhooksPage />);
    ...
    it('handles save error response', async () => {
        globalThis.fetch = vi.fn((url, opts) => {
            if (opts?.method === 'POST' && !url.includes('/test'))
                return Promise.resolve({ json: () => Promise.resolve({ success: false, error: { message: 'Invalid URL' } }) });
            return Promise.resolve({ json: () => Promise.resolve({ success: true, data: webhooks }) });
        });
        render(<AdminWebhooksPage />);
        await waitFor(() => fireEvent.click(screen.getByText('webhooksV2.create')));
