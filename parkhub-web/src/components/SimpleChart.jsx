 ...
 */
export function DonutChart({ slices, size = 200, strokeWidth = 28 }) {
  ...
  const totalOccupied = slices.reduce(
    (s, d) => s + (d.occupancy * Math.max(d.capacity, 1)) / 100,
    0,
  );
  const overallPct =
...
export function BarChart({ data, height, color = "var(--color-primary-500)" }) {
  if (data.length === 0) return null;

  const maxValue = Math.max(...data.map((d) => d.value), 1);
  const chartHeight =
    height ??
    data.length * (BAR_HEIGHT + BAR_GAP) + PADDING_TOP + PADDING_BOTTOM;
