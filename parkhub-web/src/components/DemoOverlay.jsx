...
function formatCountdown(isoString) {
  const diff = Math.max(
    0,
    Math.floor((new Date(isoString).getTime() - Date.now()) / 1000),
  );
...
export function DemoOverlay({ reloadPage = defaultReloadPage } = {}) {
  ...
  const handleVote = useCallback(() => {
    api.voteDemoReset().then((res) => {
      if (res.success) {
        setDemo((prev) =>
          prev ? { ...prev, has_voted: true, votes: prev.votes + 1 } : prev,
        );
      }
    });
  }, []);
  ...
  const nextReset = useMemo(
    () =>
      demo?.next_scheduled_reset
        ? formatCountdown(demo.next_scheduled_reset)
        : null,
    // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  const voteProgress =
    demo.vote_threshold > 0 ? (demo.votes / demo.vote_threshold) * 100 : 0;
    ...
    >
      <div className="glass-card shadow-xl">
        ...
        >
          ...
          {/* Timer */}
          <span
            className={`font-mono text-sm font-bold transition-colors duration-300 ${isLow ? "text-red-700 dark:text-red-300" : "text-surface-800 dark:text-surface-200"}`}
          >
          ...
          {collapsed ? (
            <CaretDown weight="bold" className="w-3 h-3 text-surface-400" />
          ) : (
            <CaretUp weight="bold" className="w-3 h-3 text-surface-400" />
          )}
        </button>
              ...
              <div className="px-4 pb-3 pt-1 border-t border-surface-200/50 dark:border-surface-700/50">
                ...
                <div className="flex items-center gap-2 mb-2">
                  <div
                    ...
                    aria-label={t("demo.votesNeeded", {
                      current: demo.votes,
                      needed: demo.vote_threshold,
                    })}
                  >
                    <motion.div
                ...
                >
                  <ArrowsClockwise
                    weight="bold"
                    className={`w-3.5 h-3.5 ${demo.reset_in_progress ? "animate-spin" : ""}`}
                  />
