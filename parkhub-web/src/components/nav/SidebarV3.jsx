...
function parseSlotFloor(slotNumber) {
 ...
 */
function summariseFloors(slots, mySlotNumber) {
  ...
  const floors = Array.from(buckets.entries())
    .filter(([, b]) => b.total > 0)
    .map(([label, b]) => ({
      label,
...
function formatHoursMinutes(msUntil) {
...
export function SidebarV3() {
  ...
  const checkoutMs = activeBooking
    ? new Date(activeBooking.end_time).getTime()
    : 0;
  const startMs = activeBooking
    ? new Date(activeBooking.start_time).getTime()
    : 0;
  ...
  const progressPct = Math.max(
    5,
    ((totalWindow - msUntil) / totalWindow) * 100,
  );

  const freePct =
    activeLot && activeLot.total_slots > 0
          ...
          <button
            ...
            onClick={() => {
              ...
              window.dispatchEvent(
                new CustomEvent("parkhub:open-notifications"),
              );
            }}
            title="Notifications"
        ...
        {activeBooking && slotDisplay ? (
          <div
            ...
            style={{
              ...
              borderRadius: 14,
              background:
                "linear-gradient(165deg, oklch(0.24 0.03 260) 0%, oklch(0.19 0.02 260) 100%)",
              border: "1px solid oklch(0.26 0.018 260)",
              boxShadow:
                "0 1px 0 oklch(0.28 0.02 260) inset, 0 20px 40px -20px rgba(0,0,0,0.5)",
            }}
          >
            <div
              aria-hidden="true"
            ...
            <div
              className="sv3-display relative"
...
function SectionHeading({ label, right, tag }) {
  return (
    <div
      className="flex items-center justify-between"
        ...
        {tag && (
          <span
            ...
            style={{
              ...
              borderRadius: 2,
              background:
                "color-mix(in oklch, var(--color-warning, #f59e0b) 18%, transparent)",
...
function NumberedNavItem({ item, active }) {
...
function FlatNavItem({ item, active }) {
  const [hover, setHover] = useState(false);
  return (
    <Link
      to={item.to}
...
function LotFloorMap({ freeSlots, totalSlots, freePct, floors, onOpenMap }) {
            ...
            return (
              <button
                ...
                style={{
                  ...
                  borderRadius: 4,
                  background:
                    hoverIndex === i ? "oklch(0.24 0.02 260)" : "transparent",
                  textAlign: "left",
                }}
              >
                <span
