...
async function getFirstAvailableSlot(request, token) {
...
test.describe("Concurrent Users — Booking Conflict Detection", () => {
  ...
  test("concurrent booking attempts return proper error messages", async ({
    playwright,
  }) => {
    const ctxA = await playwright.request.newContext({ baseURL: BASE });
    const ctxB = await playwright.request.newContext({ baseURL: BASE });
  ...
  test("two browser contexts see consistent lot availability", async ({
    browser,
  }) => {
    ...
    const waitForSpaReady = async (p) => {
      ...
      await p
        ...
        .catch(() => {
          /* assertion will produce the failure message */
        });
    ...
    const hasBookingContentA = /book|lot|park|slot|reserve|login/i.test(
      bodyA ?? "",
    );
    const hasBookingContentB = /book|lot|park|slot|reserve|login/i.test(
      bodyB ?? "",
    );
