...
import { loginAsAdmin, openV5, V5_LABELS } from "./v5-helpers";
...
const ANCHORS = {
  dashboard: {
    ...
    assert: async (p) =>
      expect(p.getByText("Aktive Buchungen").first()).toBeVisible(),
  },
  buchungen: {
    note: "Alle filter chip",
    assert: async (p) =>
      expect(p.getByText("Alle", { exact: true }).first()).toBeVisible(),
  },
  buchen: {
    note: "Schritt 1/3 wizard indicator",
    // Wizard is in step 1 on first load; duration chips only show on step 2.
    assert: async (p) =>
      expect(p.getByText("Schritt 1/3").first()).toBeVisible(),
  },
  fahrzeuge: {
    ...
    assert: async (p) =>
      expect(p.getByText("Meine Fahrzeuge").first()).toBeVisible(),
  },
  kalender: {
  ...
  karte: {
    ...
    assert: async (p) =>
      expect(p.getByText(/^(Frei|Keine Standorte)$/).first()).toBeVisible(),
  },
  credits: {
    note: "Monatl. Kontingent stat",
    assert: async (p) =>
      expect(p.getByText("Monatl. Kontingent").first()).toBeVisible(),
  },
  team: {
    note: "Heute anwesend stat",
    assert: async (p) =>
      expect(p.getByText("Heute anwesend").first()).toBeVisible(),
  },
  rangliste: {
    ...
    assert: async (p) =>
      expect(
        p.locator("main").getByText("Rangliste", { exact: true }).first(),
      ).toBeVisible(),
  },
  ev: {
    note: "Ladepunkte column header",
    assert: async (p) =>
      expect(p.getByText("Ladepunkte", { exact: true }).first()).toBeVisible(),
  },
  tausch: {
  ...
  vorhersagen: {
    ...
    assert: async (p) =>
      expect(
        p.locator("main").getByText("Vorhersagen", { exact: true }).first(),
      ).toBeVisible(),
  },
  gaestepass: {
  ...
  analytics: {
    ...
    assert: async (p) =>
      expect(
        p.locator("main").getByText("Analytics", { exact: true }).first(),
      ).toBeVisible(),
  },
  nutzer: {
    note: "Nutzer banner",
    assert: async (p) =>
      expect(
        p.locator("main").getByText("Nutzer", { exact: true }).first(),
      ).toBeVisible(),
  },
  billing: {
    note: "Abrechnung banner",
    assert: async (p) =>
      expect(p.getByText("Abrechnung").first()).toBeVisible(),
  },
  lobby: {
  ...
  benachrichtigungen: {
    ...
    assert: async (p) =>
      expect(p.getByText("Ankündigungen").first()).toBeVisible(),
  },
  einstellungen: {
    note: "Sprache section-label",
    assert: async (p) =>
      expect(p.getByText("Sprache", { exact: true }).first()).toBeVisible(),
  },
  standorte: {
    note: "Neuer Standort section",
    assert: async (p) =>
      expect(p.getByText("Neuer Standort").first()).toBeVisible(),
  },
  integrations: {
    ...
    assert: async (p) =>
      expect(
        p
          .locator("main")
          .getByText(
            /^(Integrationen|Fehler beim Laden|Keine Integrationen verfügbar)$/,
          )
          .first(),
      ).toBeVisible(),
  },
  ...
  audit: {
    ...
    assert: async (p) =>
      expect(
        p.locator("main").getByText("Audit-Log", { exact: true }).first(),
      ).toBeVisible(),
  },
  policies: {
  ...
  profil: {
    ...
    assert: async (p) =>
      expect(p.getByText("Kontoinformation").first()).toBeVisible(),
  },
};
...
test.describe("v5 happy paths", () => {
  ...
  for (const [screen, anchor] of Object.entries(ANCHORS)) {
