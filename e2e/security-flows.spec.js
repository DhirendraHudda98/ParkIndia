...
import { loginViaApi, DEMO_ADMIN, ADMIN_ROUTES } from "./helpers";
...
test.describe("Security — Access Control & Hardening", () => {
    ...
    for (const route of ADMIN_ROUTES) {
      test(`${route} redirects or blocks non-admin`, async ({ page }) => {
        ...
        try {
          await page.waitForFunction(
            () => {
              ...
              return /forbidden|access denied|unauthorized|not authorized/i.test(
                txt,
              );
            },
            null,
        ...
        const isBlocked =
          ...
          /forbidden|access denied|unauthorized|not authorized/i.test(
            body ?? "",
          );
  ...
  test.describe("Session & Token Security", () => {
    ...
    test("login with empty credentials returns 400 or 401", async ({
      request,
    }) => {
      const res = await request.post("/api/v1/auth/login", {
        data: { email: "", password: "" },
  ...
  test.describe("Rate Limiting", () => {
    test("rapid login attempts are eventually rate-limited", async ({
      request,
    }) => {
      ...
      if (limitHeader && parseInt(limitHeader, 10) > 1000) {
        test.skip(
          true,
          `Rate limits disabled (limit=${limitHeader}); skipping rate-limit test`,
        );
        return;
      }
    ...
    test("SQL injection in query params is handled safely", async ({
      ...
      for (const payload of sqliPayloads) {
        const res = await request.get(
          `/api/v1/bookings?search=${encodeURIComponent(payload)}`,
          {
            headers: { Authorization: `Bearer ${token}` },
          },
        );
