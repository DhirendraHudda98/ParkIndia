...
class LotController extends Controller
{
    public function index(Request $request): AnonymousResourceCollection
    {
        ...
        $lots = ParkingLot::all()->map(function ($lot) use ($slotCounts, $occupiedCounts, $userLat, $userLng) {
            $total = (int) $slotCounts->get($lot->id, 0);
            $occupied = (int) $occupiedCounts->get($lot->id, 0);
            $lot->available_slots = max(0, $total - $occupied);

            PricingEngine::applyDynamicPricing($lot, $total, $occupied);

            // Calculate distance if coordinates provided
            if ($userLat && $userLng && $lot->latitude && $lot->longitude) {
                $lot->distance = $this->calculateHaversineDistance(
                    (float) $userLat, 
                    (float) $userLng, 
                    (float) $lot->latitude, 
                    (float) $lot->longitude
    ...
    public function show(string $id)
    {
        ...
        PricingEngine::calculateForLot($lot);
