...
vi.mock('framer-motion', () => ({
    motion: {
        div: React.forwardRef(({ children, initial, animate, exit, transition, whileHover, whileTap, ...props }, ref) => (<div ref={ref} {...props}>{children}</div>)),
    },
    AnimatePresence: ({ children }) => <>{children}</>,
}));
vi.mock('@phosphor-icons/react', () => ({
    ShareNetwork: (props) => <span data-testid="icon-share" {...props}/>,
    Link: (props) => <span data-testid="icon-link" {...props}/>,
...
describe('BookingSharingModal', () => {
    beforeEach(() => {
        global.fetch = vi.fn(() => Promise.resolve({ json: () => Promise.resolve({ success: true, data: sampleShareLink }) }));
        // Mock clipboard
    ...
    it('creates share link on button click', async () => {
        ...
        await waitFor(() => {
            expect(global.fetch).toHaveBeenCalledWith('/api/v1/bookings/booking-1/share', expect.objectContaining({ method: 'POST' }));
        });
    });
    ...
    it('copies share link to clipboard', async () => {
        ...
        await waitFor(() => {
            expect(navigator.clipboard.writeText).toHaveBeenCalledWith(expect.stringContaining('/shared/abc123def456'));
    ...
    it('revokes share link', async () => {
        ...
        await waitFor(() => {
            expect(global.fetch).toHaveBeenCalledWith('/api/v1/bookings/booking-1/share', expect.objectContaining({ method: 'DELETE' }));
            expect(toast.success).toHaveBeenCalledWith('Link revoked');
        });
    });
    it('handles create link error', async () => {
        const toast = (await import('react-hot-toast')).default;
        global.fetch = vi.fn(() => Promise.resolve({ json: () => Promise.resolve({ success: false, error: 'Limit reached' }) }));
        render(<BookingSharingModal bookingId="booking-1"/>);
        fireEvent.click(screen.getByTestId('create-link-btn'));
        await waitFor(() => {
    ...
    it('handles revoke error', async () => {
        ...
        global.fetch = vi.fn(() => {
            ...
            if (callCount === 1) {
                ...
                return Promise.resolve({ json: () => Promise.resolve({ success: true, data: sampleShareLink }) });
            }
            // Revoke fails
    ...
    it('sends invite successfully', async () => {
        ...
        await waitFor(() => {
            expect(global.fetch).toHaveBeenCalledWith('/api/v1/bookings/booking-1/invite', expect.objectContaining({
                method: 'POST',
    ...
    it('handles invite API error', async () => {
        const toast = (await import('react-hot-toast')).default;
        global.fetch = vi.fn(() => Promise.resolve({ json: () => Promise.resolve({ success: false, error: 'User not found' }) }));
        render(<BookingSharingModal bookingId="booking-1"/>);
        fireEvent.click(screen.getByTestId('tab-invite'));
        await waitFor(() => screen.getByTestId('invite-email-input'));
        fireEvent.change(screen.getByTestId('invite-email-input'), { target: { value: 'guest@example.com' } });
