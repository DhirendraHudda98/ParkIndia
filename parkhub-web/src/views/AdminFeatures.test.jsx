...
const mockFeatures = ['credits', 'vehicles'];
...
vi.mock('../context/FeaturesContext', () => {
    ...
    return {
        useFeatures: () => ({
            ...
            isEnabled: (f) => mockFeatures.includes(f),
        }),
        FEATURE_REGISTRY,
        USE_CASE_PRESETS,
    };
...
vi.mock('framer-motion', () => ({
    motion: {
        div: React.forwardRef(({ children, initial, animate, exit, transition, variants, whileHover, whileTap, ...props }, ref) => (<div ref={ref} {...props}>{children}</div>)),
    },
...
vi.mock('@phosphor-icons/react', () => ({
    ...
    ArrowLeft: (props) => <span data-testid="icon-back" {...props}/>,
    ArrowClockwise: (props) => <span data-testid="icon-reset" {...props}/>,
...
describe('AdminFeaturesPage', () => {
    ...
    it('renders feature toggle buttons', () => {
        ...
        const toggleButtons = screen.getAllByRole('button').filter(btn => btn.getAttribute('aria-label')?.startsWith('features.modules.'));
    ...
    it('clicking toggle changes local state', async () => {
        ...
        const toggles = screen.getAllByRole('button').filter(btn => btn.getAttribute('aria-label')?.startsWith('features.modules.'));
        if (toggles.length > 0) {
