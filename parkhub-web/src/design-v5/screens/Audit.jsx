...
export function AuditV5({ navigate: _navigate }) {
    ...
    const { data, isLoading, isError } = useQuery({
        ...
        queryFn: async () => {
            ...
            if (!res.success)
                throw new Error(res.error?.message ?? 'Audit-Log konnte nicht geladen werden');
    ...
    if (isLoading && !data) {
        ...
        {[0, 1, 2].map((i) => (<div key={i} style={{ height: 50, borderRadius: 10, background: 'var(--v5-sur2)', marginBottom: 6, animation: 'ph-v5-pulse 1.6s ease infinite', animationDelay: `${i * 0.05}s` }}/>))}
      </div>);
    }
    if (isError || !data) {
    ...
    return (<div style={{ padding: 16, flex: 1, overflow: 'auto', display: 'flex', flexDirection: 'column', gap: 12 }}>
      ...
      <Card className="v5-ani" style={{ padding: 12, display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'flex-end', animationDelay: '0.06s' }}>
        <label style={{ display: 'flex', flexDirection: 'column', gap: 4, flex: '1 1 160px' }}>
          ...
          <input data-testid="audit-action" placeholder="z. B. login" value={actionFilter} onChange={(e) => setActionFilter(e.target.value)} style={{ padding: '6px 10px', borderRadius: 8, background: 'var(--v5-sur2)', border: '1px solid var(--v5-bor)', color: 'var(--v5-txt)', fontSize: 11, outline: 'none', fontFamily: 'inherit' }}/>
        </label>
        <label style={{ display: 'flex', flexDirection: 'column', gap: 4, flex: '1 1 160px' }}>
          <span style={{ fontSize: 10, color: 'var(--v5-mut)' }}>Nutzer</span>
          <input data-testid="audit-user" placeholder="username" value={userFilter} onChange={(e) => setUserFilter(e.target.value)} style={{ padding: '6px 10px', borderRadius: 8, background: 'var(--v5-sur2)', border: '1px solid var(--v5-bor)', color: 'var(--v5-txt)', fontSize: 11, outline: 'none', fontFamily: 'inherit' }}/>
        </label>
        <button type="button" onClick={applyFilters} data-testid="audit-apply" style={{ padding: '7px 14px', borderRadius: 8, background: 'var(--v5-acc)', color: 'var(--v5-accent-fg)', border: 'none', fontSize: 11, fontWeight: 600, cursor: 'pointer' }}>Filtern</button>
        <button type="button" onClick={() => { setActionFilter(''); setUserFilter(''); setAppliedAction(''); setAppliedUser(''); setPage(1); }} data-testid="audit-reset" style={{ padding: '7px 14px', borderRadius: 8, background: 'var(--v5-sur2)', border: '1px solid var(--v5-bor)', color: 'var(--v5-txt)', fontSize: 11, fontWeight: 500, cursor: 'pointer' }}>Zurücksetzen</button>
      </Card>

      {entries.length === 0 ? (<Card className="v5-ani" style={{ padding: 36, textAlign: 'center', animationDelay: '0.12s' }}>
        ...
        </Card>) : (<Card className="v5-ani" style={{ overflow: 'hidden', animationDelay: '0.12s' }}>
          ...
          {entries.map((e, i) => (<div key={e.id} data-testid="audit-row" style={{
                    display: 'grid', gridTemplateColumns: '140px 110px 130px 1fr',
      ...
      <div className="v5-ani" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', animationDelay: '0.18s' }}>
        <button type="button" disabled={page <= 1} onClick={() => setPage((p) => Math.max(1, p - 1))} data-testid="audit-prev" style={{ padding: '7px 14px', borderRadius: 8, background: 'var(--v5-sur2)', border: '1px solid var(--v5-bor)', color: 'var(--v5-txt)', fontSize: 11, fontWeight: 500, cursor: page > 1 ? 'pointer' : 'not-allowed', opacity: page > 1 ? 1 : 0.5 }}>← Zurück</button>
        <span style={{ fontSize: 11, color: 'var(--v5-mut)' }}>
          {data.entries.length > 0 ? `${(data.page - 1) * data.per_page + 1}–${(data.page - 1) * data.per_page + data.entries.length}` : '0'} von {data.total}
        </span>
        <button type="button" disabled={page >= data.total_pages} onClick={() => setPage((p) => Math.min(data.total_pages, p + 1))} data-testid="audit-next" style={{ padding: '7px 14px', borderRadius: 8, background: 'var(--v5-sur2)', border: '1px solid var(--v5-bor)', color: 'var(--v5-txt)', fontSize: 11, fontWeight: 500, cursor: page < data.total_pages ? 'pointer' : 'not-allowed', opacity: page < data.total_pages ? 1 : 0.5 }}>Weiter →</button>
      </div>
    </div>);
}
