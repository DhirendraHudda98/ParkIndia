...
export function DashboardPage() {
  ...
  const greetingGradient = designTheme === 'india'
    ? 'from-[#FF9933]/20 via-white/5 to-[#138808]/10'
    : hour < 12
  ...
  return (
    ...
    <motion.div key="dashboard-loaded" variants={container} initial="hidden" animate="show" className="space-y-6">
      ...
      {surfaceVariant === 'india' && (
        <motion.div variants={item}>
          <IndiaDashboardSurface
            clock={liveClock}
            creditsLeft={creditsLeft}
            activeCount={activeBookings.length}
            bookingsThisMonth={bookingsThisMonth}
      ...
      >
        <motion.div variants={item} whileHover={{ y: -5 }} transition={{ type: "spring", stiffness: 400, damping: 17 }}>
          <KpiCard
      ...
      <motion.div variants={item} className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <div className="lg:col-span-2">
          <TrendCard
            ...
            onPeriodChange={(k) => setTrendPeriod(k)}
          />
        </div>
        <SensorFeedCard
...
function MarbleStat({ label, value }) {
  return (
    ...
    <section data-testid="void-surface" className="overflow-hidden rounded-[28px] border border-surface-800 bg-black text-white shadow-2xl shadow-black/30">
      ...
      <div className="grid gap-6 p-5 lg:grid-cols-[0.85fr_1.15fr]">
        <div className="space-y-5 border-b border-white/10 pb-5 lg:border-b-0 lg:border-r lg:pb-0 lg:pr-5">
          <div>
            <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-white/45">{t('dashboard.voidLabel', 'Void Dashboard')}</p>
            <h2 className="mt-2 text-3xl font-semibold tracking-[-0.04em]">{t('dashboard.operationalHub', 'Operational Hub')}</h2>
          ...
          <div className="rounded-[24px] border border-white/10 bg-white/[0.03] p-4">
            <div className="flex items-center justify-between">
              <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-white/45">{t('dashboard.currentPass', 'Current pass')}</p>
              <span className="text-xs text-white/50">{clock}</span>
            </div>
            <p className="mt-3 text-xl font-semibold tracking-[-0.03em]">{activeBooking?.lot_name || t('dashboard.noActiveBookings')}</p>
            <p className="mt-1 text-sm text-white/60">{activeBooking ? t('dashboard.slotDetails', 'Slot {{slot}} · {{plate}}', { slot: activeBooking.slot_number, plate: activeBooking.vehicle_plate || t('bookings.noVehicle') }) : t('dashboard.emptyActiveSubtitle')}</p>
          </div>
        </div>
        ...
        <div className="grid gap-4 md:grid-cols-2">
          ...
          <article className="rounded-[24px] border border-white/10 bg-white/[0.03] p-4">
            <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-white/45">Network</p>
            <h3 className="mt-1 text-lg font-semibold">Feed</h3>
            <div className="mt-4 space-y-2">
              {feed.map((entry) => (
                <div key={entry.id} className="flex items-center justify-between gap-3">
                  ...
                  <span className={`h-1.5 w-1.5 flex-shrink-0 rounded-full ${entry.tone === 'warning' ? 'bg-amber-500' : entry.tone === 'live' ? 'bg-emerald-500' : 'bg-white/20'}`} />
                </div>
              ))}
            </div>
          </article>
...
function VoidMetric({ label, value }) {
  return (
    <div className="rounded-2xl border border-white/10 bg-white/[0.02] p-3">
      <p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-white/35">{label}</p>
      <p className="mt-1.5 text-xl font-semibold tracking-[-0.03em]">{value}</p>
    </div>
  );
}

function IndiaDashboardSurface({
  clock,
  creditsLeft,
  activeCount,
  bookingsThisMonth,
  ...
  const { t } = useTranslation();
  return (
    <section data-testid="india-surface" className="overflow-hidden rounded-[28px] border border-[#FF9933]/30 bg-[#fff9f2] shadow-xl shadow-[#FF9933]/5 dark:border-[#FF9933]/20 dark:bg-[#0a0a1a]">
      <div className="flex items-center justify-between gap-3 border-b border-[#FF9933]/20 bg-gradient-to-r from-[#FF9933] via-[#ffffff] to-[#138808] px-5 py-2 text-surface-900">
        <div className="flex items-center gap-2 text-sm font-bold">
          <span className="inline-block h-2 w-2 rounded-full bg-surface-900" />
          {announcement}
        </div>
        <span className="text-xs font-bold uppercase tracking-[0.18em]">{clock}</span>
      </div>

      <div className="grid gap-4 p-5 lg:grid-cols-[1.5fr_1fr]">
        <div className="grid gap-4 md:grid-cols-2">
          <article className="rounded-[24px] border border-[#FF9933]/20 bg-white p-5 dark:border-[#FF9933]/10 dark:bg-[#000033]">
            <div className="mb-4 flex items-start justify-between">
              <div>
                <p className="text-[11px] font-bold uppercase tracking-[0.18em] text-[#FF9933]">India Heritage</p>
                <h2 className="mt-1 text-2xl font-bold tracking-tight text-surface-900 dark:text-white">Daily Summary</h2>
              </div>
              <span className="rounded-full bg-[#FF9933]/10 px-3 py-1 text-xs font-bold text-[#FF9933]">{creditsLeft} credits</span>
            </div>
            <div className="grid grid-cols-3 gap-2">
              <IndiaStat label={t('common.active', 'Active')} value={String(activeCount)} />
              <IndiaStat label={t('dashboard.thisMonth', 'Month')} value={String(bookingsThisMonth)} />
              <IndiaStat label="CO₂" value={`${co2Saved.toFixed(1)}kg`} />
            </div>
            <div className="mt-4 rounded-2xl bg-[#000080] px-4 py-3 text-white">
              ...
              <div className="mt-3 flex items-end justify-between gap-4">
                <div>
                  <p className="text-lg font-bold">{activeBooking?.lot_name || 'No Activity'}</p>
                  <p className="text-sm text-white/80">
                    {activeBooking ? `Slot ${activeBooking.slot_number}` : 'Standby Mode'}
                  </p>
                </div>
                <span className="rounded-full bg-white/20 px-3 py-1 text-xs font-bold">
                  {activeBooking?.vehicle_plate || 'None'}
                </span>
              </div>
            </div>
          </article>
          ...
          <article className="rounded-[24px] border border-[#FF9933]/20 bg-white p-5 dark:border-[#FF9933]/10 dark:bg-[#000033]">
            <div className="mb-4 flex items-center justify-between">
              <div>
                <p className="text-[11px] font-bold uppercase tracking-[0.18em] text-[#138808]">{t('dashboard.electricMobility', 'Electric Mobility')}</p>
                <h3 className="mt-1 text-lg font-bold text-surface-900 dark:text-white">{t('dashboard.evCharging', 'EV Fast Charging')}</h3>
              </div>
              <span className="rounded-full bg-[#138808]/10 px-3 py-1 text-xs font-bold text-[#138808]">{evAvailable} {t('book.available')}</span>
            </div>
            <div className="space-y-2">
              {[0, 1, 2].map((index) => (
                <div key={index} className="flex items-center justify-between rounded-2xl bg-[#f3f4f6] px-3 py-2.5 dark:bg-[#1a1a2e]">
                  <div>
                    <p className="text-sm font-bold text-surface-900 dark:text-white">{t('dashboard.bay', 'Bay')} {index + 1}</p>
                    <p className="text-xs text-surface-500 dark:text-surface-400">{t('dashboard.ccsPort', 'Rapid CCS Port')}</p>
                  </div>
                  <span className={`rounded-full px-2.5 py-1 text-[11px] font-bold ${index < evAvailable ? 'bg-[#138808]/10 text-[#138808]' : 'bg-surface-200 text-surface-500'}`}>
                    {index < evAvailable ? t('book.available') : t('book.occupied', 'Occupied')}
                  </span>
                </div>
              ))}
            </div>
        ...
        <article className="rounded-[24px] border border-[#FF9933]/20 bg-white p-5 dark:border-[#FF9933]/10 dark:bg-[#000033]">
          <div className="mb-4 flex items-center justify-between">
            <div>
              <p className="text-[11px] font-bold uppercase tracking-[0.18em] text-[#FF9933]">Live Updates</p>
              <h3 className="mt-1 text-lg font-bold text-surface-900 dark:text-white">Smart Network</h3>
            </div>
            <span className="rounded-full bg-[#138808]/10 px-3 py-1 text-xs font-bold text-[#138808]">Active</span>
          </div>
          <div className="space-y-3">
            {feed.map((entry) => (
              <div key={entry.id} className="rounded-2xl border border-[#FF9933]/10 px-4 py-3 dark:bg-[#1a1a2e]">
                <div className="flex items-center justify-between gap-3">
                  <p className="text-sm font-bold text-surface-900 dark:text-white">{entry.title}</p>
                  <span className={`inline-block h-2.5 w-2.5 rounded-full ${entry.tone === 'warning' ? 'bg-[#FF9933]' : entry.tone === 'live' ? 'bg-[#138808]' : 'bg-surface-300'}`} />
                </div>
                <p className="mt-1 text-xs text-surface-500 dark:text-surface-400 font-medium">{entry.meta}</p>
              </div>
            ))}
          </div>
        </article>
...
function IndiaStat({ label, value }) {
  return (
    <div className="rounded-2xl bg-[#f3f4f6] px-3 py-3 dark:bg-[#1a1a2e]">
      <p className="text-[11px] font-bold uppercase tracking-[0.18em] text-[#FF9933]">{label}</p>
      <p className="mt-2 text-xl font-bold tracking-tight text-surface-900 dark:text-white">{value}</p>
    </div>
  );
}
