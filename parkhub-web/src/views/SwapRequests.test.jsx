...
vi.mock('../api/client', () => ({
    api: { getBookings: (...a) => mockGetBookings(...a) },
...
vi.mock('framer-motion', () => ({
    motion: { div: React.forwardRef(({ children, variants, ...p }, r) => <div ref={r} {...p}>{children}</div>) },
...
vi.mock('date-fns', () => ({ format: (_d, fmt) => fmt === 'dd.MM HH:mm' ? '10.04 08:00' : fmt === 'HH:mm' ? '08:00' : '10. Apr 2026' }));
...
describe('SwapRequestsPage', () => {
    beforeEach(() => {
        ...
        globalThis.fetch = vi.fn((url, opts) => {
            ...
            if (opts?.method === 'POST')
                return Promise.resolve({ json: () => Promise.resolve({ success: true }) });
            return Promise.resolve({ json: () => Promise.resolve({ success: true, data: requests }) });
        });
    });
    ...
    it('create swap failure', async () => {
        globalThis.fetch = vi.fn((url, opts) => {
            if (opts?.method === 'POST' && url.includes('/swap-request'))
                return Promise.resolve({ json: () => Promise.resolve({ success: false, error: { message: 'Not found' } }) });
            return Promise.resolve({ json: () => Promise.resolve({ success: true, data: requests }) });
        });
        const user = userEvent.setup();
        render(<SwapRequestsPage />);
    ...
    it('shows empty state', async () => {
        globalThis.fetch = vi.fn(() => Promise.resolve({ json: () => Promise.resolve({ success: true, data: [] }) }));
    ...
    it('accept error', async () => {
        globalThis.fetch = vi.fn((url, opts) => {
            if (url.includes('/accept'))
                return Promise.resolve({ json: () => Promise.resolve({ success: false, error: { message: 'Already handled' } }) });
            return Promise.resolve({ json: () => Promise.resolve({ success: true, data: requests }) });
        });
        render(<SwapRequestsPage />);
    ...
    it('decline server error returns message', async () => {
        globalThis.fetch = vi.fn((url) => {
            if (url.includes('/decline'))
                return Promise.resolve({ json: () => Promise.resolve({ success: false, error: { message: 'No' } }) });
            return Promise.resolve({ json: () => Promise.resolve({ success: true, data: requests }) });
        });
        render(<SwapRequestsPage />);
        await waitFor(() => fireEvent.click(screen.getByText('swap.decline')));
