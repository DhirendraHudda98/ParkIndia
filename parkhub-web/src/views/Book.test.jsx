...
vi.mock('../api/client', () => ({
    api: {
        ...
        getDynamicPrice: (...args) => mockGetDynamicPrice(...args),
        getOperatingHours: vi.fn().mockResolvedValue({ hours: [] }),
        getBookingRecommendations: (...args) => mockGetBookingRecommendations(...args),
    },
}));
...
vi.mock('framer-motion', () => ({
    motion: {
        ...
        button: React.forwardRef(({ children, initial, animate, exit, transition, whileHover, whileTap, variants, custom, layoutId, ...props }, ref) => (<button ref={ref} {...props}>{children}</button>)),
    },
...
vi.mock('@phosphor-icons/react', () => ({
    ...
    Motorcycle: (props) => <span data-testid="icon-motorcycle" {...props}/>,
...
vi.mock('../components/Skeleton', () => ({
    SkeletonCard: ({ height }) => <div data-testid="skeleton-card" className={height}>Loading...</div>,
}));
...
describe('BookPage', () => {
    ...
    it('shows hourly rate when API returns rate as a string (decimal:2 cast)', async () => {
        mockGetLots.mockResolvedValue({
            success: true,
            data: [makeLot({ hourly_rate: '3.50', currency: '€' })],
        });
        mockGetVehicles.mockResolvedValue({ success: true, data: [] });
        render(<BookPage />);
        await waitFor(() => {
    ...
    it('isLotOpenNow: lot is 24h shows openNow', async () => {
        const lot = makeLot({ operating_hours: { is_24h: true } });
        mockGetLots.mockResolvedValue({ success: true, data: [lot] });
        mockGetVehicles.mockResolvedValue({ success: true, data: [] });
        render(<BookPage />);
        await waitFor(() => expect(screen.getByText('Garage Alpha')).toBeInTheDocument());
    ...
    it('vehicle option without make shows just the plate', async () => {
        ...
        const lot = makeLot();
        const slot = makeSlot();
        const vehicle = makeVehicle({ make: undefined, model: undefined });
        mockGetLots.mockResolvedValue({ success: true, data: [lot] });
        mockGetVehicles.mockResolvedValue({ success: true, data: [vehicle] });
        mockGetLotSlots.mockResolvedValue({ success: true, data: [slot] });
        render(<BookPage />);
    ...
    it('vehicle option with make and no model handles undefined gracefully', async () => {
        ...
        const lot = makeLot();
        const slot = makeSlot();
        const vehicle = makeVehicle({ model: undefined });
        mockGetLots.mockResolvedValue({ success: true, data: [lot] });
        mockGetVehicles.mockResolvedValue({ success: true, data: [vehicle] });
        mockGetLotSlots.mockResolvedValue({ success: true, data: [slot] });
        render(<BookPage />);
