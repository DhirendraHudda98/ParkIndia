...
import { NAV, SECTION_HEADINGS } from '../nav';
...
function AccountFooter({ userLabel = 'Administrator', userEmail = 'admin@parkhub.test', inverted = false, }) {
    return (<div style={{
      ...
      <button type="button" aria-label="Logout" style={{
            opacity: 0.5,
            cursor: 'pointer',
...
function MarbleLayout({ active, onNavigate, userLabel, userEmail }) {
    const sections = ['main', 'fleet', 'admin'];
    return (<nav aria-label="Main Navigation" data-variant="marble" style={{
            width: 210,
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '6px 0' }}>
        {sections.map((sec) => (<div key={sec} style={{ marginBottom: 4 }}>
            <div className="v5-mono" style={{
                fontSize: 9,
                letterSpacing: 1.4,
                color: 'var(--v5-bor)',
            ...
            {NAV.filter((n) => n.section === sec).map((item) => {
                ...
                return (<button key={item.id} type="button" onClick={() => onNavigate(item.id)} aria-current={isActive ? 'page' : undefined} style={{
                        display: 'flex',
                        alignItems: 'center',
                        gap: 8,
                        width: 'calc(100% - 10px)',
                  ...
                  <V5NamedIcon name={item.icon} size={13} color={isActive ? 'var(--v5-acc)' : 'var(--v5-mut)'}/>
                  <span style={{
                        fontSize: 12,
...
function VoidLayout({ active, onNavigate, userLabel, userEmail }) {
    const [hover, setHover] = useState(null);
    const ticker = 'PARKHUB ACTIVE · ADMIN · 40 CREDITS · 1/12 OCCUPIED · EV: 3 FREE · LIVE · ';
    return (<nav aria-label="Main Navigation" data-variant="marble" data-mode="void" style={{
            width: 230,
            borderRight: '1px solid var(--v5-bor)',
            display: 'flex',
            flexDirection: 'column',
      ...
      <div aria-hidden="true" style={{
        ...
        <div style={{
          ...
          {[0, 1, 2].map((i) => (<span key={i} className="v5-mono" style={{
                fontSize: 8,
                letterSpacing: 1.8,
                color: '#2A2A28',
                paddingRight: 40,
      ...
      <div style={{
        ...
        <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 4 }}>
          <LiveDot color="var(--v5-acc)"/>
          <span className="v5-mono" style={{ fontSize: 8, color: 'var(--v5-acc)' }}>
            LIVE
          </span>
        </div>
      </div>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '6px 0' }}>
        {NAV.map((item) => {
            ...
            return (<button key={item.id} type="button" onClick={() => onNavigate(item.id)} onMouseEnter={() => setHover(item.id)} onMouseLeave={() => setHover(null)} aria-current={isActive ? 'page' : undefined} style={{
                    display: 'flex',
                    alignItems: 'center',
                    width: '100%',
                    height: 36,
              ...
              <div style={{ width: 32, display: 'flex', justifyContent: 'center', flexShrink: 0 }}>
                <V5NamedIcon name={item.icon} size={13} color={isActive ? 'var(--v5-acc)' : isHover ? '#555' : '#2E2E2A'}/>
              </div>
              ...
              {isActive && (<div aria-hidden="true" style={{
                        marginLeft: 'auto',
                        marginRight: 10,
                        width: 4,
                        height: 4,
...
export function MarbleSidebar(props) {
    const { isVoid } = useV5Theme();
    return isVoid ? <VoidLayout {...props}/> : <MarbleLayout {...props}/>;
}
