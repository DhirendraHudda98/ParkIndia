...
describe("ParkingPass", () => {
  ...
  it("shows error when fetch fails", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText("Could not load parking pass"),
      ).toBeInTheDocument();
    });
  });
  ...
  it("shows loading state initially", () => {
    ...
    expect(
      screen.queryByAltText("QR code parking pass"),
    ).not.toBeInTheDocument();
    expect(
      screen.queryByText("Could not load parking pass"),
    ).not.toBeInTheDocument();
  });
  ...
  it("shows booking without vehicle plate", () => {
    ...
    render(<ParkingPass booking={bookingNoPlate} onClose={mockOnClose} />);
  ...
  it("handles re-mount with different booking id", async () => {
    const { rerender, unmount } = render(
      <ParkingPass booking={mockBooking} onClose={mockOnClose} />,
    );
    await waitFor(() =>
      expect(screen.getByAltText("QR code parking pass")).toBeInTheDocument(),
    );
    rerender(
      <ParkingPass
        booking={{ ...mockBooking, id: "different-id" }}
        onClose={mockOnClose}
      />,
    );
    await waitFor(() =>
