...
// ── Hoisted API mocks ──
const { mockGet2FAStatus, mockSetup2FA, mockVerify2FA, mockDisable2FA } =
  vi.hoisted(() => ({
...
vi.mock("@phosphor-icons/react", () => ({
  ShieldCheck: (props) => <span data-testid="icon-ShieldCheck" {...props} />,
  SpinnerGap: (props) => <span data-testid="icon-SpinnerGap" {...props} />,
...
describe("TwoFactorSetupComponent", () => {
  ...
  it("renders with 2FA disabled, shows Enable button", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.queryByText("Loading 2FA status..."),
      ).not.toBeInTheDocument();
    });

    expect(screen.getByText("Two-Factor Authentication")).toBeInTheDocument();
    expect(
      screen.getByText("Add an extra layer of security"),
    ).toBeInTheDocument();
  ...
  it("renders with 2FA enabled, shows Disable button", async () => {
    ...
    expect(
      screen.getByText("Enabled — your account is protected"),
    ).toBeInTheDocument();
  ...
  it("shows error when disable fails", async () => {
    ...
    mockDisable2FA.mockResolvedValue({
      success: false,
  ...
  it("shows error when verify API returns failure", async () => {
    ...
    await user.click(screen.getByText("Verify"));
    await waitFor(() =>
      expect(mockToast.error).toHaveBeenCalledWith("Bad code"),
    );
  });
  ...
  it("shows error when 2FA setup fails", async () => {
    ...
    mockSetup2FA.mockResolvedValue({
      success: false,
      error: { message: "Setup fail" },
    });
    render(<TwoFactorSetupComponent />);
    await waitFor(() => expect(screen.getByText("Enable")).toBeInTheDocument());
    await user.click(screen.getByText("Enable"));
    await waitFor(() =>
      expect(mockToast.error).toHaveBeenCalledWith("Setup fail"),
    );
  });
