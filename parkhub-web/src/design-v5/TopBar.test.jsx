...
function renderTopBar(overrides = {}) {
    return render(<V5ThemeProvider>
      <V5TopBar title="Dashboard" breadcrumb="HOME" onOpenCommand={vi.fn()} onToggleAssistant={vi.fn()} assistantOpen={overrides.assistantOpen ?? false}/>
    </V5ThemeProvider>);
}
describe('V5TopBar — assistant toggle branding', () => {
    it('uses Assistent aria-label and visible label without AI/KI text', () => {
        renderTopBar();
        expect(screen.getByRole('button', { name: 'Assistent umschalten' })).toBeInTheDocument();
