...
2. Replaces `vehicle_plate` on all bookings with `[DELETED]`
...
| **Access control** | 4-tier RBAC (user/premium/admin/superadmin), Sanctum token auth |
...
| **XSS prevention** | React JSX auto-escaping, nonce-based CSP |
...
ParkHub PHP does not set any cookies. The following `localStorage` entries are used — all are
...
*This guide covers DSGVO compliance for ParkHub PHP v3.2.0. For international compliance
