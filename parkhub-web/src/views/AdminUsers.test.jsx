...
vi.mock('react', async () => {
    ...
    const wrapped = function useStateWrapper(initial) {
        if (globalThis.__patchEnabled && initial instanceof Set && initial.size === 0) {
            return actual.useState(globalThis.__patchSelectedIds);
        }
...
vi.mock('../api/client', () => ({
    api: {
        adminUsers: (...a) => mockAdminUsers(...a),
        adminUpdateUserRole: (...a) => mockAdminUpdateUserRole(...a),
        adminUpdateUser: (...a) => mockAdminUpdateUser(...a),
        adminGrantCredits: (...a) => mockAdminGrantCredits(...a),
        adminUpdateUserQuota: (...a) => mockAdminUpdateUserQuota(...a),
        adminBulkUpdate: (...a) => mockAdminBulkUpdate(...a),
        adminBulkDelete: (...a) => mockAdminBulkDelete(...a),
    },
}));
...
vi.mock('../components/ui/DataTable', () => ({
    DataTable: ({ data, columns, emptyMessage }) => {
        ...
        {data.length === 0 ? <p>{emptyMessage}</p> : (<table>
            <tbody>
              {data.map((row) => (<tr key={row.id} data-testid={`user-row-${row.id}`}>
                  {columns.map((col, i) => {
...
describe('AdminUsersPage', () => {
    ...
    describe('bulk actions', () => {
        function patchSelectedIds(initial = new Set(['u-1', 'u-2'])) {
            ...
            globalThis.__patchSelectedIds = initial;
            globalThis.__patchEnabled = true;
        }
        afterEach(() => {
            globalThis.__patchEnabled = false;
        });
