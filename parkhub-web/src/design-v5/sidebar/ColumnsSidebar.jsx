 ...
 */
export function ColumnsSidebar({ active, onNavigate, userLabel = 'Administrator', userEmail }) {
    ...
    return (<nav aria-label="Hauptnavigation" data-variant="columns" style={{
            width: 280,
            flexShrink: 0,
            height: '100%',
            ...
            position: 'relative',
            overflow: 'hidden',
            backgroundImage: 'radial-gradient(ellipse 400px 300px at 0% 0%, color-mix(in oklch, var(--v5-acc) 12%, transparent), transparent 70%)',
      ...
      <div style={{
        ...
        <button type="button" aria-label={userName} title={`${userName} · ${userMail}`} style={{
            width: 28,
            height: 28,
            borderRadius: '50%',
            background: 'linear-gradient(135deg, var(--v5-acc), oklch(0.4 0.1 175))',
      ...
      <div style={{ padding: '4px 14px 14px' }}>
        <div style={{
            ...
            borderRadius: 14,
            background: 'linear-gradient(165deg, var(--v5-sur2) 0%, var(--v5-sur) 100%)',
          ...
          <div aria-hidden="true" style={{
            ...
            height: 160,
            borderRadius: '50%',
            background: 'radial-gradient(circle, color-mix(in oklch, var(--v5-acc) 30%, transparent), transparent 65%)',
            pointerEvents: 'none',
        }}/>
          ...
          <div style={{
            ...
            <button type="button" onClick={() => onNavigate('einchecken')} data-testid="cols-pass-link" style={{
            fontSize: 10,
            fontWeight: 700,
          ...
          <div className="v5-mono" style={{ fontSize: 28, lineHeight: 1, marginBottom: 2, position: 'relative', fontWeight: 700 }}>
          ...
          {/* Time until checkout */}
          {untilLabel && activeBooking?.end_time && (<div style={{ position: 'relative' }}>
              <div style={{
                display: 'flex',
                alignItems: 'baseline',
                justifyContent: 'space-between',
                ...
                <span className="v5-mono" style={{ fontSize: 12, fontWeight: 700, color: 'var(--v5-txt)' }}>
                  {untilLabel}
                </span>
              </div>
            </div>)}
          ...
          <div style={{ display: 'flex', gap: 6, marginTop: 12, position: 'relative' }}>
            <button type="button" onClick={() => onNavigate('einchecken')} style={{
            flex: 1,
            padding: '7px 10px',
            fontSize: 11.5,
            fontWeight: 700,
      ...
      <div style={{ flex: 1, overflowY: 'auto', overflowX: 'hidden', padding: '0 6px' }}>
        {['main', 'fleet', 'admin'].map((section) => (<div key={section} style={{ padding: '6px 8px 4px' }}>
            <SectionHeading section={section}/>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
              {NAV.filter((n) => n.section === section).map((item) => (<NumberedNavItem key={item.id} num={item.n} label={item.label} iconName={item.icon} isActive={active === item.id} onClick={() => onNavigate(item.id)}/>))}
            </div>
          </div>))}

        {/* Lot occupancy — signature card */}
        {currentLot && (<div style={{ padding: '14px 8px 8px' }}>
            <SectionHeading label="Aktueller Standort"/>
            <LotOccupancyCard name={currentLot.name ?? 'Standort'} total={currentLot.total_slots ?? 0} free={currentLot.available_slots ?? 0} onOpen={() => onNavigate('karte')}/>
          </div>)}
      </div>

      {/* FOOTER: What's next */}
      {upcomingBooking?.start_time && (<WhatsNextCard when={upcomingBooking.start_time} onClick={() => onNavigate('buchen')}/>)}
    </nav>);
}
function SectionHeading({ section, label }) {
    const text = label ??
...
function NumberedNavItem({ num, label, iconName, isActive, onClick, }) {
    const [hover, setHover] = useState(false);
    return (<button type="button" onClick={onClick} onMouseEnter={() => setHover(true)} onMouseLeave={() => setHover(false)} aria-current={isActive ? 'page' : undefined} style={{
            width: '100%',
            display: 'flex',
            alignItems: 'center',
            gap: 12,
      ...
      <div style={{
        ...
        <div>
          <div className="v5-mono" style={{ fontSize: 20, lineHeight: 1, fontWeight: 700 }}>
            {free}
            <span style={{ fontSize: 12, color: 'var(--v5-mut)', fontWeight: 500, marginLeft: 4 }}>
              / {total}
            </span>
          </div>
          <div style={{
...
function WhatsNextCard({ when, onClick }) {
    let label = '';
    try {
    ...
    return (<div data-testid="cols-whats-next" style={{
