...
class AdminAnalyticsController extends Controller
{
    ...
    public function overview(): JsonResponse
    {
        ...
        // 7. Bookings by city (last 30 days)
        $bookingsByCity = Booking::where('bookings.start_time', '>=', now()->subDays(30))
            ->tap($scopeBookingsQualified)
            ->join('parking_lots', 'bookings.lot_id', '=', 'parking_lots.id')
            ->selectRaw('COALESCE(parking_lots.city, "Unknown") as city, COUNT(*) as booking_count')
            ->groupBy('city')
            ->orderByDesc('booking_count')
            ->get()
            ->map(fn ($row) => [
