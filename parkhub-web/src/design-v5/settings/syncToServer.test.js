...
describe('syncSettingsToServer', () => {
    ...
    it('throws when the API resolves with success=false', async () => {
        ...
        await expect(syncSettingsToServer(DEFAULT_SETTINGS)).rejects.toThrow(/PAYLOAD_TOO_LARGE/);
    });
    it('propagates network rejections from updateSettings', async () => {
        ...
        await expect(syncSettingsToServer(DEFAULT_SETTINGS)).rejects.toThrow(/network down/);
    });
});
