...
export function SCard({ title, subtitle, actions, id, children }) {
  return (
    <section
      id={id}
...
export function SRow({ title, description, lockedBy, compact, children }) {
  return (
    <div
...
export function SSeg({ options, value, onChange, disabled }) {
  return (
    <div
      role="radiogroup"
...
export function SToggle({ value, onChange, disabled, label }) {
  return (
    <button
      type="button"
      role="switch"
...
export function ThemeSwatches({ value, onChange, options, disabled }) {
  return (
...
export const NAV_LAYOUTS = [
  {
    ...
    description: "Left rail with labels",
  },
  {
    value: "rail",
    label: "Icon rail",
    description: "Icons only, side-popping tooltip",
  },
  {
    value: "top",
    label: "Top tabs",
    description: "Horizontal navigation + overflow",
  },
  {
    value: "dock",
    label: "Floating dock",
    description: "macOS-style bottom dock",
  },
  {
    value: "focus",
    label: "Focus",
    description: "Dark rail with live pass + floor heatmap",
  },
];

export function NavLayoutGrid({ value, onChange, disabled }) {
  return (
    <div
      role="radiogroup"
 ...
 */
function NavLayoutPreview({ layout, active }) {
  const primary = active
    ? "bg-primary-500"
    : "bg-surface-300 dark:bg-surface-600";
  const secondary = "bg-surface-200 dark:bg-surface-700";
  const frame =
    "rounded bg-surface-100 dark:bg-surface-800 border border-surface-200 dark:border-surface-700";
  ...
  switch (layout) {
    ...
    case "top":
      return (
        <div
          className={`relative flex flex-col w-full h-20 p-1 gap-1 ${frame}`}
        >
        ...
        >
          <div className="w-2/5 flex flex-col gap-0.5 p-0.5">
            ...
            <span
              ...
              style={{ background: "oklch(0.58 0.16 25)" }}
            />
            <span
              className="block h-0.5 w-2/3 rounded-full"
              style={{ background: "oklch(0.70 0.14 75)" }}
            />
            <span
              className="block h-0.5 w-1/2 rounded-full"
              style={{ background: "oklch(0.58 0.14 150)" }}
            />
          </div>
          <div className="flex-1 flex flex-col gap-0.5 p-0.5">
            <span
              className="block h-2 w-3/4 rounded-sm"
              style={{ background: "oklch(0.32 0.02 260)" }}
            />
            <span
