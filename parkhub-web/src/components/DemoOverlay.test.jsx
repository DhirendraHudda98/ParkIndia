...
vi.mock("../api/client", () => ({
  api: {
    getDemoConfig: (...a) => mockGetDemoConfig(...a),
    getDemoStatus: (...a) => mockGetDemoStatus(...a),
    voteDemoReset: (...a) => mockVoteDemoReset(...a),
  },
}));
  ...
  useTranslation: () => ({
    t: (key, fallbackOrOpts, opts) => {
      ...
      if (key === "demo.votesNeeded") {
        const o =
          (typeof fallbackOrOpts === "object" ? fallbackOrOpts : opts) || {};
        return `${o.current ?? 0}/${o.needed ?? 0}`;
      }
      return (
        map[key] || (typeof fallbackOrOpts === "string" ? fallbackOrOpts : key)
      );
    },
  }),
}));
...
describe("DemoOverlay component", () => {
  ...
  it("renders nothing when demo mode is disabled", async () => {
    mockGetDemoConfig.mockResolvedValue({
      success: true,
  ...
  it("updates the local countdown every second while expanded", async () => {
    ...
    expect(
      screen.getByText((_, element) => element?.textContent === "29:59"),
    ).toBeInTheDocument();
  });
});
