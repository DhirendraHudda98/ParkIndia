...
export function KpiCard({
  ...
  return (
    <motion.div
      ...
      transition={spring}
      data-testid={
        rest["data-testid"] ?? `kpi-${label.toLowerCase().replace(/\s+/g, "-")}`
      }
...
function DeltaBadge({ value, suffix = "%", positive, Icon }) {
    ...
    >
      <header className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3 mb-4">
        ...
        {periods && periods.length > 0 && (
          <nav
            className="inline-flex rounded-lg bg-surface-100 dark:bg-surface-800/60 p-0.5"
            aria-label={title}
          >
            {periods.map((p) => (
              <button
      ...
      >
        <defs>
          <linearGradient id="trend-grad" x1="0%" y1="0%" x2="0%" y2="100%">
            <stop
              offset="0%"
              stopColor="var(--color-primary-500, #14b8a6)"
              stopOpacity="0.35"
            />
            <stop
              offset="100%"
...
export function SensorFeedCard({ title, subtitle, sensors }) {
  return (
    <motion.section
      initial={{ opacity: 0, y: 12 }}
      animate={{ opacity: 1, y: 0 }}
          ...
          return (
            ...
            >
              <span className="flex items-center gap-2 text-sm text-surface-700 dark:text-surface-300">
                <span
                  className={`w-1.5 h-1.5 rounded-full ${styles.dot} ${s.status === "active" ? "pulse-dot" : ""}`}
                />
                {s.name}
              </span>
              <span
    ...
    >
      ...
      {rows.length === 0 ? (
        <div className="py-12 text-center">
          <CircleDashed
            weight="bold"
            className="w-8 h-8 text-surface-300 dark:text-surface-600 mx-auto mb-3"
          />
            ...
            <tbody>
              {rows.map((r) => (
                ...
                >
                  ...
                  <td
                    className="py-3 pr-4 text-surface-700 dark:text-surface-300"
                    style={{ fontVariantNumeric: "tabular-nums" }}
                  >
                    {r.slot}
                  </td>
                  <td
                    className="py-3 pr-4 text-surface-700 dark:text-surface-300 hidden sm:table-cell"
                    style={{ fontVariantNumeric: "tabular-nums" }}
                  >
                    {r.checkInTime ?? "—"}
                  </td>
                  <td
                    className="py-3 pr-4 text-surface-700 dark:text-surface-300 hidden md:table-cell"
                    style={{ fontVariantNumeric: "tabular-nums" }}
                  >
                    {r.duration ?? "—"}
                  </td>
