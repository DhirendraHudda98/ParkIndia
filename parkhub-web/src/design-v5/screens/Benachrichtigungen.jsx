...
export function BenachrichtigungenV5({ navigate: _navigate }) {
    ...
    const SEVERITIES = [
        ...
        { value: 'success', label: t('common.success', 'Success') },
        { value: 'warning', label: t('common.warning', 'Warning') },
        { value: 'critical', label: t('common.critical', 'Critical') },
    ];
    ...
    const { data: items = [], isLoading, isError } = useQuery({
        ...
        queryFn: async () => {
            ...
            if (!res.success)
                throw new Error(res.error?.message ?? 'Could not load announcements');
            return res.data ?? [];
        },
        staleTime: 30_000,
    });
    ...
    return (<div style={{ padding: 16, flex: 1, overflow: 'auto', display: 'flex', flexDirection: 'column', gap: 12 }}>
      <div className="v5-ani" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <span style={{ fontWeight: 700, fontSize: 15, color: 'var(--v5-txt)' }}>{t('announcements.title', 'Announcements')}</span>
          <Badge variant="gray"><NumberFlow value={items.length}/></Badge>
          {activeCount > 0 && <Badge variant="primary" dot>{activeCount} active</Badge>}
        </div>
      </div>

      <Card className="v5-ani" style={{ padding: 16, display: 'flex', flexDirection: 'column', gap: 10, animationDelay: '0.06s' }}>
        <div style={{ fontSize: 12, fontWeight: 600, color: 'var(--v5-txt)' }}>{t('announcements.new', 'New Announcement')}</div>
        <input data-testid="benach-new-title" type="text" placeholder={t('announcements.field_title', 'Title')} value={title} onChange={(e) => setTitle(e.target.value)} style={{
        ...
        <textarea data-testid="benach-new-message" placeholder={t('announcements.field_message', 'Message')} value={message} onChange={(e) => setMessage(e.target.value)} rows={3} style={{
            padding: '8px 11px', borderRadius: 9, background: 'var(--v5-sur2)',
            border: '1px solid var(--v5-bor)', color: 'var(--v5-txt)', fontSize: 12,
            width: '100%', outline: 'none', boxSizing: 'border-box', fontFamily: 'inherit',
            resize: 'vertical',
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
          <div role="group" aria-label="Priority" style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
            {SEVERITIES.map((s) => {
            const selected = severity === s.value;
            return (<button key={s.value} type="button" aria-pressed={selected} onClick={() => setSeverity(s.value)} style={{
          ...
          <label style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 11, color: 'var(--v5-mut)' }}>
            <input type="checkbox" checked={active} onChange={(e) => setActive(e.target.checked)} data-testid="benach-new-active"/>
            {t('announcements.field_active', 'Active')}
          </label>
          <div style={{ flex: 1 }}/>
          <button type="button" disabled={!title.trim() || !message.trim() || createMutation.isPending} onClick={() => createMutation.mutate()} data-testid="benach-new-submit" style={{
            ...
            {createMutation.isPending ? 'Sending…' : t('common.send', 'Send')}
          </button>
        </div>
      </Card>

      {items.length === 0 ? (<Card className="v5-ani" style={{ padding: 36, textAlign: 'center', animationDelay: '0.12s' }}>
          <V5NamedIcon name="bell" size={20} color="var(--v5-mut)"/>
          <div style={{ marginTop: 10, fontWeight: 600, color: 'var(--v5-txt)' }}>{t('announcements.none', 'No Announcements')}</div>
          <div style={{ fontSize: 12, color: 'var(--v5-mut)', marginTop: 4 }}>
            {t('announcements.none_hint', 'Create a new announcement for the team above.')}
          </div>
          ...
          {items.map((a, i) => {
                ...
                return (<Card key={a.id} data-testid="benach-row" className="v5-ani" style={{
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: 'flex', alignItems: 'baseline', gap: 8, flexWrap: 'wrap' }}>
                    ...
                    {!a.active && <Badge variant="gray">Inactive</Badge>}
                ...
                <button type="button" disabled={deleting} aria-label={`Delete announcement ${a.id}`} onClick={() => deleteMutation.mutate(a.id)} style={{
                    ...
                    }}>{deleting ? '…' : t('common.delete', 'Delete')}</button>
              </Card>);
            })}
        </div>)}
    </div>);
