...
describe("InstallPrompt", () => {
    ...
    Object.defineProperty(navigator, "serviceWorker", {
      value: {
        addEventListener: vi.fn((_, handler) =>
          swMessageListeners.push(handler),
        ),
        removeEventListener: vi.fn(),
  ...
  it("shows sync status toast when MUTATION_QUEUED message received", async () => {
    ...
    act(() => {
      swMessageListeners.forEach((handler) => {
        handler({ data: { type: "MUTATION_QUEUED", queueLength: 3 } });
      });
    });
  ...
  it("clears sync toast on SYNC_RESULT with synced > 0", async () => {
    ...
    act(() => {
      swMessageListeners.forEach((handler) => {
        handler({ data: { type: "MUTATION_QUEUED", queueLength: 2 } });
      });
    });

    act(() => {
      swMessageListeners.forEach((handler) => {
        handler({ data: { type: "SYNC_RESULT", synced: 2 } });
      });
    });
  ...
  it("clears sync toast immediately on SYNC_RESULT with synced 0", async () => {
    ...
    act(() => {
      swMessageListeners.forEach((handler) => {
        handler({ data: { type: "MUTATION_QUEUED", queueLength: 1 } });
      });
    });
    ...
    act(() => {
      swMessageListeners.forEach((handler) => {
        handler({ data: { type: "SYNC_RESULT", synced: 0 } });
      });
    });
