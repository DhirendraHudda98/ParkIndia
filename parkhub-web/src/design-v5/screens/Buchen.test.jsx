...
vi.mock('../../api/client', () => ({
    api: {
        ...
        getLotSlots: (...a) => mockGetLotSlots(...a),
        getVehicles: (...a) => mockGetVehicles(...a),
...
describe('BuchenV5', () => {
    ...
    it('blocks confirm when startDate is cleared and toasts an error without calling createBooking', async () => {
        ...
        const startInput = document.getElementById('buchen-start');
        expect(startInput).toBeTruthy();
        fireEvent.change(startInput, { target: { value: '' } });
        fireEvent.click(screen.getByRole('button', { name: /Weiter/ }));
        await waitFor(() => expect(screen.getByTestId('buchen-confirm')).toBeInTheDocument());
        fireEvent.click(screen.getByTestId('buchen-confirm'));
        await waitFor(() => expect(mockToast).toHaveBeenCalledWith('Bitte gültige Zeiten angeben', 'error'));
