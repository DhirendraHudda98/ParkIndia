  ...
  it("does not render when online", () => {
    ...
    globalThis.fetch = vi.fn().mockResolvedValue({
      ...
      json: () =>
        Promise.resolve({
          success: true,
          data: { next_booking: null, lot_info: [] },
        }),
    });
...
describe("BottomNavBar", () => {
  ...
  it("marks nested booking routes as active", () => {
    ...
    expect(screen.getByLabelText("Bookings")).toHaveAttribute(
      "aria-current",
      "page",
    );
  });
...
describe("OfflineIndicator - events", () => {
  it("responds to offline/online events", async () => {
    ...
    expect(
      screen.queryByText("You are offline. Some features may be unavailable."),
    ).toBeNull();
  });
});
