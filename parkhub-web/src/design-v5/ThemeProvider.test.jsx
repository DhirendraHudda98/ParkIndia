...
import { V5ThemeProvider, useV5Theme, V5_MODES } from './ThemeProvider';
function Probe() {
    ...
    return (<div>
      ...
      {V5_MODES.map((m) => (<button key={m} onClick={() => setMode(m)} data-testid={`set-${m}`}>
          {m}
        </button>))}
    </div>);
}
describe('V5ThemeProvider', () => {
    ...
    it('ignores an unknown mode string in storage', () => {
        window.localStorage.setItem('ph-v5-mode', 'neon-explosion');
        render(<V5ThemeProvider>
        <Probe />
      </V5ThemeProvider>);
