...
vi.mock('react-i18next', () => ({ useTranslation: () => ({ t: (k, o) => o?.name ? `Share ${o.name}` : o?.code ? `Code: ${o.code}` : k }) }));
...
describe('GuestPassPage', () => {
    beforeEach(() => {
        ...
        globalThis.fetch = vi.fn((url, opts) => {
            ...
            if (opts?.method === 'POST')
                return Promise.resolve({ json: () => Promise.resolve({ success: true, data: { id: 'gnew', lot_id: 'l1', lot_name: 'Lot A', slot_id: 's1', slot_number: '5', guest_name: 'New Guest', guest_email: null, guest_code: 'XYZ789', start_time: '2026-04-12T08:00:00Z', end_time: '2026-04-12T17:00:00Z', status: 'active', created_at: '2026-04-12' } }) });
            if (url.includes('/slots'))
                return Promise.resolve({ json: () => Promise.resolve({ success: true, data: slots }) });
            if (url.includes('/lots'))
                return Promise.resolve({ json: () => Promise.resolve({ success: true, data: lots }) });
            if (url.includes('/bookings/guest'))
                return Promise.resolve({ json: () => Promise.resolve({ success: true, data: guestBookings }) });
    ...
    it('create failure', async () => {
        globalThis.fetch = vi.fn((url, opts) => {
            if (opts?.method === 'POST')
                return Promise.resolve({ json: () => Promise.resolve({ success: false, error: { message: 'Slot taken' } }) });
            if (url.includes('/slots'))
                return Promise.resolve({ json: () => Promise.resolve({ success: true, data: slots }) });
            if (url.includes('/lots'))
                return Promise.resolve({ json: () => Promise.resolve({ success: true, data: lots }) });
