    ...
    beforeEach(() => {
        global.fetch = vi.fn((url) => {
            if (typeof url === 'string' && url.includes('/api/v1/visitors')) {
                return Promise.resolve({ json: () => Promise.resolve({ success: true, data: sampleVisitors }) });
            }
...
describe('AdminVisitorsPage', () => {
    beforeEach(() => {
        global.fetch = vi.fn(() => Promise.resolve({ json: () => Promise.resolve({ success: true, data: sampleVisitors }) }));
    });
    afterEach(() => {
        vi.restoreAllMocks();
    });
    ...
    it('shows empty state when no visitors', async () => {
        global.fetch = vi.fn(() => Promise.resolve({ json: () => Promise.resolve({ success: true, data: [] }) }));
        render(<AdminVisitorsPage />);
...
describe('VisitorsPage - extended', () => {
    beforeEach(() => {
        global.fetch = vi.fn((url, opts) => {
            ...
            if (typeof url === 'string' && url.includes('/register')) {
                return Promise.resolve({ json: () => Promise.resolve({ success: true, data: { id: 'v-new' } }) });
            }
    ...
    it('submits registration form successfully', async () => {
        ...
        // date
        if (dateInput)
            fireEvent.change(dateInput, { target: { value: '2026-04-20T10:00' } });
        // vehicle plate (optional)
        if (textInputs[1])
            fireEvent.change(textInputs[1], { target: { value: 'XY-999' } });
        // purpose (optional)
        if (textInputs[2])
            fireEvent.change(textInputs[2], { target: { value: 'Meeting' } });
        fireEvent.click(screen.getByText('Save'));
        await waitFor(() => {
            expect(global.fetch).toHaveBeenCalledWith('/api/v1/visitors/register', expect.objectContaining({ method: 'POST' }));
        });
    });
    ...
    it('shows error when registration server returns failure', async () => {
        ...
        global.fetch = vi.fn((url, opts) => {
            if (typeof url === 'string' && url.includes('/visitors/register')) {
                return Promise.resolve({ json: () => Promise.resolve({ success: false, error: { message: 'Server fail' } }) });
            }
            return Promise.resolve({ json: () => Promise.resolve({ success: true, data: sampleVisitors }) });
        });
        render(<VisitorsPage />);
    ...
    it('checks in a pending visitor', async () => {
        ...
        await waitFor(() => {
            expect(global.fetch).toHaveBeenCalledWith('/api/v1/visitors/v1/check-in', expect.objectContaining({ method: 'PUT' }));
        });
    });
    it('cancels a pending visitor', async () => {
        ...
        await waitFor(() => {
            expect(global.fetch).toHaveBeenCalledWith('/api/v1/visitors/v1', expect.objectContaining({ method: 'DELETE' }));
        });
    });
    ...
    it('handles check-in error', async () => {
        global.fetch = vi.fn((url, opts) => {
            ...
            if (typeof url === 'string' && url.includes('/check-in')) {
                return Promise.resolve({ json: () => Promise.resolve({ success: false, error: { message: 'Already checked in' } }) });
            }
            return Promise.resolve({ json: () => Promise.resolve({ success: true, data: [] }) });
        });
        render(<VisitorsPage />);
    ...
    it('handles cancel error', async () => {
        global.fetch = vi.fn((url, opts) => {
            ...
            if (opts?.method === 'DELETE') {
                return Promise.resolve({ json: () => Promise.resolve({ success: false, error: { message: 'Cannot cancel' } }) });
            }
            return Promise.resolve({ json: () => Promise.resolve({ success: true, data: [] }) });
        });
        render(<VisitorsPage />);
