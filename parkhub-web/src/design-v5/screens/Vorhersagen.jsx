...
function fallbackPrediction(idx) {
    if (idx < 5)
        return { predicted: 60, peakHour: 9, offPeakHour: 14 };
...
export function VorhersagenV5({ navigate: _navigate }) {
    ...
    if (statsQuery.isLoading) {
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: 8 }}>
          {[0, 1, 2, 3, 4, 5, 6].map((i) => (<div key={i} style={{ height: 160, borderRadius: 12, background: 'var(--v5-sur2)', animation: 'ph-v5-pulse 1.6s ease infinite', animationDelay: `${i * 0.06}s` }}/>))}
        </div>
      </div>);
    }
    ...
    return (<div style={{ padding: 16, flex: 1, overflow: 'auto', display: 'flex', flexDirection: 'column', gap: 12 }}>
      ...
      {recommendation && (<Card className="v5-ani" style={{ padding: 18, animationDelay: '0.06s', background: 'linear-gradient(145deg, var(--v5-acc-muted), transparent)', border: '1px solid color-mix(in oklch, var(--v5-acc) 30%, transparent)' }} data-testid="recommendation">
      ...
      <div className="v5-ani" style={{ animationDelay: '0.12s' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(120px, 1fr))', gap: 8 }} data-testid="forecast-grid">
          {predictions.map((day) => (<Card key={day.dayIndex} data-testid="day-card" style={{ padding: 12, display: 'flex', flexDirection: 'column', gap: 8 }}>
              ...
              <div className="v5-mono" style={{ fontSize: 22, fontWeight: 800, color: levelColor(day.level), fontVariantNumeric: 'tabular-nums', letterSpacing: -1 }}>
                {day.predicted}%
              </div>
