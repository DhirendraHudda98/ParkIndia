    ...
    useTranslation: () => ({
        t: (key, fallback, opts) => {
            ...
            if (opts && typeof base === 'string') {
                return Object.entries(opts).reduce((acc, [k, v]) => acc.replace(new RegExp(`{{\\s*${k}\\s*}}`, 'g'), String(v)), base);
            }
            return base;
        },
    }),
...
describe('AdminModulesPage', () => {
    ...
    it('PATCHes the module API with the correct payload when toggled', async () => {
        ...
        const patchCall = fetchMock.mock.calls.find((c) => typeof c[0] === 'string' && c[0].includes('/api/v1/admin/modules/'));
