...
vi.mock('../api/client', () => ({
    api: {
        getTranslationProposals: (...args) => mockGetProposals(...args),
        reviewProposal: (...args) => mockReviewProposal(...args),
    },
}));
...
vi.mock('framer-motion', () => ({
    motion: {
        div: React.forwardRef(({ children, initial, animate, exit, transition, variants, whileHover, whileTap, layout, ...props }, ref) => (<div ref={ref} {...props}>{children}</div>)),
    },
...
vi.mock('../components/ui/DataTable', () => ({
    DataTable: ({ data, columns, emptyMessage }) => (<div data-testid="data-table">
      {data.length === 0 ? <p>{emptyMessage}</p> : (<div>
          <p>{data.length} items</p>
          {data.map((row) => (<div key={row.id} data-testid={`proposal-row-${row.id}`}>
              {columns.map((col, i) => {
