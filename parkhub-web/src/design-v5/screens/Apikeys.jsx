...
export function ApikeysV5({ navigate: _navigate }) {
    ...
    const { data: keys = [], isLoading, isError } = useQuery({
        ...
        queryFn: async () => {
            ...
            if (!res.success)
                throw new Error(res.error?.message ?? 'API-Schlüssel konnten nicht geladen werden');
            return res.data ?? [];
        },
        staleTime: 30_000,
    });
    const create = useMutation({
        mutationFn: async (label) => {
            ...
            if (!res.success)
                throw new Error(res.error?.message ?? 'Erstellen fehlgeschlagen');
            return res.data;
        },
        onSuccess: (data) => {
        ...
        onError: (err) => toast(err.message || 'Erstellen fehlgeschlagen', 'error'),
    });
    const rotate = useMutation({
        mutationFn: async (id) => {
            const res = await api.rotateApiKey(id);
            if (!res.success)
                throw new Error(res.error?.message ?? 'Rotation fehlgeschlagen');
            return res.data;
        },
        onSuccess: (data) => {
            qc.invalidateQueries({ queryKey: ['apikeys'] });
        ...
        onError: (err) => toast(err.message || 'Rotation fehlgeschlagen', 'error'),
    });
    const revoke = useMutation({
        mutationFn: async (id) => {
            const res = await api.revokeApiKey(id);
            if (!res.success)
                throw new Error(res.error?.message ?? 'Widerruf fehlgeschlagen');
            return res.data;
        },
        onSuccess: () => {
            qc.invalidateQueries({ queryKey: ['apikeys'] });
        ...
        onError: (err) => toast(err.message || 'Widerruf fehlgeschlagen', 'error'),
    });
    async function copyToken() {
        if (!revealed?.token || typeof navigator === 'undefined' || !navigator.clipboard)
            return;
        try {
    ...
    return (<div style={{ padding: 16, flex: 1, overflow: 'auto', display: 'flex', flexDirection: 'column', gap: 12 }}>
      ...
      {revealed && (<Card className="v5-ani" data-testid="apikeys-revealed" style={{ padding: 16, borderLeft: '3px solid var(--v5-warn)' }}>
          ...
          <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
            ...
            <button type="button" onClick={copyToken} data-testid="apikeys-copy" style={{ padding: '7px 12px', borderRadius: 9, background: 'var(--v5-acc)', color: 'var(--v5-accent-fg)', border: 'none', fontSize: 11, fontWeight: 600, cursor: 'pointer' }}>Kopieren</button>
            <button type="button" onClick={() => setRevealed(null)} data-testid="apikeys-dismiss" style={{ padding: '7px 12px', borderRadius: 9, background: 'var(--v5-sur2)', border: '1px solid var(--v5-bor)', color: 'var(--v5-txt)', fontSize: 11, fontWeight: 500, cursor: 'pointer' }}>Schließen</button>
          </div>
        </Card>)}
      ...
      <Card className="v5-ani" style={{ padding: 16, display: 'flex', flexDirection: 'column', gap: 10 }}>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <input data-testid="apikeys-label" placeholder="Label (z. B. CI/CD)" value={newLabel} onChange={(e) => setNewLabel(e.target.value)} style={{ flex: 1, padding: '8px 11px', borderRadius: 9, background: 'var(--v5-sur2)', border: '1px solid var(--v5-bor)', color: 'var(--v5-txt)', fontSize: 12, outline: 'none', fontFamily: 'inherit' }}/>
          <button type="button" disabled={newLabel.trim().length < 2 || create.isPending} onClick={() => create.mutate(newLabel.trim())} data-testid="apikeys-create" style={{
      ...
      {keys.length === 0 ? (<Card className="v5-ani" style={{ padding: 36, textAlign: 'center' }}>
          ...
          {keys.map((k, i) => {
                ...
                <div style={{ display: 'flex', gap: 6, justifyContent: 'flex-end' }}>
                  <button type="button" disabled={rotBusy} onClick={() => rotate.mutate(k.id)} aria-label={`Schlüssel ${k.label} rotieren`} style={{ padding: '4px 10px', borderRadius: 7, background: 'var(--v5-acc-muted)', border: 'none', fontSize: 10, fontWeight: 500, color: 'var(--v5-acc)', cursor: rotBusy ? 'default' : 'pointer', opacity: rotBusy ? 0.5 : 1 }}>{rotBusy ? '…' : 'Rotieren'}</button>
                  <button type="button" disabled={revBusy} onClick={() => revoke.mutate(k.id)} aria-label={`Schlüssel ${k.label} widerrufen`} style={{ padding: '4px 10px', borderRadius: 7, background: 'color-mix(in oklch, var(--v5-err) 8%, transparent)', border: 'none', fontSize: 10, fontWeight: 500, color: 'var(--v5-err)', cursor: revBusy ? 'default' : 'pointer', opacity: revBusy ? 0.5 : 1 }}>{revBusy ? '…' : 'Widerrufen'}</button>
                </div>
              </div>);
            })}
        </Card>)}
