...
describe('useNotifications', () => {
    ...
    beforeEach(() => {
        ...
        Object.defineProperty(globalThis, 'PushManager', { value: class {
            }, writable: true, configurable: true });
        globalThis.fetch = vi.fn((url) => {
            if (url.includes('/vapid-key'))
                return Promise.resolve({ ok: true, json: () => Promise.resolve({ data: { public_key: 'AAAA' } }) });
    ...
    it('subscribe empty vapid key', async () => {
        globalThis.fetch = vi.fn((url) => {
            if (url.includes('/vapid-key'))
                return Promise.resolve({ ok: true, json: () => Promise.resolve({ data: { public_key: null } }) });
            return Promise.resolve({ ok: true });
        });
        const { result } = renderHook(() => useNotifications());
        await act(async () => { await result.current.subscribe(); });
    ...
    it('subscribe server registration fails', async () => {
        globalThis.fetch = vi.fn((url) => {
            if (url.includes('/vapid-key'))
                return Promise.resolve({ ok: true, json: () => Promise.resolve({ data: { public_key: 'BEl62iUYgUivxIkv69yViEuiBIa' } }) });
            if (url.includes('/subscribe'))
                return Promise.resolve({ ok: false });
            return Promise.resolve({ ok: true });
        });
    ...
    it('subscribe catches non-Error exceptions', async () => {
        Notification.requestPermission = vi.fn().mockRejectedValue('string error');
        const { result } = renderHook(() => useNotifications());
        await act(async () => { await result.current.subscribe(); });
