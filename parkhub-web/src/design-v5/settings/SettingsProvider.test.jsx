...
import { V5SettingsProvider, useV5Feature, useV5Settings, } from './SettingsProvider';
...
describe('<V5SettingsProvider>', () => {
    ...
    it('debounces sync to backend', async () => {
        ...
        const user = userEvent.setup();
        render(<V5SettingsProvider syncToServer={sync} syncDebounceMs={50}>
        <Probe />
      </V5SettingsProvider>);
        await user.click(screen.getByText('set-void'));
        ...
        const last = sync.mock.calls[sync.mock.calls.length - 1]?.[0];
    ...
    it('reflects sync errors in syncState', async () => {
        ...
        const user = userEvent.setup();
        render(<V5SettingsProvider syncToServer={sync} syncDebounceMs={20}>
        <Probe />
      </V5SettingsProvider>);
        await user.click(screen.getByText('set-void'));
    ...
    it('useV5Feature returns the live flag value', () => {
        ...
        render(<V5SettingsProvider initialOverride={{
                ...DEFAULT_SETTINGS,
