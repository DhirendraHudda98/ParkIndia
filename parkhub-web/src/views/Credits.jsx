        ...
        <div className="grid gap-6 lg:grid-cols-[1.15fr_0.85fr]">
          <div>
            <div className={`mb-3 inline-flex items-center gap-2 rounded-full px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.2em] ${
              ...
              {isVoid ? 'Void balance deck' : isIndia ? 'India Heritage Ledger' : 'Marble credit ledger'}
            </div>
          ...
          <div className={`rounded-[24px] border p-5 ${
            ...
            <div className="flex items-center gap-5">
              <CreditProgressRing value={balance} total={quota} isVoid={isVoid} isIndia={isIndia} />
              <div className="min-w-0 flex-1">
                ...
                <p className={`mt-2 text-sm leading-6 ${isVoid ? 'text-slate-300' : isIndia ? 'text-[#000080]/70 dark:text-[#FF9933]/80' : 'text-surface-600 dark:text-surface-300'}`}>
                ...
                <div className="mt-4 flex flex-wrap gap-2">
                  <span className={`rounded-full px-3 py-1 text-xs font-semibold ${isVoid ? 'bg-cyan-500/10 text-cyan-100' : isIndia ? 'bg-[#138808]/10 text-[#138808]' : 'bg-emerald-500/10 text-emerald-700 dark:text-emerald-300'}`}>
      ...
      <motion.div variants={item} className="grid grid-cols-1 gap-4 lg:grid-cols-[1.1fr_0.9fr]">
        <section className={`rounded-[24px] border p-5 shadow-[0_18px_50px_-38px_rgba(15,23,42,0.18)] ${isIndia ? 'border-[#FF9933]/10 bg-white' : 'border-surface-200 bg-white dark:border-surface-800 dark:bg-surface-950/80'}`}>
          <div className="mb-4 flex items-center justify-between gap-3">
            <div>
              ...
              <h2 className={`mt-1 text-xl font-semibold tracking-[-0.03em] ${isIndia ? 'text-[#000080]' : 'text-surface-900 dark:text-white'}`}>{t('credits.summaryTitle', 'Credit overview')}</h2>
            </div>
            <button 
              type="button" 
              onClick={async () => {
                const toast = (await import('react-hot-toast')).default;
                const toastId = toast.loading(t('credits.adding', 'Adding credits...'));
                const res = await api.addWalletCredits(10);
                if (res.success) {
                  toast.success(t('credits.addSuccess', '10 credits added!'), { id: toastId });
                  api.getUserCredits().then((res) => {
                    if (res.success && res.data) setCredits(res.data);
                  });
                } else {
                  toast.error(t('credits.addError', 'Failed to add credits'), { id: toastId });
                }
              }}
              className={`rounded-full border px-3 py-1.5 text-xs font-semibold transition-colors ${isIndia ? 'border-[#FF9933] text-[#FF9933] hover:bg-[#FF9933] hover:text-white' : 'border-surface-200 text-surface-600 hover:border-primary-400 hover:text-primary-700 dark:border-surface-700 dark:text-surface-300'}`}>
        ...
        <section className={`rounded-[24px] border p-5 ${
          isVoid
            ...
            : isIndia
            ? 'border-[#FF9933]/10 bg-white/80 dark:bg-[#000080]/10'
          ...
          <p className={`text-[11px] font-semibold uppercase tracking-[0.2em] ${isVoid ? 'text-white/45' : isIndia ? 'text-[#FF9933]' : 'text-surface-500 dark:text-surface-400'}`}>
          ...
          <div className="mt-4 space-y-3">
            <LedgerSummaryRow
              label={t('credits.balance', 'Balance')}
              value={String(balance)}
              tone={isVoid ? 'text-cyan-200' : isIndia ? 'text-[#000080]' : 'text-emerald-700 dark:text-emerald-300'}
            />
            <LedgerSummaryRow
              label={t('credits.additions', 'Additions')}
              value={`+${spotlight.additions}`}
              tone={isVoid ? 'text-emerald-200' : isIndia ? 'text-[#138808]' : 'text-emerald-700 dark:text-emerald-300'}
            />
            <LedgerSummaryRow
              label={t('credits.deductions', 'Deductions')}
              value={`-${spotlight.deductions}`}
              tone={isVoid ? 'text-rose-200' : isIndia ? 'text-[#FF9933]' : 'text-rose-700 dark:text-rose-300'}
            />
            <LedgerSummaryRow
              label={t('credits.nextRefill', 'Next Refill')}
              value={nextRefill}
              tone={isVoid ? 'text-white' : isIndia ? 'text-[#000080]' : 'text-surface-900 dark:text-white'}
            />
          </div>
        </section>
      </motion.div>

      <motion.section variants={item} className={`rounded-[24px] border p-5 shadow-[0_18px_50px_-38px_rgba(15,23,42,0.18)] ${isIndia ? 'border-[#FF9933]/10 bg-white' : 'border-surface-200 bg-white dark:border-surface-800 dark:bg-surface-950/80'}`}>
        <div className="mb-4 flex items-center justify-between gap-3">
          <div>
            ...
            <h2 className={`mt-1 text-xl font-semibold tracking-[-0.03em] ${isIndia ? 'text-[#000080]' : 'text-surface-900 dark:text-white'}`}>{t('credits.ledgerTitle', 'Transaction ledger')}</h2>
          </div>
        ...
        {!transactions.length ? (
          ...
          <div className="space-y-3">
            {transactions.map((tx) => (
              <CreditTransactionRow key={tx.id} tx={tx} isIndia={isIndia} />
            ))}
          </div>
        )}
      </motion.section>
...
function CreditHeroStat({
  ...
  return (
    <div className={`rounded-[22px] border px-4 py-4 ${
      ...
      <p className={`mt-1 text-xs ${isVoid ? 'text-slate-300' : isIndia ? 'text-[#000080]/50' : 'text-surface-500 dark:text-surface-400'}`}>{meta}</p>
    </div>
  );
}

function CreditProgressRing({ value, total, isVoid, isIndia }) {
  const radius = 34;
  ...
  return (
    <div className="relative h-24 w-24 shrink-0">
      <svg width={96} height={96} viewBox="0 0 96 96">
        <circle cx={48} cy={48} r={radius} fill="none" stroke={isVoid ? 'rgba(148,163,184,0.18)' : isIndia ? '#00008010' : '#e7e5e4'} strokeWidth={10} />
        <circle
          cx={48}
          cy={48}
          r={radius}
          fill="none"
          stroke={isVoid ? '#22d3ee' : isIndia ? '#FF9933' : '#10b981'}
          strokeWidth={10}
          strokeLinecap="round"
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center">
        <span className={`text-lg font-black ${isVoid ? 'text-white' : isIndia ? 'text-[#000080]' : 'text-surface-900 dark:text-white'}`}>{value}</span>
        <span className={`text-[10px] ${isVoid ? 'text-slate-400' : isIndia ? 'text-[#000080]/40' : 'text-surface-500 dark:text-surface-400'}`}>/ {total}</span>
      </div>
    </div>
  );
}
...
function MetricCard({
  ...
  const toneClasses = {
    ...
    india: 'bg-[#FF9933]/10 text-[#FF9933]',
  };
...
function LedgerSummaryRow({ label, value, tone }) {
  return (
...
function CreditTransactionRow({ tx, isIndia }) {
  ...
  return (
    <div className="flex items-center gap-3 rounded-[20px] border border-surface-200 bg-surface-50/80 px-4 py-3 dark:border-surface-800 dark:bg-surface-900/60">
      <div className={`flex h-10 w-10 items-center justify-center rounded-2xl ${
        positive
          ? isIndia ? 'bg-[#138808]/10 text-[#138808]' : 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-300'
          : isIndia ? 'bg-[#FF9933]/10 text-[#FF9933]' : 'bg-rose-100 text-rose-700 dark:bg-rose-900/30 dark:text-rose-300'
      }`}>
        {positive ? <ArrowDown weight="bold" className="h-4 w-4" /> : <ArrowUp weight="bold" className="h-4 w-4" />}
      </div>

      <div className="min-w-0 flex-1">
        <p className={`text-sm font-medium ${isIndia ? 'text-[#000080]' : 'text-surface-900 dark:text-white'}`}>
      ...
      <div className="text-right">
        <span className={`text-sm font-semibold ${positive ? (isIndia ? 'text-[#138808]' : 'text-emerald-700 dark:text-emerald-300') : (isIndia ? 'text-[#FF9933]' : 'text-rose-700 dark:text-rose-300')}`}>
          {positive ? '+' : ''}{tx.amount}
        </span>
...
function estimateNextRefill(lastRefilled) {
