...
describe("ConfigEditor", () => {
  ...
  it("renders every supported field type with the right input", () => {
    ...
    expect(
      screen.getByTestId("cfg-field-notify_email").getAttribute("type"),
    ).toBe("email");
    expect(screen.getByTestId("cfg-field-opens_at").getAttribute("type")).toBe(
      "time",
    );
    expect(screen.getByTestId("cfg-field-label").getAttribute("type")).toBe(
      "text",
    );
    expect(screen.getByTestId("cfg-field-active").getAttribute("role")).toBe(
      "switch",
    );
    expect(screen.getByTestId("cfg-field-max_slots").getAttribute("type")).toBe(
      "number",
    );
  });
  ...
  it("submits coerced values on save", async () => {
    ...
    // Change the integer
    fireEvent.change(screen.getByTestId("cfg-field-max_slots"), {
      target: { value: "7" },
    });
    // Change the text
    fireEvent.change(screen.getByTestId("cfg-field-label"), {
      target: { value: "other" },
    });
  ...
  it("blocks submit and shows inline errors when a required field is missing", async () => {
    ...
    expect(
      screen.getByTestId("cfg-field-mode").getAttribute("aria-invalid"),
    ).toBe("true");
    expect(
  ...
  it("respects busy state by disabling controls and showing saving copy", () => {
    ...
    expect(screen.getByTestId("config-editor-save").textContent).toMatch(
      /Saving/i,
    );
    expect(screen.getByTestId("cfg-field-max_slots").disabled).toBe(true);
    expect(screen.getByTestId("cfg-field-active").disabled).toBe(true);
  });
