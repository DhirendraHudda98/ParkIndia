...
export function FloatingDock({ unreadCount, isAdmin }) {
  ...
  const pinned = PINNED_KEYS.map((k) =>
    allItems.find((i) => i.key === k),
  ).filter((i) => !!i);
  ...
  return (
    <>
      ...
      >
        ...
        <div
          aria-hidden="true"
          className="w-px h-8 mx-1 bg-surface-200 dark:bg-surface-700"
        />
 ...
 */
function DockIcon({ item, active, badge, mouseX, label }) {
