...
vi.mock('@phosphor-icons/react', () => ({
    ...
    XCircle: (props) => <span data-testid="icon-x" {...props}/>,
}));
const mockToastError = vi.fn();
vi.mock('react-hot-toast', () => ({
    default: { success: vi.fn(), error: (...a) => mockToastError(...a) },
}));
...
describe('AdminCompliancePage', () => {
    beforeEach(() => {
        global.fetch = vi.fn(() => Promise.resolve({ json: () => Promise.resolve({ success: true, data: sampleReport }) }));
    });
    afterEach(() => {
        vi.restoreAllMocks();
    });
    ...
    it('download data map error', async () => {
        global.fetch = vi.fn((url) => {
            ...
            return Promise.resolve({ json: () => Promise.resolve({ success: true, data: sampleReport }) });
        });
        render(<AdminCompliancePage />);
        await waitFor(() => fireEvent.click(screen.getByTestId('download-datamap')));
    ...
    it('returns null when no report', async () => {
        global.fetch = vi.fn(() => Promise.resolve({ json: () => Promise.resolve({ success: false, data: null }) }));
    ...
    it('shows non_compliant check with icon', async () => {
        ...
        global.fetch = vi.fn(() => Promise.resolve({ json: () => Promise.resolve({ success: true, data: reportWithNonCompliant }) }));
        render(<AdminCompliancePage />);
