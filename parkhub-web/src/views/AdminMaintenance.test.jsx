...
const windows = [
    { id: 'w1', lot_id: 'l1', lot_name: 'Lot A', start_time: futureStart, end_time: futureEnd, reason: 'Repaint', affected_slots: { type: 'all' }, created_at: '2026-01-01' },
    { id: 'w2', lot_id: 'l2', lot_name: 'Lot B', start_time: pastStart, end_time: pastEnd, reason: 'Fix lights', affected_slots: { type: 'specific', slot_ids: ['s1', 's2'] }, created_at: '2026-01-01' },
    { id: 'w3', lot_id: 'l1', lot_name: 'Lot A', start_time: activeStart, end_time: activeEnd, reason: 'Active fix', affected_slots: { type: 'all' }, created_at: '2026-01-01' },
];
...
describe('AdminMaintenancePage', () => {
    beforeEach(() => {
        ...
        globalThis.fetch = vi.fn((url, opts) => {
            ...
            if (url.includes('/maintenance/active'))
                return Promise.resolve({ json: () => Promise.resolve({ success: true, data: [{ id: 'w3' }] }) });
            if (url.includes('/lots'))
                return Promise.resolve({ json: () => Promise.resolve({ success: true, data: lots }) });
            return Promise.resolve({ json: () => Promise.resolve({ success: true, data: windows }) });
        });
    });
    ...
    it('handles create API error', async () => {
        globalThis.fetch = vi.fn((url, opts) => {
            if (opts?.method === 'POST')
                return Promise.resolve({ json: () => Promise.resolve({ success: false, error: { message: 'Overlap' } }) });
            if (url.includes('/maintenance/active'))
                return Promise.resolve({ json: () => Promise.resolve({ success: true, data: [] }) });
            if (url.includes('/lots'))
                return Promise.resolve({ json: () => Promise.resolve({ success: true, data: lots }) });
    ...
    it('handles delete failure', async () => {
        globalThis.fetch = vi.fn((url, opts) => {
            if (opts?.method === 'DELETE')
                return Promise.resolve({ json: () => Promise.resolve({ success: false, error: { message: 'Cannot delete' } }) });
            if (url.includes('/maintenance/active'))
                return Promise.resolve({ json: () => Promise.resolve({ success: true, data: [] }) });
            if (url.includes('/lots'))
                return Promise.resolve({ json: () => Promise.resolve({ success: true, data: lots }) });
