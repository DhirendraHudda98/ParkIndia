...
vi.mock('framer-motion', () => ({
    motion: { div: React.forwardRef(({ children, layout, ...p }, r) => <div ref={r} {...p}>{children}</div>) },
...
describe('AdminRolesPage', () => {
    beforeEach(() => {
        ...
        globalThis.fetch = vi.fn((url, opts) => {
            ...
            if (opts?.method === 'POST' || opts?.method === 'PUT')
                return Promise.resolve({ json: () => Promise.resolve({ success: true }) });
            return Promise.resolve({ json: () => Promise.resolve({ success: true, data: roles }) });
        });
    ...
    it('handles save error', async () => {
        globalThis.fetch = vi.fn((url, opts) => {
            if (opts?.method === 'POST')
                return Promise.resolve({ json: () => Promise.resolve({ success: false, error: { message: 'Duplicate' } }) });
            return Promise.resolve({ json: () => Promise.resolve({ success: true, data: roles }) });
        });
        const user = userEvent.setup();
        render(<AdminRolesPage />);
    ...
    it('handles delete error response', async () => {
        globalThis.fetch = vi.fn((url, opts) => {
            if (opts?.method === 'DELETE')
                return Promise.resolve({ json: () => Promise.resolve({ success: false, error: { message: 'In use' } }) });
            return Promise.resolve({ json: () => Promise.resolve({ success: true, data: roles }) });
        });
        render(<AdminRolesPage />);
