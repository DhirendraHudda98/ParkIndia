...
vi.mock("react-i18next", () => ({
  useTranslation: () => ({ t: (_, f) => f }),
}));

const { mockGetInMemoryToken } = vi.hoisted(() => ({
  mockGetInMemoryToken: vi.fn(() => null),
}));
vi.mock("../api/client", () => ({
...
describe("ExportButton", () => {
  it("renders toggle", () => {
    render(<ExportButton />);
    expect(screen.getByTestId("export-toggle")).toBeDefined();
  });
  ...
  it("closes on second click", () => {
    render(<ExportButton />);
    const b = screen.getByTestId("export-toggle");
    fireEvent.click(b);
    expect(screen.getByTestId("export-dropdown")).toBeDefined();
    fireEvent.click(b);
    expect(screen.queryByTestId("export-dropdown")).toBeNull();
  ...
  it("date defaults", () => {
    render(<ExportButton />);
    fireEvent.click(screen.getByTestId("export-toggle"));
    const to = screen.getByTestId("export-to");
    expect(to.value).toBe(new Date().toISOString().slice(0, 10));
  });
  it("date change", () => {
    render(<ExportButton />);
    fireEvent.click(screen.getByTestId("export-toggle"));
    const f = screen.getByTestId("export-from");
    fireEvent.change(f, { target: { value: "2026-01-15" } });
    expect(f.value).toBe("2026-01-15");
  });
});
...
describe("ExportButton download", () => {
  it("fetches with auth", async () => {
    ...
    vi.stubGlobal("URL", {
      ...URL,
      createObjectURL: vi.fn().mockReturnValue("b:x"),
      revokeObjectURL: vi.fn(),
    });
    render(<ExportButton />);
    fireEvent.click(screen.getByTestId("export-toggle"));
    fireEvent.click(screen.getByTestId("export-bookings"));
  ...
  it("correct URL per type", async () => {
    ...
    for (const t of ["bookings", "users", "revenue"]) {
      ...
      fireEvent.click(screen.getByTestId("export-toggle"));
      fireEvent.click(screen.getByTestId(`export-${t}`));
      await waitFor(() => expect(fm).toHaveBeenCalledTimes(1));
      expect(fm.mock.calls[0][0]).toContain(`/api/v1/admin/export/${t}`);
      unmount();
    }
  });
  it("handles error", async () => {
    ...
    render(<ExportButton />);
    fireEvent.click(screen.getByTestId("export-toggle"));
    fireEvent.click(screen.getByTestId("export-users"));
  ...
  it("no token still sends CSRF header and credentials", async () => {
    ...
    await waitFor(() => expect(fm).toHaveBeenCalledTimes(1));
    expect(fm.mock.calls[0][1].headers["X-Requested-With"]).toBe(
      "XMLHttpRequest",
    );
  ...
  it("custom baseUrl", async () => {
    ...
    render(<ExportButton baseUrl="https://api.test.com" />);
    fireEvent.click(screen.getByTestId("export-toggle"));
    fireEvent.click(screen.getByTestId("export-bookings"));
    await waitFor(() => expect(fm).toHaveBeenCalledTimes(1));
    expect(fm.mock.calls[0][0]).toMatch(/^https:\/\/api\.test\.com\//);
  });
});
