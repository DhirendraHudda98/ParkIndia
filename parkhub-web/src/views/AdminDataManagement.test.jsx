...
describe('AdminDataManagementPage', () => {
    ...
    it('handles JSON file import', async () => {
        ...
        fireEvent.click(screen.getByTestId('import-btn'));
        await waitFor(() => {
            expect(global.fetch).toHaveBeenCalledWith('/api/v1/admin/import/users', expect.objectContaining({
    ...
    it('exports with date range params', () => {
        ...
        if (btn)
            fireEvent.click(btn);
        expect(openSpy).toHaveBeenCalledWith(expect.stringContaining('from=2026-01-01'), '_blank');
        openSpy.mockRestore();
    });
    it('exports lots', () => {
        const openSpy = vi.spyOn(window, 'open').mockImplementation(() => null);
    ...
    it('imports lots type using correct endpoint', async () => {
        ...
        fireEvent.click(screen.getByTestId('import-btn'));
        await waitFor(() => {
            expect(global.fetch).toHaveBeenCalledWith('/api/v1/admin/import/lots', expect.anything());
        });
    });
