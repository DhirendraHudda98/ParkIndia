...
export const IconRegistry = {
    render: () => (<div style={{
      ...
      {ALL_ICONS.map((name) => (<div key={name} style={{
          ...
          <span className="v5-mono" style={{ fontSize: 9, color: 'var(--v5-mut)', letterSpacing: 0.6 }}>
            {name}
          </span>
        </div>))}
    </div>),
...
export const Sizes = {
    render: () => (<div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
      {[12, 16, 20, 24, 32, 40].map((size) => (<V5NamedIcon key={size} name="gear" size={size}/>))}
    </div>),
};
