...
vi.mock('../api/client', () => ({
    api: {
        ...
        createLot: (...args) => mockCreateLot(...args),
        updateLot: (...args) => mockUpdateLot(...args),
        deleteLot: (...args) => mockDeleteLot(...args),
        getAdminDynamicPricing: (...args) => mockGetAdminDynamicPricing(...args),
        getLotHours: (...args) => mockGetLotHours(...args),
        updateAdminDynamicPricing: (...args) => mockUpdateAdminDynamicPricing(...args),
        updateAdminLotHours: (...args) => mockUpdateAdminLotHours(...args),
    },
}));
vi.mock('framer-motion', () => ({
    motion: {
        div: React.forwardRef(({ children, initial, animate, exit, transition, ...props }, ref) => (<div ref={ref} {...props}>{children}</div>)),
        tr: React.forwardRef(({ children, initial, animate, exit, transition, ...props }, ref) => (<tr ref={ref} {...props}>{children}</tr>)),
    },
...
vi.mock('@phosphor-icons/react', () => ({
    ...
    CurrencyEur: (props) => <span data-testid="icon-currency" {...props}/>,
    TrendUp: (props) => <span data-testid="icon-trend-up" {...props}/>,
    TrendDown: (props) => <span data-testid="icon-trend-down" {...props}/>,
...
vi.mock('../components/ui/ConfirmDialog', () => ({
    ConfirmDialog: ({ open, onConfirm, onCancel, title, message }) => open ? (<div data-testid="confirm-dialog">
        <p>{title}</p>
        <p>{message}</p>
...
describe('AdminLotsPage', () => {
    ...
    it('total_slots input defaults to 10', async () => {
        ...
        await waitFor(() => expect(screen.getByText('New Lot')).toBeInTheDocument());
        await user.click(screen.getByText('New Lot'));
        const slotsInput = screen.getByLabelText('Total Slots *');
    ...
    it('updates dynamic pricing fields via onChange handlers', async () => {
        ...
        const basePriceInput = document.getElementById('dp-base-price');
        fireEvent.change(basePriceInput, { target: { value: '3.5' } });
        // Surge multiplier (label includes desc text — find by id)
        const surgeMultInput = document.getElementById('dp-surge-mult');
        expect(surgeMultInput).toBeTruthy();
        fireEvent.change(surgeMultInput, { target: { value: '2.0' } });
        const discountMultInput = document.getElementById('dp-discount-mult');
        fireEvent.change(discountMultInput, { target: { value: '0.5' } });
        const surgeThreshInput = document.getElementById('dp-surge-thresh');
        fireEvent.change(surgeThreshInput, { target: { value: '90' } });
        const discountThreshInput = document.getElementById('dp-discount-thresh');
