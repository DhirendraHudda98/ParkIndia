...
describe('AdminUpdatesPage', () => {
    ...
    it('toggles auto-update and calls settings API', async () => {
        ...
        await waitFor(() => {
            const settingsCalls = fetchMock.mock.calls.filter(([url, opts]) => url.includes('/admin/settings') && opts?.method === 'POST');
    ...
    it('switches channel to beta', async () => {
        ...
        await waitFor(() => {
            const settingsCalls = fetchMock.mock.calls.filter(([url, opts]) => url.includes('/admin/settings') && opts?.method === 'POST');
            const betaCall = settingsCalls.find(([_, opts]) => {
    ...
    it('handles the flat system version response shape', async () => {
        global.fetch = mockFetch({
            version: { name: 'parkhub-php', version: '5.0.8' },
        });
        render(<AdminUpdatesPage />);
        await waitFor(() => {
