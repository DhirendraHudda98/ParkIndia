...
describe("EmptyState", () => {
  ...
  it("renders description when provided", () => {
    ...
    render(
      <EmptyState
        variant="no-bookings"
        title="No bookings"
        description="Book a spot to get started"
      />,
    );
  ...
  it("renders fallback icon when rich_empty_states is disabled", () => {
    mockIsEnabled.mockReturnValue(false);
    const FakeIcon = (props) => <span data-testid="fallback-icon" {...props} />;
    render(
      <EmptyState variant="no-bookings" title="No bookings" icon={FakeIcon} />,
    );
  ...
  it("renders different illustration for no-vehicles variant", () => {
    mockIsEnabled.mockReturnValue(true);
    const { container } = render(
      <EmptyState variant="no-vehicles" title="No vehicles" />,
    );
    const svg = container.querySelector("svg");
    expect(svg).toBeInTheDocument();
  });
  ...
  it("renders different illustration for no-transactions variant", () => {
    mockIsEnabled.mockReturnValue(true);
    const { container } = render(
      <EmptyState variant="no-transactions" title="No transactions" />,
    );
    const svg = container.querySelector("svg");
    expect(svg).toBeInTheDocument();
  });
