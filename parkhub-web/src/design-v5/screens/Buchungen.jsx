...
export function BuchungenV5({ navigate }) {
    ...
    const { data: bookings = [], isLoading, isError } = useQuery({
        ...
        queryFn: async () => {
            ...
            if (!res.success)
                throw new Error(res.error?.message ?? 'Bookings could not be loaded');
            return res.data ?? [];
        },
        staleTime: 30_000,
    ...
    const cancelMutation = useMutation({
        mutationFn: async (id) => {
            ...
            if (!res.success)
                throw new Error(res.error?.message ?? 'Cancellation failed');
            return res.data;
        },
        onMutate: async (id) => {
            ...
            if (previous) {
                qc.setQueryData(['buchungen'], previous.map((b) => (b.id === id ? { ...b, status: 'cancelled' } : b)));
            }
            return { previous };
        },
    ...
    if (isError) {
        ...
        <Card className="v5-ani" style={{ padding: 28, textAlign: 'center', maxWidth: 360 }}>
          ...
          <div style={{ fontSize: 12, color: 'var(--v5-mut)', marginTop: 4 }}>Bookings could not be loaded.</div>
        </Card>
      </div>);
    }
    return (<div style={{ padding: 16, flex: 1, overflow: 'auto', display: 'flex', flexDirection: 'column', gap: 12 }}>
      <div className="v5-ani" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <span style={{ fontWeight: 700, fontSize: 15, color: 'var(--v5-txt)' }}>{t('bookings.title')}</span>
          <Badge variant="gray"><NumberFlow value={filtered.length}/></Badge>
        </div>
        <button type="button" onClick={() => navigate('buchen')} className="v5-btn" style={{ padding: '7px 14px', borderRadius: 9, background: 'var(--v5-acc)', color: 'var(--v5-accent-fg)', border: 'none', fontSize: 11, fontWeight: 600, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 5 }}>
          <V5NamedIcon name="plus" size={12}/>
          {t('bookings.book_spot')}
        </button>
      </div>

      <div className="v5-ani" role="group" aria-label="Status Filter" style={{ display: 'flex', gap: 6, flexWrap: 'wrap', animationDelay: '0.06s' }}>
        {FILTERS.map((f) => {
            const active = filter === f.key;
            return (<button key={f.key} type="button" aria-pressed={active} onClick={() => setFilter(f.key)} style={{
                    padding: '5px 12px',
                    borderRadius: 999,
                    fontSize: 11,
                    fontWeight: 500,
      ...
      {filtered.length === 0 ? (<Card className="v5-ani" style={{ padding: 36, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 10, animationDelay: '0.12s' }}>
          ...
          <div style={{ textAlign: 'center' }}>
            <div style={{ fontWeight: 600, color: 'var(--v5-txt)', fontSize: 13 }}>{t('bookings.none_found')}</div>
            <div style={{ fontSize: 11, color: 'var(--v5-mut)', marginTop: 3 }}>
              {filter === 'alle' ? t('bookings.reserve_now') : `No bookings with status "${FILTERS.find((f) => f.key === filter)?.label}".`}
            </div>
          </div>
          {filter === 'alle' && (<button type="button" onClick={() => navigate('buchen')} className="v5-btn" style={{ padding: '8px 16px', borderRadius: 9, background: 'var(--v5-acc)', color: 'var(--v5-accent-fg)', border: 'none', fontSize: 11, fontWeight: 600, cursor: 'pointer' }}>
              + {t('bookings.book_spot')}
            </button>)}
          ...
          <div className="v5-mono" style={{
            ...
            <span>{t('bookings.id')}</span>
            <span>{t('bookings.lot')}</span>
            <span>{t('bookings.plate')}</span>
            <span>{t('bookings.date_time')}</span>
          ...
          {filtered.map((b, i) => {
                ...
                return (<div key={b.id} data-testid="buchungen-row" className="v5-row" style={{
                        display: 'grid',
                        gridTemplateColumns: '90px 1fr 100px 150px 90px 110px 100px',
                        padding: '10px 16px',
                ...
                <div>
                  ...
                  <div style={{ fontSize: 10, color: 'var(--v5-mut)' }}>Spot {b.slot_number ?? '?'}</div>
                </div>
                ...
                <div style={{ display: 'flex', gap: 5, alignItems: 'center' }}>
                  {isActive && (<button type="button" onClick={() => {
                            navigate('einchecken');
                            toast('Check-in opened', 'info');
                  ...
                  {isCancellable && (<button type="button" disabled={isCancellingThis} aria-label={`Cancel booking ${b.id}`} onClick={() => cancelMutation.mutate(b.id)} style={{ padding: '3px 8px', borderRadius: 6, background: 'color-mix(in oklch, var(--v5-err) 8%, transparent)', border: 'none', fontSize: 10, color: 'var(--v5-err)', cursor: isCancellingThis ? 'default' : 'pointer', opacity: isCancellingThis ? 0.5 : 1, fontWeight: 500 }}>
                      {isCancellingThis ? '…' : t('bookings.cancel')}
                    </button>)}
                </div>
              </div>);
            })}
