...
export function IntegrationsV5({ navigate: _navigate }) {
    ...
    const { data: items = [], isLoading, isError } = useQuery({
        ...
        queryFn: async () => {
            ...
            if (!res.success)
                throw new Error(res.error?.message ?? 'Integrationen konnten nicht geladen werden');
            return res.data ?? [];
        },
        staleTime: 30_000,
    });
    const connect = useMutation({
        mutationFn: async (id) => {
            ...
            if (!res.success)
                throw new Error(res.error?.message ?? 'Verbindung fehlgeschlagen');
            return res.data;
        },
        onSuccess: () => {
        ...
        onError: (err) => toast(err.message || 'Verbindung fehlgeschlagen', 'error'),
    });
    const disconnect = useMutation({
        mutationFn: async (id) => {
            const res = await api.disconnectIntegration(id);
            if (!res.success)
                throw new Error(res.error?.message ?? 'Trennen fehlgeschlagen');
            return res.data;
        },
        onSuccess: () => {
            qc.invalidateQueries({ queryKey: ['integrations'] });
        ...
        onError: (err) => toast(err.message || 'Trennen fehlgeschlagen', 'error'),
    });
    const connectedCount = items.filter((i) => i.connected).length;
    if (isLoading) {
        return (<div style={{ padding: 16, flex: 1, overflow: 'auto' }}>
        {[0, 1, 2].map((i) => (<div key={i} style={{ height: 80, borderRadius: 12, background: 'var(--v5-sur2)', marginBottom: 8, animation: 'ph-v5-pulse 1.6s ease infinite', animationDelay: `${i * 0.1}s` }}/>))}
      </div>);
    }
    if (isError) {
    ...
    return (<div style={{ padding: 16, flex: 1, overflow: 'auto', display: 'flex', flexDirection: 'column', gap: 12 }}>
      ...
      {items.length === 0 ? (<Card className="v5-ani" style={{ padding: 36, textAlign: 'center' }}>
        ...
        </Card>) : (<div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 10 }}>
          {items.map((item, idx) => {
                const busy = (connect.isPending && connect.variables === item.id) ||
                    (disconnect.isPending && disconnect.variables === item.id);
                return (<Card key={item.id} data-testid="integrations-card" className="v5-ani" style={{ padding: 16, display: 'flex', flexDirection: 'column', gap: 10, animationDelay: `${idx * 0.04}s` }}>
                ...
                {item.connected && item.account_label && (<div style={{ fontSize: 10, color: 'var(--v5-mut)' }}>
                    Konto: <span style={{ color: 'var(--v5-txt)' }}>{item.account_label}</span>
                    {' · '}seit {formatWhen(item.connected_at)}
                  </div>)}
                <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
                  {item.connected ? (<button type="button" disabled={busy} onClick={() => disconnect.mutate(item.id)} data-testid="integrations-disconnect" style={{
                        ...
                        }}>{busy ? '…' : 'Trennen'}</button>) : (<button type="button" disabled={busy} onClick={() => connect.mutate(item.id)} data-testid="integrations-connect" style={{
                            padding: '6px 14px', borderRadius: 8,
