...
function FeaturesConsumer() {
  ...
  return (
    <div>
      ...
      <button
        data-testid="enable-invoices"
        onClick={() => setFeature("invoices", true)}
      >
        Enable Invoices
      </button>
      <button
        data-testid="disable-credits"
        onClick={() => setFeature("credits", false)}
      >
        Disable Credits
      </button>
      <button
      ...
      <button
        data-testid="apply-residential"
        onClick={() => applyPreset("residential")}
      >
        Apply Residential
      </button>
      <button
        data-testid="set-minimal"
        onClick={() => setFeatures(["vehicles"])}
      >
        Set Minimal
      </button>
    </div>
...
describe("FeaturesContext", () => {
  ...
  it("reads features from localStorage", async () => {
    localStorageMock.setItem(
      "parkhub_features",
      JSON.stringify(["vehicles", "credits"]),
    );
...
describe("USE_CASE_PRESETS", () => {
  ...
  it("business preset is the most feature-rich", () => {
    expect(USE_CASE_PRESETS.business.length).toBeGreaterThanOrEqual(
      USE_CASE_PRESETS.residential.length,
    );
    expect(USE_CASE_PRESETS.business.length).toBeGreaterThanOrEqual(
      USE_CASE_PRESETS.personal.length,
    );
  });
