          ...
          <div className="grid gap-6 lg:grid-cols-[1.15fr_0.85fr]">
            <div>
              <div className={`mb-3 inline-flex items-center gap-2 rounded-full px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.2em] ${
                ...
                {isVoid ? 'Void history ledger' : isIndia ? 'India Parking Ledger' : 'Marble history ledger'}
              </div>
            ...
            <div className={`rounded-[24px] border p-5 ${
              isVoid
                ...
                : isIndia
                ? 'border-[#FF9933]/20 bg-white/80 dark:bg-white/[0.04]'
          ...
          <motion.div variants={fadeUp} className="grid grid-cols-1 gap-4 md:grid-cols-[1.1fr_0.9fr]">
            ...
            <div className={`rounded-[24px] border p-5 ${
              isVoid
                ...
                : isIndia
                ? 'border-[#FF9933]/10 bg-white/80 dark:bg-surface-950/80'
              ...
              <div className="flex h-32 items-center justify-center rounded-[20px] border border-dashed border-surface-200 dark:border-surface-800">
                <div className="text-center">
                  <p className={`text-4xl font-bold ${isIndia ? 'text-[#000080] dark:text-[#FF9933]' : 'text-primary-600 dark:text-primary-400'}`}>
              ...
              <div className="mt-4 grid gap-3 sm:grid-cols-2">
                <PanelMetric label="Completed" value={String(completedBookings)} helper="Current page" isVoid={isVoid} isIndia={isIndia} compact />
                <PanelMetric label="Cancelled" value={String(cancelledBookings)} helper="Current page" isVoid={isVoid} isIndia={isIndia} compact />
              </div>
            </div>
          </motion.div>
        )}
                ...
                >
                  ...
                  {booking.total_price != null && (
                    <div className="flex-shrink-0 text-right">
                      <p className="font-semibold text-surface-900 dark:text-white">{booking.total_price} {booking.currency || 'INR'}</p>
                    </div>
                  )}
                </div>
              ))}
  ...
  return (
    <div className={`rounded-[22px] border px-4 py-4 ${
      ...
      <div className="flex items-start justify-between gap-3">
        ...
        <div className={`rounded-2xl p-2 ${isVoid ? 'bg-white/[0.06] text-cyan-100' : isIndia ? 'bg-white text-[#FF9933] dark:bg-white/10' : 'bg-white text-emerald-700 dark:bg-white/10 dark:text-emerald-300'}`}>
          {icon}
        </div>
      </div>
    </div>
