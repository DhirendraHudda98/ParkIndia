...
test.describe("Mobile Booking Flow", () => {
  ...
  test("book page loads on mobile without horizontal scroll", async ({
    page,
  }) => {
  ...
  test("mobile book page shows nearby lots or empty state", async ({
    page,
  }) => {
    await page.goto("/book");
    await page.waitForLoadState("networkidle");
    // Either lot cards or a no-lots message
    const lotOrEmpty = page.getByText(
      /available|No .*(lots|parking)|Select a Lot|Choose/i,
    );
    await expect(lotOrEmpty.first()).toBeVisible({ timeout: 30_000 });
  });

  test("mobile dashboard shows active bookings section without horizontal scroll", async ({
    page,
  }) => {
  ...
  test("quick book action is accessible on mobile dashboard", async ({
    page,
  }) => {
  ...
  test("bookings list page loads on mobile without horizontal scroll", async ({
    page,
  }) => {
