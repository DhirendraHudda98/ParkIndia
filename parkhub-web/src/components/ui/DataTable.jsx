    ...
    const dataRows = rows.map((row) =>
      visibleColumns
        .map((h) => {
          const cell = row
            .getAllCells()
            .find((c) => c.column.id === h.column.id);
                  ...
                  return (
                    <th
                      ...
                      className={`text-left px-5 py-3.5 text-xs font-semibold text-surface-500 dark:text-surface-400 uppercase tracking-wider ${
                        canSort
                          ? "cursor-pointer select-none hover:text-surface-700 dark:hover:text-surface-200 transition-colors"
                          : ""
                      }`}
                      onClick={
                        canSort
                    ...
                    >
                      <span className="inline-flex items-center gap-1">
                        ...
                        {canSort && (
                          <span
                            className="inline-flex flex-col -space-y-1 ml-0.5"
                            aria-hidden="true"
                          >
                            <CaretUp
                              weight="bold"
                              className={`w-3 h-3 ${sorted === "asc" ? "text-primary-600" : "text-surface-300 dark:text-surface-600"}`}
                            />
                            <CaretDown
                              weight="bold"
                              className={`w-3 h-3 ${sorted === "desc" ? "text-primary-600" : "text-surface-300 dark:text-surface-600"}`}
                            />
                          </span>
                        )}
                      </span>
