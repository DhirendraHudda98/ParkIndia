...
function AuthConsumer() {
  ...
  return (
    <div>
      ...
      <button data-testid="login-btn" onClick={() => login("admin", "demo")}>
        Login
      </button>
...
describe("AuthContext", () => {
  ...
  it("refreshUser updates user from API", async () => {
    ...
    await waitFor(() =>
      expect(screen.getByTestId("cur").textContent).toBe("alice"),
    );
    mockMe.mockResolvedValueOnce({
      success: true,
      data: {
    ...
    await waitFor(() =>
      expect(screen.getByTestId("cur").textContent).toBe("alice2"),
    );
  });
