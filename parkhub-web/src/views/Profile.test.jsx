...
vi.mock('../api/client', () => ({
    api: {
        ...
        setAccessibilityNeeds: (...args) => mockSetAccessibilityNeeds(...args),
    },
}));
...
describe('ProfilePage', () => {
    ...
    it('exports data on click', async () => {
        ...
        mockApi.api.exportMyData = vi.fn().mockResolvedValue(mockBlob);
    ...
    it('export data failure shows error toast', async () => {
        const mockApi = await import('../api/client');
        mockApi.api.exportMyData = vi.fn().mockRejectedValue(new Error('Export failed'));
        const user = userEvent.setup();
        render(<ProfilePage />);
        await user.click(screen.getByText('Daten exportieren'));
        await waitFor(() => {
    ...
    it('delete account action calls API and logs out on success', async () => {
        const mockApi = await import('../api/client');
        mockApi.api.deleteMyAccount = vi.fn().mockResolvedValue({ success: true });
        const user = userEvent.setup();
        render(<ProfilePage />);
        const deleteButtons = screen.getAllByText('Konto löschen');
        const deleteBtn = deleteButtons.find(el => el.closest('button'));
    ...
    it('delete account failure shows error toast', async () => {
        const mockApi = await import('../api/client');
        mockApi.api.deleteMyAccount = vi.fn().mockResolvedValue({ success: false, error: { message: 'Cannot delete' } });
        const user = userEvent.setup();
        render(<ProfilePage />);
        const deleteButtons = screen.getAllByText('Konto löschen');
        const deleteBtn = deleteButtons.find(el => el.closest('button'));
    ...
    it('delete account exception shows error toast', async () => {
        const mockApi = await import('../api/client');
        mockApi.api.deleteMyAccount = vi.fn().mockRejectedValue(new Error('Network'));
        const user = userEvent.setup();
        render(<ProfilePage />);
        const deleteButtons = screen.getAllByText('Konto löschen');
        const deleteBtn = deleteButtons.find(el => el.closest('button'));
