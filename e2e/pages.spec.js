...
test.describe("Pages — Protected Routes (after login)", () => {
  ...
  test("dashboard shows stats or content", async ({ page }) => {
    ...
    await expect(page.locator("main, [data-testid], h1, h2")).not.toHaveCount(
      0,
    );
  });
  ...
  test("/profile page has settings section", async ({ page }) => {
    ...
    await expect(page.locator("body")).toContainText(
      /profile|settings|theme|account/i,
    );
  });
});
  ...
  for (const route of ADMIN_ROUTES) {
    test(`${route} loads for admin`, async ({ page }) => {
      ...
      await expect(page.locator("body")).not.toContainText(
        /returned an object instead of string/i,
      );
    });
  }

  test("admin modules, chargers, and updates settle without broken UI chrome", async ({
    page,
  }) => {
    for (const route of [
      "/admin/modules",
    ...
    await expect(page.locator("body")).not.toContainText(
      /Could not load charger statistics/i,
    );
