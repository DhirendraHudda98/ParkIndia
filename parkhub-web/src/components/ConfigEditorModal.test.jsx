...
vi.mock("../api/client", () => ({
  api: {
    getModuleConfig: (...args) => getModuleConfigMock(...args),
    patchModuleConfig: (...args) => patchModuleConfigMock(...args),
  },
}));
...
const SAMPLE = {
  schema: {
    type: "object",
    properties: {
      mode: { type: "string", enum: ["basic", "pro"] },
      active: { type: "boolean" },
    },
    required: ["mode"],
  },
...
describe("ConfigEditorModal", () => {
  ...
  it("fetches and hydrates the form when opened", async () => {
    ...
    const onClose = vi.fn();
    render(
      <ConfigEditorModal moduleName="sample" isOpen={true} onClose={onClose} />,
    );
    ...
    expect(screen.getByTestId("cfg-field-mode").value).toBe("basic");
  });
  ...
  it("shows a load-error state when the fetch fails", async () => {
    ...
    render(
      <ConfigEditorModal moduleName="sample" isOpen={true} onClose={vi.fn()} />,
    );
    await waitFor(() => {
  ...
  it("saves values, closes, and shows a success toast on 200", async () => {
    ...
    fireEvent.change(screen.getByTestId("cfg-field-mode"), {
      target: { value: "pro" },
    });
    ...
    expect(patchModuleConfigMock.mock.calls[0][1]).toEqual({
      mode: "pro",
      active: true,
    });
