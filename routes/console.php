...
// Cancel unconfirmed/abandoned bookings
Schedule::command('bookings:release-unconfirmed')->everyFiveMinutes();

// ── ParkIndia: Refresh real-time parking availability every 5 minutes ─────────
// Calls ParkingAvailabilityService::refreshAll() — updates available_spots in DB
// and busts the availability cache. Runs in IST timezone.
Schedule::command('parking:check-availability', ['--quiet-log' => true])
    ->everyFiveMinutes()
    ->timezone('Asia/Kolkata')
    ->withoutOverlapping()
    ->runInBackground()
    ->name('parkindia:availability-refresh');
