    ...
    beforeEach(() => {
        ...
        global.fetch = vi.fn((url) => {
            if (url.includes('/by-cost-center')) {
                return Promise.resolve({ json: () => Promise.resolve({ success: true, data: sampleCcData }) });
            }
            if (url.includes('/by-department')) {
                return Promise.resolve({ json: () => Promise.resolve({ success: true, data: sampleDeptData }) });
            }
            if (url.includes('/export')) {
                return Promise.resolve({ blob: () => Promise.resolve(new Blob(['csv'])) });
            }
