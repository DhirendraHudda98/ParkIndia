...
export function CreditsV5({ navigate: _navigate }) {
    ...
    const { data: credits, isLoading, isError } = useQuery({
        ...
        queryFn: async () => {
            ...
            if (!res.success)
                throw new Error(res.error?.message ?? 'Credits konnten nicht geladen werden');
            return res.data;
        },
        staleTime: 30_000,
    ...
    const buyMutation = useMutation({
        mutationFn: async () => {
            ...
            if (!res.success)
                throw new Error(res.error?.message ?? 'Credits aufladen fehlgeschlagen');
            return res;
        },
        onSuccess: () => {
            toast('10 Credits erfolgreich aufgeladen!', 'success');
            qc.invalidateQueries({ queryKey: ['credits'] });
        },
        onError: () => toast('Credits aufladen fehlgeschlagen', 'error'),
    });
    if (isLoading) {
        return (<div style={{ padding: 16, flex: 1, overflow: 'auto', display: 'flex', flexDirection: 'column', gap: 12 }}>
        {[240, 90, 200].map((h, i) => (<div key={i} style={{ height: h, borderRadius: 14, background: 'var(--v5-sur2)', animation: 'ph-v5-pulse 1.6s ease infinite', animationDelay: `${i * 0.1}s` }}/>))}
      </div>);
    }
    if (isError || !credits) {
    ...
    return (<div style={{ padding: 16, flex: 1, overflow: 'auto', display: 'flex', flexDirection: 'column', gap: 12 }}>
      <Card className="v5-ani" style={{
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 12 }}>
          <div>
            ...
            <div className="v5-mono" style={{ fontSize: 44, fontWeight: 800, color: 'var(--v5-acc)', letterSpacing: -2, lineHeight: 1 }}>
          ...
          <button type="button" disabled={buyMutation.isPending} onClick={() => buyMutation.mutate()} className="v5-btn" style={{
            padding: '9px 18px',
            borderRadius: 10,
        ...
        <div>
          <div style={{ height: 4, background: 'color-mix(in oklch, var(--v5-acc) 18%, transparent)', borderRadius: 4 }} aria-label={`${balance} von ${quota} Credits`} role="meter" aria-valuenow={balance} aria-valuemin={0} aria-valuemax={Math.max(quota, balance)}>
      ...
      <div className="v5-ani" style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 10, animationDelay: '0.06s' }}>
        ...
        <StatCard label="Letzte Aufladung" value={credits.last_refilled ? formatDate(credits.last_refilled) : '—'} icon="cal" delay={0.12}/>
      </div>
      ...
      <Card className="v5-ani" style={{ overflow: 'hidden', animationDelay: '0.12s' }}>
        ...
        {txs.length === 0 ? (<div style={{ padding: '28px 18px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8 }}>
            ...
            return (<Row key={tx.id} label={label} sub={tx.description ?? formatDate(tx.created_at)} last={i === txs.length - 1}>
                <div style={{ textAlign: 'right' }}>
                  <div className="v5-mono" style={{ fontSize: 13, fontWeight: 700, color: positive ? 'var(--v5-ok)' : 'var(--v5-err)' }}>
                    {positive ? '+' : ''}{tx.amount}
                  </div>
                </div>
              </Row>);
