...
vi.mock('framer-motion', () => ({
    motion: {
        ...
        tr: React.forwardRef(({ children, initial, animate, transition, ...props }, ref) => (<tr ref={ref} {...props}>{children}</tr>)),
    },
...
describe('AdminAccessiblePage', () => {
    beforeEach(() => {
        global.fetch = vi.fn((url) => {
            if (url.includes('/accessible-stats')) {
                return Promise.resolve({ json: () => Promise.resolve({ success: true, data: sampleStats }) });
            }
            if (url.includes('/lots/lot-1/slots')) {
                return Promise.resolve({ json: () => Promise.resolve({ success: true, data: sampleSlots }) });
            }
    ...
    it('toggles slot accessibility on click', async () => {
        ...
        await waitFor(() => {
            expect(global.fetch).toHaveBeenCalledWith(expect.stringContaining('/accessible'), expect.objectContaining({ method: 'PUT' }));
        });
    });
    ...
    it('shows error toast when slot toggle returns failure', async () => {
        global.fetch = vi.fn((url, opts) => {
            ...
            if (opts?.method === 'PUT' && url.includes('/accessible')) {
                return Promise.resolve({ json: () => Promise.resolve({ success: false, error: { message: 'Forbidden' } }) });
            }
            return Promise.resolve({ json: () => Promise.resolve({ success: true, data: null }) });
        });
        render(<AdminAccessiblePage />);
