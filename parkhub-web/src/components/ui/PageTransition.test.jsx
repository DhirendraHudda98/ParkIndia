...
vi.mock("framer-motion", () => ({
  motion: {
    div: React.forwardRef(
      (
        { children, initial, animate, exit, transition, variants, ...props },
        ref,
      ) => (
...
describe("PageTransition", () => {
  ...
  it("applies className prop", () => {
    render(<PageTransition className="page-wrapper">content</PageTransition>);
    expect(screen.getByTestId("motion-div")).toHaveClass("page-wrapper");
  });

  it("renders without className (optional)", () => {
    render(<PageTransition>no class</PageTransition>);
