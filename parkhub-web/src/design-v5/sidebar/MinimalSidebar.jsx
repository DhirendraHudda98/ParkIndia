 ...
 */
export function MinimalSidebar({ active, onNavigate, userLabel = 'Administrator' }) {
    const [hover, setHover] = useState(null);
    return (<nav aria-label="Hauptnavigation" data-variant="minimal" style={{
            width: 52,
      ...
      <div style={{
        ...
        <div aria-label="ParkHub" title="ParkHub" style={{
            width: 28,
            height: 28,
            borderRadius: 8,
      ...
      <div style={{
        ...
        {NAV.map((item) => {
            ...
            return (<button key={item.id} type="button" onClick={() => onNavigate(item.id)} onMouseEnter={() => setHover(item.id)} onMouseLeave={() => setHover(null)} onFocus={() => setHover(item.id)} onBlur={() => setHover(null)} aria-current={isActive ? 'page' : undefined} aria-label={item.label} title={item.label} style={{
                    position: 'relative',
                    width: 36,
                    height: 36,
                    borderRadius: 9,
              ...
              <V5NamedIcon name={item.icon} size={15} color={isActive ? 'var(--v5-acc)' : 'var(--v5-mut)'}/>
              ...
              {isHover && (<span role="tooltip" style={{
                        position: 'absolute',
                        left: '100%',
                        marginLeft: 8,
                        top: '50%',
      ...
      <div style={{
        ...
        <div aria-label={userLabel} title={userLabel} style={{
            width: 28,
            height: 28,
            borderRadius: '50%',
