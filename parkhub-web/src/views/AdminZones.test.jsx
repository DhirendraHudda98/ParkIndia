...
vi.mock('framer-motion', () => ({
    motion: {
        div: React.forwardRef(({ children, initial, animate, exit, layout, ...props }, ref) => (<div ref={ref} {...props}>{children}</div>)),
    },
...
vi.mock('@phosphor-icons/react', () => ({
    ...
    Tag: (props) => <span data-testid="icon-tag" {...props}/>,
}));
...
describe('AdminZonesPage', () => {
    ...
    it('saves zone pricing successfully', async () => {
        ...
        globalThis.fetch = vi.fn((url, opts) => {
            if (opts?.method === 'PUT')
                return Promise.resolve(putResp);
            if (typeof url === 'string' && url.includes('/zones/pricing'))
                return Promise.resolve(zonesResp);
    ...
    it('shows error on save pricing failure', async () => {
        ...
        globalThis.fetch = vi.fn((url, opts) => {
            if (opts?.method === 'PUT')
                return Promise.resolve({ ok: true, json: () => Promise.resolve({ success: false, error: { message: 'Invalid' } }) });
            if (typeof url === 'string' && url.includes('/zones/pricing'))
                return Promise.resolve(zonesResp);
            return Promise.resolve(lotsResp);
        });
