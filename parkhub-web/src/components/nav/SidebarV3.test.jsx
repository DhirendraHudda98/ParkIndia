...
const WAIT_FOR_OPTS = { timeout: 15_000 };
...
describe("SidebarV3", () => {
  ...
  it("renders without crashing when all API responses are empty", async () => {
    getBookingsMock.mockReturnValue(ok([]));
    getLotsMock.mockReturnValue(ok([]));
    getLotSlotsMock.mockReturnValue(ok([]));
  ...
  it("renders the Live Pass card for an active booking", async () => {
    ...
    getLotsMock.mockReturnValue(ok([lot]));
    getLotSlotsMock.mockReturnValue(ok([]));
