...
const { mockUser, localStorageMock } = vi.hoisted(() => {
    const mockUser = { current: null, loading: false };
    let store = {};
...
vi.mock('framer-motion', () => ({
    motion: {
        ...
        aside: React.forwardRef(({ children, ...props }, ref) => <aside ref={ref} {...props}>{children}</aside>),
        ...
        li: React.forwardRef(({ children, ...props }, ref) => <li ref={ref} {...props}>{children}</li>),
        ul: React.forwardRef(({ children, ...props }, ref) => <ul ref={ref} {...props}>{children}</ul>),
...
vi.mock('@phosphor-icons/react', () => new Proxy({}, { get: (_, name) => (props) => <span data-testid={`icon-${String(name)}`} {...props}/> }));
...
const stub = (id) => () => <div data-testid={`page-${id}`}>{id}</div>;
