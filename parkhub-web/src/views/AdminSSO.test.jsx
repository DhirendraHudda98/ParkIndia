...
vi.mock('@phosphor-icons/react', () => ({
    ...
    ToggleLeft: (props) => <span data-testid="icon-toggle-off" {...props}/>,
    ToggleRight: (props) => <span data-testid="icon-toggle-on" {...props}/>,
}));
...
describe('AdminSSOPage', () => {
    ...
    it('sends PUT request on save', async () => {
        ...
        await waitFor(() => {
            expect(mockFetch).toHaveBeenCalledWith(expect.stringContaining('/api/v1/admin/sso/'), expect.objectContaining({ method: 'PUT' }));
        });
    });
    ...
    it('deletes provider', async () => {
        globalThis.fetch = vi.fn((url, opts) => {
            ...
            return Promise.resolve({ json: () => Promise.resolve({ success: true, data: sampleProviders }) });
        });
        render(<AdminSSOPage />);
        ...
        await waitFor(() => {
            expect(globalThis.fetch).toHaveBeenCalledWith(expect.stringContaining('/api/v1/admin/sso/okta'), expect.objectContaining({ method: 'DELETE' }));
        });
    });
    ...
    it('save error response', async () => {
        globalThis.fetch = vi.fn((url, opts) => {
            if (opts?.method === 'PUT')
                return Promise.resolve({ json: () => Promise.resolve({ success: false, error: { message: 'Duplicate slug' } }) });
            return Promise.resolve({ json: () => Promise.resolve({ success: true, data: { providers: [] } }) });
        });
        render(<AdminSSOPage />);
        await waitFor(() => fireEvent.click(screen.getByText('Add Provider')));
        const inputs = screen.getAllByRole('textbox');
