...
test.describe("PWA — Offline Resilience & Reconnection", () => {
  test("offline indicator shown when network is lost", async ({
    page,
    context,
  }) => {
    ...
    const hasOfflineText =
      /offline|no.*connection|disconnected|network.*unavailable/i.test(
        pageContent ?? "",
      );
  ...
  test("graceful error when creating booking while offline", async ({
    page,
    context,
  }) => {
    await loginViaUi(page);
    ...
    if (btnCount > 0) {
      ...
      const hasErrorFeedback =
        errorCount > 0 ||
        /error|failed|offline|unavailable/i.test(bodyText ?? "");
  ...
  test("pages still load from cache when offline", async ({
    page,
    context,
  }) => {
