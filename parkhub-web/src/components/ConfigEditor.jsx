...
function validateShape(schema, values) {
  const out = [];
...
export function ConfigEditor({
  ...
  const setField = useCallback((name, prop, raw) => {
  ...
  return (
    <form onSubmit={onSubmit} noValidate className="space-y-4">
      ...
      {Object.entries(schema.properties).map(([name, prop], idx) => {
        ...
        const inputRefProp = idx === 0 ? { ref: firstFieldRef } : {};
