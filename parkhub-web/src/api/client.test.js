...
describe("API client", () => {
  ...
  it("calls bulk update endpoint", async () => {
    globalThis.fetch = vi.fn().mockResolvedValue({
      ...
      json: () =>
        Promise.resolve({
          success: true,
          data: { total: 2, succeeded: 2, failed: 0, errors: [] },
        }),
    });
  ...
  it("calls bulk delete endpoint", async () => {
    globalThis.fetch = vi.fn().mockResolvedValue({
      ...
      json: () =>
        Promise.resolve({
          success: true,
          data: { total: 1, succeeded: 1, failed: 0, errors: [] },
        }),
    });
  ...
  it("calls updateMe endpoint", async () => {
    globalThis.fetch = vi.fn().mockResolvedValue({
      ...
      status: 200,
      json: () =>
        Promise.resolve({ success: true, data: { id: "1", name: "New Name" } }),
    });
  ...
  it("calls getSetupStatus endpoint", async () => {
    globalThis.fetch = vi.fn().mockResolvedValue({
      ...
      json: () =>
        Promise.resolve({
          success: true,
          data: { setup_complete: true, has_admin: true },
        }),
    });
  ...
  it("calls createLot and deleteLot endpoints", async () => {
    globalThis.fetch = vi.fn().mockResolvedValue({
      ...
      json: () =>
        Promise.resolve({
          success: true,
          data: { id: "l1", name: "Test Lot" },
        }),
    });
    await api.createLot({ name: "Test Lot", total_slots: 10 });
    const [url, opts] = firstCall();
    expect(url).toBe("/api/v1/lots");
    expect(opts.method).toBe("POST");
  ...
  it("calls updateLot endpoint", async () => {
    ...
    await api.updateLot("l1", { name: "Updated" });
    const [url, opts] = firstCall();
  ...
  it("calls booking endpoints", async () => {
    ...
    await api.createBooking({ lot_id: "l1", slot_id: "s1" });
    const [, opts] = firstCall();
    expect(opts.method).toBe("POST");
  ...
  it("calls vehicle endpoints", async () => {
    ...
    await api.createVehicle({ plate: "ABC-123" });
  ...
  it("calls admin user management endpoints", async () => {
    ...
    await api.adminUpdateUser("u1", { is_active: false });
  ...
  it("calls announcement CRUD endpoints", async () => {
    ...
    await api.adminUpdateAnnouncement("ann1", {
      title: "T2",
      message: "M2",
      severity: "warning",
      active: false,
  ...
  it("calls getDesignThemePreference and updateDesignThemePreference", async () => {
    globalThis.fetch = vi.fn().mockResolvedValue({
      ...
      status: 200,
      json: () =>
        Promise.resolve({ success: true, data: { design_theme: "dark" } }),
    });
    ...
    globalThis.fetch = vi.fn().mockResolvedValue({
      ...
      status: 200,
      json: () =>
        Promise.resolve({ success: true, data: { design_theme: "light" } }),
    });
  ...
  it("calls getAuditLog with query params", async () => {
    globalThis.fetch = vi.fn().mockResolvedValue({
      ...
      status: 200,
      json: () =>
        Promise.resolve({ success: true, data: { entries: [], total: 0 } }),
    });
  ...
  it("calls tenant endpoints", async () => {
    ...
    await api.updateTenant("t1", { name: "Tenant B" });
    expect(firstCall()[1].method).toBe("PUT");
  });
  ...
  it("calls getBookingStats", async () => {
    globalThis.fetch = vi.fn().mockResolvedValue({
      ...
      status: 200,
      json: () =>
        Promise.resolve({ success: true, data: { total_bookings: 10 } }),
    });
  ...
  it("calls geofence endpoints", async () => {
    globalThis.fetch = vi.fn().mockResolvedValue({
      ...
      status: 200,
      json: () =>
        Promise.resolve({ success: true, data: { checked_in: true } }),
    });
  ...
  it("calls absence approval endpoints", async () => {
    ...
    await api.submitAbsenceRequest({
  ...
  it("calls rescheduleBooking", async () => {
    globalThis.fetch = vi.fn().mockResolvedValue({
      ...
      json: () =>
        Promise.resolve({
          success: true,
          data: { booking_id: "b1", success: true },
        }),
    });
  ...
  it("calls widget endpoints", async () => {
    ...
    globalThis.fetch = vi.fn().mockResolvedValue({
      ...
      status: 200,
      json: () =>
        Promise.resolve({ success: true, data: { widget_id: "w1", data: {} } }),
    });
  ...
  it("calls dynamic pricing endpoints", async () => {
    globalThis.fetch = vi.fn().mockResolvedValue({
      ...
      status: 200,
      json: () =>
        Promise.resolve({ success: true, data: { current_price: 3.5 } }),
    });
    ...
    await api.updateAdminDynamicPricing("l1", { enabled: true });
    expect(firstCall()[1].method).toBe("PUT");
  });
  ...
  it("calls operating hours endpoints", async () => {
    ...
    await api.updateAdminLotHours("l1", { is_24h: false });
    expect(firstCall()[1].method).toBe("PUT");
  });
  ...
  it("calls translation endpoints", async () => {
    ...
    expect(firstCall()[0]).toBe(
      "/api/v1/translations/proposals?status=pending",
    );
  ...
  it("calls voteOnProposal and reviewProposal", async () => {
    ...
    await api.reviewProposal("p1", { status: "approved" });
    expect(firstCall()[1].method).toBe("PUT");
  });
  ...
  it("calls getDemoConfig and getDemoStatus", async () => {
    ...
    globalThis.fetch = vi.fn().mockResolvedValue({
      ...
      json: () =>
        Promise.resolve({
          success: true,
          data: { votes: 0, threshold: 3, last_reset: null, can_vote: true },
        }),
    });
  ...
  it("calls getUserCredits and getUserStats", async () => {
    ...
    globalThis.fetch = vi.fn().mockResolvedValue({
      ...
      status: 200,
      json: () =>
        Promise.resolve({ success: true, data: { total_bookings: 5 } }),
    });
  ...
  it("calls adminStats", async () => {
    globalThis.fetch = vi.fn().mockResolvedValue({
      ...
      status: 200,
      json: () =>
        Promise.resolve({ success: true, data: { total_users: 100 } }),
    });
  ...
  it("calls completeSetup", async () => {
    ...
    await api.completeSetup({ admin_password: "test" });
    expect(firstCall()[1].method).toBe("POST");
  ...
  it("calls createTranslationProposal", async () => {
    ...
    await api.createTranslationProposal({ key: "test.key", value: "Test" });
    expect(firstCall()[1].method).toBe("POST");
  });
    ...
    globalThis.fetch = vi.fn().mockImplementation(() => {
      ...
      if (callCount < 2) {
        return Promise.resolve({
          ...
          json: () =>
            Promise.resolve({
              error: { code: "HTTP_504", message: "Timeout" },
            }),
        });
      }
      return Promise.resolve({
  ...
  it("importAbsenceIcal sends FormData with file", async () => {
    const mockFile = new File(["ical data"], "calendar.ics", {
      type: "text/calendar",
    });
    globalThis.fetch = vi.fn().mockResolvedValue({
