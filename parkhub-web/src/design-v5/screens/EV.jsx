...
export function EVV5({ navigate: _navigate }) {
    ...
    const lotsQuery = useQuery({
        ...
        queryFn: async () => {
            ...
            if (!res.success)
                throw new Error(res.error?.message ?? 'Parkhäuser konnten nicht geladen werden');
            return res.data ?? [];
        },
        staleTime: 60_000,
    ...
    const chargersQuery = useQuery({
        ...
        queryFn: async () => {
            ...
            if (!res.success)
                throw new Error(res.error?.message ?? 'Ladestationen konnten nicht geladen werden');
            return res.data ?? [];
        },
        staleTime: 15_000,
        refetchOnWindowFocus: true,
    ...
    const sessionsQuery = useQuery({
        ...
        queryFn: async () => {
            ...
            if (!res.success)
                throw new Error(res.error?.message ?? 'Sitzungen konnten nicht geladen werden');
            return res.data ?? [];
        },
        staleTime: 15_000,
        refetchOnWindowFocus: true,
    ...
    const startMutation = useMutation({
        mutationFn: async (chargerId) => {
            const res = await api.startCharging(chargerId);
            if (!res.success)
                throw new Error(res.error?.message ?? 'Laden starten fehlgeschlagen');
            return res.data;
        },
        onSuccess: () => {
    ...
    const stopMutation = useMutation({
        mutationFn: async (chargerId) => {
            ...
            if (!res.success)
                throw new Error(res.error?.message ?? 'Laden stoppen fehlgeschlagen');
            return res.data;
        },
        onSuccess: () => {
            qc.invalidateQueries({ queryKey: ['ev-chargers'] });
    ...
    if (isLoading) {
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: 10 }}>
          {[0, 1, 2, 3].map((i) => (<div key={i} style={{ height: 130, borderRadius: 14, background: 'var(--v5-sur2)', animation: 'ph-v5-pulse 1.6s ease infinite', animationDelay: `${i * 0.08}s` }}/>))}
        </div>
      </div>);
    }
    if (isError) {
    ...
    return (<div style={{ padding: 16, flex: 1, overflow: 'auto', display: 'flex', flexDirection: 'column', gap: 12 }}>
      <div className="v5-ani" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10, flexWrap: 'wrap' }}>
        ...
        {lots.length > 1 && (<select aria-label="Parkhaus" value={activeLot} onChange={(e) => setSelectedLot(e.target.value)} style={{
                padding: '6px 10px',
                borderRadius: 9,
      ...
      {chargers.length === 0 ? (<Card className="v5-ani" style={{ padding: 36, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 10, animationDelay: '0.12s' }}>
        ...
        </Card>) : (<div className="v5-ani" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(240px, 1fr))', gap: 10, animationDelay: '0.12s' }}>
          {chargers.map((c, i) => {
                ...
                return (<Card key={c.id} data-testid="charger-card" className="v5-ani" style={{ padding: '14px 16px', display: 'flex', flexDirection: 'column', gap: 10, animationDelay: `${i * 0.05}s` }}>
                ...
                {c.location_hint && (<div style={{ fontSize: 11, color: 'var(--v5-mut)' }}>{c.location_hint}</div>)}
                {active && (<div style={{ fontSize: 11, color: 'var(--v5-acc)', display: 'flex', alignItems: 'center', gap: 6 }}>
                    <V5NamedIcon name="bolt" size={11} color="var(--v5-acc)"/>
                    Laden seit {formatTime(active.start_time)}
                  </div>)}
                {c.status === 'available' && !active && (<button type="button" disabled={isStarting} onClick={() => startMutation.mutate(c.id)} className="v5-btn" style={{
                            padding: '8px 14px',
                            borderRadius: 9,
                ...
                {active && (<button type="button" disabled={isStopping} onClick={() => stopMutation.mutate(c.id)} style={{
                            padding: '8px 14px',
                            borderRadius: 9,
...
function StatTile({ label, value, accent = false }) {
    return (<Card style={{ padding: 14, background: accent ? 'var(--v5-acc-muted)' : 'var(--v5-sur)' }}>
