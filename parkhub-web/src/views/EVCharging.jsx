...
function getSurfaceTone(designTheme) {
...
export function EVChargingPage() {
  ...
  const surfaceTone = isIndia ? 'india' : getSurfaceTone(designTheme);
  ...
  async function handleStart(chargerId) {
    try {
  ...
  async function handleStop(chargerId) {
    try {
  ...
  const activeSession = (chargerId) => sessions.find(s => s.charger_id === chargerId && s.status === 'active');
  ...
  return (
    ...
    >
        ...
        <div className="grid gap-6 lg:grid-cols-[1.15fr_0.85fr]">
          <div>
            <div className={`mb-3 inline-flex items-center gap-2 rounded-full px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.2em] ${
              ...
              {isVoid ? 'Void charging deck' : isIndia ? 'ParkIndia Charging Suite' : 'Marble charging suite'}
            </div>
            ...
            <div className="flex items-start gap-3">
              <div className="min-w-0 flex-1">
                <h1 className="flex items-center gap-2 text-3xl font-black tracking-[-0.04em]">
                  ...
                  {t('evCharging.title')}
                  <button onClick={() => setShowHelp(!showHelp)} className={isVoid ? 'text-white/55 hover:text-cyan-200' : isIndia ? 'text-[#FF9933]/60 hover:text-[#FF9933]' : 'text-surface-400 hover:text-primary-500'} aria-label="Help">
            ...
            <div className="mt-5 grid gap-3 sm:grid-cols-3">
              {statusCards.map((card, index) => (
                ...
                >
                  <p className={`text-[11px] font-semibold uppercase tracking-[0.18em] ${isVoid ? 'text-white/55' : isIndia ? 'text-[#FF9933]/60' : 'text-surface-500 dark:text-white/45'}`}>
                    {card.label}
                  </p>
                  <p className={`mt-2 text-2xl font-semibold tracking-[-0.03em] ${isIndia ? 'text-[#000080]' : ''}`}>{card.value}</p>
                  <p className={`mt-1 text-xs ${isVoid ? 'text-slate-300' : isIndia ? 'text-[#000080]/70' : 'text-surface-600 dark:text-surface-300'}`}>{card.meta}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className={`rounded-[24px] border p-5 ${
            ...
            <p className={`text-[11px] font-semibold uppercase tracking-[0.2em] ${isVoid ? 'text-white/45' : isIndia ? 'text-[#FF9933]/60' : 'text-surface-500 dark:text-white/45'}`}>
              {t('evCharging.routeLabel', 'Route')}
            </p>
            <div className="mt-4 space-y-4">
              <label className="block">
                <span className={`mb-2 block text-sm font-medium ${isVoid ? 'text-white' : isIndia ? 'text-[#000080]' : 'text-surface-900 dark:text-white'}`}>
              ...
              <div className={`rounded-[20px] border p-4 ${
                ...
                <p className={`text-[11px] font-semibold uppercase tracking-[0.18em] ${isVoid ? 'text-white/50' : isIndia ? 'text-[#FF9933]/60' : 'text-surface-500 dark:text-white/45'}`}>
                  {t('evCharging.opsSummary', 'Live summary')}
                </p>
                <p className={`mt-2 text-lg font-semibold tracking-[-0.03em] ${isIndia ? 'text-[#000080]' : ''}`}>
                  {selectedLotName}
                </p>
                <p className={`mt-1 text-sm ${isVoid ? 'text-slate-300' : isIndia ? 'text-[#000080]/70' : 'text-surface-600 dark:text-surface-300'}`}>
                  {chargers.length > 0
      ...
      {showHelp && (
        <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} className={`rounded-[22px] border p-4 text-sm ${
          isVoid
            ...
            : isIndia
            ? 'border-[#FF9933]/30 bg-[#FF9933]/5 text-[#000080]'
      ...
      {loading ? (
        <div className="flex justify-center py-12"><div className={`w-8 h-8 border-2 border-t-transparent rounded-full animate-spin ${isIndia ? 'border-[#FF9933]' : 'border-primary-500'}`} /></div>
      ) : chargers.length === 0 ? (
        <div className={`rounded-[24px] border py-12 text-center ${
          isVoid
            ? 'border-white/10 bg-slate-950/40 text-slate-400'
            : isIndia
            ? 'border-[#FF9933]/20 bg-white text-[#000080]/60'
          ...
          {chargers.map(ch => {
            ...
            return (
              <motion.div
                ...
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
                className={`rounded-[24px] border p-4 shadow-[0_18px_50px_-38px_rgba(15,23,42,0.22)] transition-colors ${
                  isVoid
              ...
              >
                <div className="flex items-center justify-between mb-3">
                  <div>
                    <span className={`font-semibold ${isIndia ? 'text-[#000080]' : 'text-surface-900 dark:text-white'}`}>{ch.label}</span>
                    <p className={`text-xs ${isIndia ? 'text-[#000080]/60' : 'text-surface-500 dark:text-surface-400'}`}>{selectedLotName}</p>
                  </div>
                  <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${statusColors[ch.status]}`}>
                    {t(`evCharging.status.${ch.status}`)}
                  </span>
                </div>
                <div className={`space-y-1 text-sm mb-3 ${isIndia ? 'text-[#000080]/80' : 'text-surface-500 dark:text-surface-300'}`}>
                ...
                {ch.status === 'available' && (
                  <button onClick={() => handleStart(ch.id)} className={`w-full flex items-center justify-center gap-2 text-sm py-2 rounded-xl transition font-semibold text-white ${
                ...
                {active && (
                  <div>
                    <div className={`flex items-center gap-1 text-sm mb-2 font-medium ${isIndia ? 'text-[#FF9933]' : 'text-amber-600'}`}>
                      <Clock className="w-4 h-4" />{t('evCharging.chargingSince')} {new Date(active.start_time).toLocaleTimeString()}
                    </div>
                    <button onClick={() => handleStop(ch.id)} className={`w-full flex items-center justify-center gap-2 text-sm py-2 rounded-xl transition font-semibold border-2 ${
                      isIndia ? 'border-[#FF9933]/30 text-[#FF9933] hover:bg-[#FF9933]/5' : 'border-red-300 text-red-600 hover:bg-red-50'
                    }`}>
      ...
      {sessions.length > 0 && (
        <div>
          <h2 className={`text-lg font-semibold mb-3 ${isIndia ? 'text-[#000080]' : 'text-surface-900 dark:text-white'}`}>{t('evCharging.history')}</h2>
          <div className="space-y-2">
            {completedSessions.map(s => (
              <div
                key={s.id}
                className={`flex items-center justify-between rounded-[20px] border p-3 text-sm transition-colors ${
                  isVoid
                    ? 'border-white/10 bg-slate-950/40'
                    : isIndia
                    ? 'border-[#FF9933]/10 bg-white'
                    : 'border-surface-200 bg-white dark:border-surface-700 dark:bg-surface-900/70'
                }`}
              >
                <div>
                  <span className={`font-medium ${isIndia ? 'text-[#000080]' : 'text-surface-900 dark:text-white'}`}>{new Date(s.start_time).toLocaleDateString()}</span>
                  <span className={`ml-2 ${isIndia ? 'text-[#000080]/60' : 'text-surface-500'}`}>{new Date(s.start_time).toLocaleTimeString()} - {s.end_time ? new Date(s.end_time).toLocaleTimeString() : '-'}</span>
                </div>
                <span className={`font-semibold ${isIndia ? 'text-[#FF9933]' : 'text-green-600'}`}>{s.kwh_consumed.toFixed(1)} kWh</span>
              </div>
            ))}
          </div>
        </div>
    ...
    >
        ...
        <div className="grid gap-6 lg:grid-cols-[1.15fr_0.85fr]">
          <div>
            <div className={`mb-3 inline-flex items-center gap-2 rounded-full px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.2em] ${
              ...
              <Lightning weight="fill" className="h-3.5 w-3.5" />
              {isVoid ? 'Void fleet monitor' : isIndia ? 'ParkIndia Fleet Monitor' : 'Marble ops overview'}
            </div>
            <h1 className="flex items-center gap-2 text-3xl font-black tracking-[-0.04em]">
              <Lightning weight="duotone" className="h-7 w-7 text-yellow-500" />
          ...
          <div className={`rounded-[24px] border p-5 ${
            ...
            <div className="mt-4 grid gap-3 sm:grid-cols-2">
              <div className={`rounded-[20px] border p-4 ${
                ...
                <p className={`mt-2 text-2xl font-semibold tracking-[-0.03em] ${isIndia ? 'text-[#000080]' : ''}`}>{stats?.total_chargers ?? '—'}</p>
              </div>
              <div className={`rounded-[20px] border p-4 ${
                isVoid
                  ? 'border-white/10 bg-white/[0.04]'
                ...
                <p className={`mt-2 text-2xl font-semibold tracking-[-0.03em] ${isIndia ? 'text-[#000080]' : ''}`}>{stats?.total_sessions ?? '—'}</p>
              </div>
            </div>
          </div>
        </div>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
          {[
            { label: t('evCharging.totalChargers'), value: stats.total_chargers, color: isIndia ? 'text-[#000080]' : 'text-surface-900 dark:text-white' },
            { label: t('evCharging.status.available'), value: stats.available, color: isIndia ? 'text-[#FF9933]' : 'text-green-600' },
            ...
            { label: t('evCharging.totalSessions'), value: stats.total_sessions, color: isIndia ? 'text-[#000080]' : 'text-primary-600' },
            { label: t('evCharging.totalKwh'), value: `${stats.total_kwh.toFixed(0)} kWh`, color: 'text-yellow-600' },
          ].map((s, i) => (
            <div
              key={i}
              className={`rounded-[22px] border p-4 text-center transition-colors ${
                isVoid
                  ? 'border-white/10 bg-slate-950/40'
                  : isIndia
                  ? 'border-[#FF9933]/10 bg-white'
            ...
            >
              ...
              <div className={`text-sm mt-1 ${isIndia ? 'text-[#000080]/60' : 'text-surface-500'}`}>{s.label}</div>
            </div>
          ))}
        </div>
      )}
