...
test.describe("Profile Page", () => {
  ...
  test("displays user name and email", async ({ page }) => {
    ...
    await expect(page.getByText(/admin@parkhub\.test|Admin/)).toBeVisible({
      timeout: 15_000,
    });
  });
  ...
  test("shows password change section", async ({ page }) => {
    await expect(
      page.getByText(/Passwort ändern|Change Password/i),
    ).toBeVisible({ timeout: 15_000 });
  });
  ...
  test("password section expands on click", async ({ page }) => {
    ...
    await expect(
      page.getByLabel(/Aktuelles Passwort|Current Password/i),
    ).toBeVisible({ timeout: 10_000 });
    await expect(page.getByLabel(/Neues Passwort|New Password/i)).toBeVisible();
  });

  test("shows GDPR section", async ({ page }) => {
    await expect(page.getByText("DSGVO / GDPR")).toBeVisible({
      timeout: 15_000,
    });
    await expect(
      page.getByText(/Daten exportieren|Data Export/i),
    ).toBeVisible();
  ...
  test("shows stats cards", async ({ page }) => {
    ...
    await expect(page.getByText(/Buchungen|Bookings/i).first()).toBeVisible({
      timeout: 15_000,
    });
  });
});
