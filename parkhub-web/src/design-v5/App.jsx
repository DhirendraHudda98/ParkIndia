...
import { breadcrumbFor, byId } from './nav';
...
function SettingsBridge() {
    ...
    useEffect(() => {
        if (settings.appearance.mode !== mode)
...
function V5Shell() {
    ...
    const [screen, setScreen] = useState(() => {
        ...
        const storedFallback = stored && byId.has(stored) ? stored : DEFAULT_SCREEN;
    ...
    return (<div style={{
      ...
      <div style={{
        ...
        <V5TopBar title={meta?.label ?? ''} breadcrumb={breadcrumbFor(screen)} onOpenCommand={() => setCmdOpen(true)} onToggleAssistant={() => setAssistantOpen((o) => !o)} assistantOpen={assistantOpen}/>
        <div style={{ flex: 1, display: 'flex', overflow: 'hidden' }}>
          <main key={screen} 
    // Feature: viewTransitions — apply fade-up animation only when on.
    className={settings.features.viewTransitions ? 'v5-ani' : undefined} style={{
            flex: 1,
            overflow: 'hidden',
            display: 'flex',
            flexDirection: 'column',
