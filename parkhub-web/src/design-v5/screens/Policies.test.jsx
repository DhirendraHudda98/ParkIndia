...
vi.mock('../../api/client', () => ({
    api: {
        getPolicies: (...a) => mockList(...a),
        updatePolicy: (...a) => mockUpdate(...a),
    },
}));
...
describe('PoliciesV5', () => {
    ...
    it('selects first policy into editor', async () => {
        ...
        renderScreen();
        await waitFor(() => expect(screen.getByTestId('policies-editor').value).toBe('Alter Text'));
    });
    it('clicking another policy switches editor content', async () => {
        ...
        await waitFor(() => expect(screen.getByTestId('policies-editor').value).toBe('DSGVO'));
    });
