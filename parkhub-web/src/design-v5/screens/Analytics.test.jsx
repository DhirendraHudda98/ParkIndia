...
vi.mock('../../api/client', () => ({
    api: {
        getAdminStatsExtended: (...a) => mockStats(...a),
    },
}));
...
vi.mock('uplot', () => {
    class FakeUPlot {
        constructor(_opts, _data, target) {
...
describe('AnalyticsV5', () => {
    ...
    it('renders both charts as uPlot canvases with aria-labels', async () => {
        ...
        await waitFor(() => expect(screen.getByRole('img', { name: /Auslastung nach Stunde/ })).toBeInTheDocument());
        ...
        await waitFor(() => expect(container.querySelectorAll('canvas').length).toBeGreaterThanOrEqual(2));
...
describe('AnalyticsV5 — stable uPlot data reference (mirrors parkhub-rust #379)', () => {
    ...
    it('ChartBlock memoizes the `data` tuple with [xs, ys] deps', () => {
        ...
        expect(ANALYTICS_SRC).toMatch(/const\s+data\s*=\s*useMemo\s*<\s*\[\s*number\[\]\s*,\s*number\[\]\s*\]\s*>\s*\(\s*\(\s*\)\s*=>\s*\[\s*xs\s*,\s*ys\s*\]\s*,\s*\[\s*xs\s*,\s*ys\s*\]\s*\)/);
    });
...
describe('AnalyticsV5 — Lighthouse LCP budget (lazy uPlot)', () => {
    it('Analytics.tsx dynamically imports UPlotChart via React.lazy', () => {
        ...
        expect(ANALYTICS_SRC).toMatch(/lazy\(\s*\(\)\s*=>\s*import\(\s*['"]\.\.\/primitives\/UPlotChart['"]\s*\)/);
        // No eager `UPlotChart` import from '../primitives' barrel either.
        expect(ANALYTICS_SRC).not.toMatch(/import\s*{[^}]*\bUPlotChart\b[^}]*}\s*from\s*['"]\.\.\/primitives['"]/);
    });
    ...
    it('primitives barrel does NOT re-export UPlotChart as a runtime binding', () => {
        ...
        expect(PRIMITIVES_SRC).not.toMatch(/export\s*{[^}]*\bUPlotChart\b(?![A-Za-z])[^}]*}\s*from\s*['"]\.\/UPlotChart['"]/);
        expect(PRIMITIVES_SRC).not.toMatch(/import\s*{[^}]*\bUPlotChart\b(?![A-Za-z])[^}]*}\s*from\s*['"]\.\/UPlotChart['"]/);
    });
});
