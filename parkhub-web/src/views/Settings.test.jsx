...
vi.mock('../components/ui/SettingsPrimitives', () => ({
    SCard: ({ title, subtitle, children }) => (<section>
      <h2>{title}</h2>
      <p>{subtitle}</p>
      {children}
    </section>),
    SRow: ({ title, description, children }) => (<div>
      <h3>{title}</h3>
      <p>{description}</p>
      {children}
    </div>),
    SSeg: ({ options, onChange }) => (<div>
      {options.map((option) => (<button key={option.value} type="button" onClick={() => onChange(option.value)}>
          {option.label}
        </button>))}
    </div>),
    SToggle: ({ value, onChange, label }) => (<button type="button" aria-pressed={value} onClick={() => onChange(!value)}>
      {label}
    </button>),
    ThemeSwatches: ({ options, onChange }) => (<div>
      {options.map((option) => (<button key={option.value} type="button" onClick={() => onChange(option.value)}>
          {option.label}
        </button>))}
    </div>),
    NavLayoutGrid: ({ onChange }) => (<button type="button" onClick={() => onChange('classic')}>
      Choose classic
    </button>),
}));
