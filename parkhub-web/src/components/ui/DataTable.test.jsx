...
describe("DataTable", () => {
  ...
  it("filters data with global search (includesString mode)", () => {
    render(
      <DataTable data={sampleData} columns={columns} searchValue="alice" />,
    );
    expect(screen.getByText("Alice")).toBeInTheDocument();
  ...
  it("handles onRowClick callback", () => {
    const onClick = vi.fn();
    render(
      <DataTable data={sampleData} columns={columns} onRowClick={onClick} />,
    );
  ...
  it("exports CSV on button click", () => {
    ...
    render(
      <DataTable data={sampleData} columns={columns} exportFilename="users" />,
    );
