...
vi.mock("react-router-dom", () => ({
  ...
  NavLink: ({ to, children, end, onClick, className, ...props }) => {
...
vi.mock("framer-motion", () => ({
  motion: {
    ...
    aside: React.forwardRef(
      (
        { children, initial, animate, exit, transition, onDragEnd, ...props },
        ref,
      ) => (
        <aside
          ref={ref}
          ...
          onDragEnd={(event) =>
            onDragEnd?.(
              event,
              event.detail ?? { offset: { x: 0 }, velocity: { x: 0 } },
            )
          }
        >
          {children}
...
describe("Layout", () => {
  ...
  it("persists collapsed section state in localStorage", async () => {
    ...
    if (!firstSettings) throw new Error("Settings header not found");
    await user.click(firstSettings);
    expect(window.localStorage.getItem("parkhub_sidebar_settings_open")).toBe(
      "true",
    );
    ...
    expect(window.localStorage.getItem("parkhub_sidebar_fleet_open")).toBe(
      "false",
    );
  });
  ...
  it("mobile sidebar clicking nav link closes sidebar", async () => {
    ...
    if (firstLink) {
      ...
      expect(
        screen.queryByLabelText("Navigation menu"),
