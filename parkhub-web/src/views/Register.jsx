...
export function RegisterPage() {
  ...
  return (
    <main className={`min-h-dvh flex items-center justify-center px-6 py-12 ${
      isIndia ? 'bg-[#FFFAF0] dark:bg-[#0A0A1A]' : 'bg-white dark:bg-surface-950'
    }`}>
      <motion.div
      ...
      >
        <Link to="/login" className={`inline-flex items-center gap-2 text-sm mb-8 transition-colors ${
          isIndia ? 'text-[#000080] hover:text-[#FF9933]' : 'text-surface-500 hover:text-primary-600'
        }`}>
        ...
        <div className="flex items-center gap-3 mb-8">
          <div className={`w-9 h-9 rounded-lg flex items-center justify-center ${
            isIndia ? 'bg-[#FF9933]' : 'bg-primary-600'
          }`}>
            <CarSimple weight="fill" className="w-5 h-5 text-white" />
          </div>
          <span className={`text-lg font-bold tracking-tight ${
            isIndia ? 'text-[#000080] dark:text-white' : 'text-surface-900 dark:text-white'
          }`}>ParkIndia</span>
        </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-5" noValidate>
          ...
          <button type="submit" disabled={isSubmitting} className={`btn w-full py-2.5 disabled:opacity-50 ${
            isIndia ? 'bg-[#FF9933] hover:bg-[#E68A00] text-white font-bold' : 'btn-primary'
          }`}>
        ...
        <p className="text-center text-sm text-surface-500 dark:text-surface-400 mt-6">
          ...
          <Link to="/login" className={`${
            isIndia ? 'text-[#000080] dark:text-[#FF9933]' : 'text-primary-600 dark:text-primary-400'
          } font-medium hover:underline`}>{t('auth.signIn')}</Link>
        </p>
      </motion.div>
    </main>
  );
