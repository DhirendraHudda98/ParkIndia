...
vi.mock('@phosphor-icons/react', () => ({
    ...
    Gear: (props) => <span data-testid="icon-gear" {...props}/>,
...
describe('AdminPluginsPage', () => {
    beforeEach(() => {
        global.fetch = vi.fn((url) => {
            if (url.includes('/config')) {
                return Promise.resolve({ json: () => Promise.resolve({ success: true, data: { webhook_url: '', channel: '#parking' } }) });
            }
            return Promise.resolve({ json: () => Promise.resolve({ success: true, data: samplePlugins }) });
        });
    });
    afterEach(() => {
        vi.restoreAllMocks();
    ...
    it('shows empty state when no plugins', async () => {
        global.fetch = vi.fn(() => Promise.resolve({ json: () => Promise.resolve({ success: true, data: { plugins: [], total: 0, enabled: 0 } }) }));
        render(<AdminPluginsPage />);
        await waitFor(() => {
    ...
    it('opens config and edits boolean field', async () => {
        global.fetch = vi.fn((url) => {
            if (url.includes('/auto-assign-preferred/config')) {
                return Promise.resolve({ json: () => Promise.resolve({ success: true, data: { fallback_to_any: true } }) });
            }
            if (url.includes('/config')) {
                return Promise.resolve({ json: () => Promise.resolve({ success: true, data: { webhook_url: '', channel: '#parking' } }) });
            }
    ...
    it('opens config and edits text field', async () => {
        ...
        expect(textField.value).toBe('https://new.com');
    });
    it('saves config', async () => {
        ...
        await waitFor(() => {
            expect(global.fetch).toHaveBeenCalledWith(expect.stringContaining('/config'), expect.objectContaining({ method: 'PUT' }));
        });
    });
    it('save config network error', async () => {
        global.fetch = vi.fn((url, opts) => {
            if (opts?.method === 'PUT' && url.includes('/config'))
                return Promise.reject(new Error('net'));
            if (url.includes('/config'))
                return Promise.resolve({ json: () => Promise.resolve({ success: true, data: { webhook_url: '', channel: '#parking' } }) });
            return Promise.resolve({ json: () => Promise.resolve({ success: true, data: samplePlugins }) });
