      ...
      <motion.div key="favorites-loaded" variants={container} initial="hidden" animate="show" className="space-y-6">
          ...
          <div className="grid gap-6 lg:grid-cols-[1.1fr_0.9fr]">
            <div>
              <div className={`mb-3 inline-flex items-center gap-2 rounded-full px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.2em] ${
                ...
                {isVoid ? 'Void favorites deck' : isIndia ? 'India Heritage Favorites' : 'Marble saved slots'}
              </div>
        ...
        {enriched.length === 0 ? (
          <motion.div
            ...
            className={`rounded-[24px] border px-6 py-14 text-center ${
              isVoid
                ...
                : isIndia
                ? 'border-[#FF9933]/10 bg-white shadow-lg'
          ...
          >
            ...
            <p className={`mt-5 text-base font-semibold ${isIndia ? 'text-[#000080]' : 'text-surface-900 dark:text-white'}`}>{t('favorites.empty')}</p>
            <p className={`mt-2 text-sm ${isVoid ? 'text-slate-300' : isIndia ? 'text-[#000080]/60' : 'text-surface-500 dark:text-surface-400'}`}>
                  ...
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex items-start gap-3">
                      <div className={`mt-0.5 flex h-11 w-11 items-center justify-center rounded-[18px] ${
                        isAvailable
                          ? isIndia ? 'bg-[#138808]/10 text-[#138808]' : 'bg-emerald-500/12 text-emerald-600 dark:text-emerald-300'
                          : isVoid
                            ? 'bg-white/[0.06] text-slate-300'
                            : isIndia ? 'bg-[#000080]/5 text-[#000080]/40' : 'bg-surface-100 text-surface-500 dark:bg-surface-800 dark:text-surface-300'
                      }`}>
                      ...
                      <div>
                        <div className="flex flex-wrap items-center gap-2">
                          <p className={`text-lg font-semibold tracking-[-0.03em] ${isIndia ? 'text-[#000080]' : 'text-surface-950 dark:text-white'}`}>
                            {t('favorites.slot')} {favorite.slot_number || '—'}
                          </p>
                          <span className={`rounded-full px-3 py-1 text-xs font-semibold ${
                            isAvailable
                              ? isIndia ? 'bg-[#138808]/10 text-[#138808]' : 'bg-emerald-500/10 text-emerald-700 dark:text-emerald-300'
                              : isVoid
                                ? 'bg-white/[0.06] text-slate-300'
                        ...
                        <div className={`mt-2 flex items-center gap-1.5 text-sm ${isVoid ? 'text-slate-300' : isIndia ? 'text-[#000080]/60' : 'text-surface-500 dark:text-surface-400'}`}>
                    ...
                    <button
                      ...
                      className={`rounded-xl p-2 transition-colors ${
                        isVoid
                          ...
                          : isIndia ? 'text-[#FF9933] hover:bg-[#FF9933]/5' : 'text-surface-400 hover:bg-red-50 hover:text-red-600 dark:hover:bg-red-900/20'
                      }`}
                  ...
                  {favorite.created_at && (
                    <p className={`mt-4 border-t pt-4 text-xs ${
                      isVoid
                        ...
                        : isIndia ? 'border-[#FF9933]/10 text-[#000080]/40' : 'border-surface-100 text-surface-400 dark:border-surface-800 dark:text-surface-500'
                    }`}>
  ...
  return (
    <div className={`rounded-[20px] border px-4 py-3 backdrop-blur ${
      isVoid
        ...
        : isIndia
          ? accent
            ? 'border-[#FF9933] bg-white/85'
            : 'border-[#000080]/10 bg-white/80'
          : accent
            ? 'border-emerald-200 bg-white/85 dark:border-emerald-900/40 dark:bg-white/[0.04]'
            : 'border-white/80 bg-white/80 dark:border-white/10 dark:bg-white/[0.04]'
    }`}>
      <p className={`text-[11px] font-semibold uppercase tracking-[0.16em] ${isVoid ? 'text-white/50' : isIndia ? 'text-[#FF9933]' : 'text-surface-500 dark:text-white/45'}`}>
        {label}
      </p>
      <div className="mt-2 flex items-end justify-between gap-3">
        <span className={`text-2xl font-semibold tracking-[-0.03em] ${isIndia ? 'text-[#000080]' : ''}`}>{value}</span>
        <span className={`text-xs ${isVoid ? 'text-slate-300' : isIndia ? 'text-[#000080]/50' : 'text-surface-500 dark:text-surface-400'}`}>{meta}</span>
      </div>
    </div>
  );
}

function DetailMetric({ label, value, isVoid, isIndia }) {
  return (
    <div className={`rounded-[18px] border px-4 py-3 ${
      isVoid
        ? 'border-white/10 bg-slate-950/70'
        : isIndia ? 'border-[#FF9933]/10 bg-[#000080]/5' : 'border-surface-200 bg-surface-50 dark:border-surface-800 dark:bg-surface-900/70'
    }`}>
      <p className={`text-[11px] font-semibold uppercase tracking-[0.16em] ${isVoid ? 'text-white/45' : isIndia ? 'text-[#FF9933]' : 'text-surface-500 dark:text-surface-400'}`}>
        {label}
      </p>
      <p className={`mt-2 text-sm font-medium ${isIndia ? 'text-[#000080]' : 'text-surface-950 dark:text-white'}`}>{value}</p>
    </div>
  );
}
