...
export function V5AssistantPanel({ open, suggestions = DEFAULT_SUGGESTIONS, stats = DEFAULT_STATS, }) {
    const toast = useV5Toast();
    if (!open)
        return null;
    return (<aside aria-label="Assistent" className="v5-aniR" style={{
            width: 260,
      ...
      <div style={{
        ...
        {suggestions.map((s, i) => {
            ...
            return (<article key={s.title} style={{
              ...
              <button type="button" className="v5-btn" onClick={() => toast(`${s.action} ausgeführt`, 'success')} style={{
                    padding: '5px 10px',
                    borderRadius: 7,
        ...
        {stats.map((s, i) => (<div key={s.label} style={{
            ...
            <span className="v5-mono" style={{ color: s.color, fontWeight: 700 }}>
              {s.value}
            </span>
          </div>))}
      </div>
