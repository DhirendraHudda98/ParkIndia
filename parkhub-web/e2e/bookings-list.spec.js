...
test.describe("Bookings List", () => {
  ...
  test("bookings page shows search input", async ({ page }) => {
    await page.goto("/bookings");
    await page.waitForLoadState("networkidle");
    await expect(page.getByPlaceholder("Search lot...")).toBeVisible({
      timeout: 15_000,
    });
  });

  test("bookings page shows active bookings section or empty state", async ({
    page,
  }) => {
    await page.goto("/bookings");
    await page.waitForLoadState("networkidle");
    ...
    const content = page
      .getByText(/Active|Upcoming|Past|No bookings|no active|no upcoming/i)
      .first();
