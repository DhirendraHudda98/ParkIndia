...
export function BookingsPage() {
  ...
  async function handleDownloadInvoice(booking) {
    const loadingToast = toast.loading(t('bookings.downloadingInvoice', 'Preparing your invoice...'));
    try {
      const blob = await api.getBookingInvoicePdf(booking.id);
      ...
      toast.success(t('bookings.invoiceDownloaded', 'Invoice downloaded successfully'), { id: loadingToast });
    } catch (err) {
      console.error('Invoice download failed:', err);
      toast.error(t('bookings.invoiceDownloadError', 'Failed to download invoice'), { id: loadingToast });
    }
  }
  ...
  const isActiveOrConfirmed = (status) => status === 'active' || status === 'confirmed';
  const now = Date.now();
    ...
    <AnimatePresence mode="wait">
      ...
      >
        <motion.div
          ...
          className={`overflow-hidden rounded-3xl border p-5 shadow-[0_24px_80px_-48px_rgba(15,23,42,0.45)] ${
            navLayout === 'focus'
              ? 'border-slate-800/80 bg-linear-to-br from-slate-950 via-slate-900 to-slate-950 text-white'
              : 'border-surface-200/70 bg-linear-to-br from-white via-white to-primary-50/60 dark:border-surface-800 dark:from-surface-950 dark:via-surface-950 dark:to-primary-950/20'
          }`}
        >
...
function Empty({ text, showAction, t }) {
  if (showAction) {
    return (
      <div className="rounded-3xl border border-surface-200 bg-white p-8 text-center shadow-[0_24px_60px_-48px_rgba(15,23,42,0.55)] dark:border-surface-800 dark:bg-surface-950/70">
        <div
  ...
  return (
    <div className="rounded-3xl border border-dashed border-surface-300 bg-white/90 p-6 text-left dark:border-surface-700 dark:bg-surface-900/80">
...
function BookingList({
  ...
  return (
    <div className="overflow-hidden rounded-3xl border border-surface-200 bg-white/95 shadow-[0_24px_60px_-48px_rgba(15,23,42,0.55)] dark:border-surface-800 dark:bg-surface-950/80">
