...
test.describe("Admin Webhooks", () => {
  ...
  test("navigate to admin webhooks page", async ({ page }) => {
    ...
    await expect(page.getByText("Webhooks v2")).toBeVisible({
      timeout: 15_000,
    });
  });
  ...
  test("webhook form shows event toggle buttons", async ({ page }) => {
    ...
    await expect(
      page
        .getByRole("button", { name: "booking.created" })
        .or(page.getByRole("button", { name: "booking.cancelled" })),
    ).toBeVisible({ timeout: 10_000 });
  });
  ...
  test("webhooks page is reachable via admin tab navigation", async ({
    page,
  }) => {
    ...
    await expect(
      page
        ...
        .or(page.getByRole("link", { name: /Webhook/i })),
    ).toBeVisible({ timeout: 15_000 });
