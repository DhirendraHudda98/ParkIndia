...
function inferStatus(absenceType) {
    if (!absenceType)
        return 'booked';
    if (absenceType === 'homeoffice')
        return 'home';
    if (absenceType === 'vacation' || absenceType === 'sick' || absenceType === 'other')
        return 'absent';
    return 'available';
}
...
function groupByUser(list) {
    ...
    for (const entry of list) {
        if (!groups[entry.user_name])
            groups[entry.user_name] = [];
...
export function TeamV5({ navigate: _navigate }) {
    const { data: entries = [], isLoading, isError } = useQuery({
        queryKey: ['team-absences'],
        queryFn: async () => {
            const res = await api.teamAbsences();
            if (!res.success)
                throw new Error(res.error?.message ?? 'Team konnte nicht geladen werden');
            return res.data ?? [];
        },
        staleTime: 30_000,
    ...
    const todayEntries = useMemo(() => entries.filter((e) => e.start_date <= todayStr && e.end_date >= todayStr), [entries, todayStr]);
    const upcomingEntries = useMemo(() => entries.filter((e) => e.start_date > todayStr).sort((a, b) => a.start_date.localeCompare(b.start_date)), [entries, todayStr]);
    ...
    if (isLoading) {
        ...
        {[0, 1, 2].map((i) => (<div key={i} style={{ height: 70, borderRadius: 12, background: 'var(--v5-sur2)', marginBottom: 8, animation: 'ph-v5-pulse 1.6s ease infinite', animationDelay: `${i * 0.1}s` }}/>))}
      </div>);
    }
    if (isError) {
    ...
    <div role="region" aria-label="Team-Übersicht" tabIndex={0} style={{ padding: 16, flex: 1, overflow: 'auto', display: 'flex', flexDirection: 'column', gap: 12 }}>
      ...
      <Card className="v5-ani" style={{ overflow: 'hidden', animationDelay: '0.12s' }} data-testid="team-roster">
        ...
        {members.map((m, i) => (<div key={m.name} data-testid="team-row" className="v5-row" style={{
                display: 'grid',
                gridTemplateColumns: '40px 1fr 120px 100px 100px',
                padding: '10px 16px',
      ...
      {todayEntries.length > 0 && (<Card className="v5-ani" style={{ padding: 14, animationDelay: '0.16s' }} data-testid="today-absences">
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
            {todayEntries.map((e, i) => (<div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 12, color: 'var(--v5-txt)' }}>
