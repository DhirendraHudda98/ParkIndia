...
vi.mock('../api/client', () => ({
    api: {
        calendarEvents: (...args) => mockCalendarEvents(...args),
        generateCalendarToken: (...args) => mockGenerateCalendarToken(...args),
        rescheduleBooking: (...args) => mockRescheduleBooking(...args),
    },
}));
...
describe('CalendarPage', () => {
    ...
    it('opens subscribe modal and shows URL on click', async () => {
        ...
        await waitFor(() => {
            ...
            expect(screen.getByTestId('subscription-url')).toHaveValue('http://localhost:3000/api/v1/calendar/ical/test-token-123');
        });
    });
    ...
    it('handles copy link in subscribe modal', async () => {
        ...
        await waitFor(() => {
            expect(mockWriteText).toHaveBeenCalledWith('http://localhost:3000/api/v1/calendar/ical/test-token-123');
        });
    });
    ...
    it('subscribe modal closes when clicking backdrop', async () => {
        ...
        const modal = screen.getByText('Subscribe to Calendar').closest('.fixed.inset-0');
